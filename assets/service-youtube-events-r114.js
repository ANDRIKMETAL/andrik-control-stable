(() => {
  ...
  function render(data={}){
    ...
    set('youtubeEventsFastCheck',`Cron 2 мин (комментарии/LIVE): ${fmt(fast.lastCheckAt)}${fastAge} · лайки ~5 мин · подписки/отписки ~5 мин: ${subscriberPoll.lastCheckAt?fmt(subscriberPoll.lastCheckAt):'ожидает'} · последний доставленный total ${Number(subscriberPoll.lastNotifiedTotal||0)}`);
    ...
    set('youtubeEventsFastResult',`Быстрый результат: ${fast.recoveredByFull?'восстановлен полным контролем':fastStatus==='success'?'успех':fastStatus==='warning'?'предупреждение':fastStatus==='failed'?'ОШИБКА':'ожидает'} · отправлено ${Number(fast.summary?.sent||0)} · LIVE-чат ${Number(fast.summary?.liveChatSent||0)} · эфир ${fast.summary?.liveVideoPinned?'закреплён':'—'} · лайки ${fast.summary?.skippedQuota?'следующая ~5-мин проверка':'проверены'} · ошибок ${Number(fast.summary?.failed||0)}${commentNote} · восстановлено stale ${Number(fast.staleLikeClaims||0)}`);
