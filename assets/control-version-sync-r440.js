/* ANDRIK Control R440 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R440__)return;
  window.__ANDRIK_VERSION_SYNC_R440__=true;
  const RELEASE='R440';
  const RUNTIME='55.00-r440';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R440 R439 STABLE BASE + YOUTUBE DAILY VIEW LIVE COUNTER + SUMMARY ARCHIVE + ADMIN BALANCE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:439,version:'55.00'}}));
    }finally{busy=false;}
  };
