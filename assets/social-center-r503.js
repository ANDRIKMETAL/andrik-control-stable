/* ANDRIK R503 — Social Center: TikTok Sandbox / Production OAuth switch + real Display API snapshots.
   Credentials, tokens and history are isolated per TikTok environment. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R503__)return;
  window.__ANDRIK_SOCIAL_CENTER_R503__=true;
  const RELEASE='R503';
  ...
  const TIKTOK_MODE_KEY='andrik-tiktok-oauth-mode-r503';
  const normalizeTikTokMode=value=>String(value||'').toLowerCase()==='production'?'production':'sandbox';
  const readTikTokMode=()=>{try{return normalizeTikTokMode(localStorage.getItem(TIKTOK_MODE_KEY)||'sandbox')}catch(_){return'sandbox'}};
  ...
  const state={active:{site:true,youtube:true,instagram:true,facebook:true,tiktok:true},data:null,loading:false,tiktokMode:readTikTokMode()};
  ...
  async function api(refresh=false){
    ...
    try{
      const params=new URLSearchParams({tiktok_mode:state.tiktokMode});if(refresh)params.set('refresh','1');
  ...
  async function connectTikTok(){
    ...
    if(button){button.disabled=true;button.textContent=`Открываем ${state.tiktokMode==='sandbox'?'Sandbox':'Production'}…`}
    try{
  ...
  function switchTikTokMode(mode){
    ...
    state.tiktokMode=next;state.data=null;load(false);
  }
  function tiktokModeSwitchHtml(meta={}){
    const available=meta.availableModes||{};
    const mk=mode=>{const active=state.tiktokMode===mode;const ready=Boolean(available[mode]);const label=mode==='sandbox'?'SANDBOX':'PRODUCTION';return `<button type="button" class="social-center-r503-mode${active?' is-active':''}${ready?' is-ready':''}" data-tiktok-mode-r503="${mode}" aria-pressed="${active?'true':'false'}">${label}${ready?' ✓':''}</button>`};
    return `<div class="social-center-r503-mode-switch" aria-label="Среда TikTok OAuth">${mk('sandbox')}${mk('production')}</div>`;
  }
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      if(key==='instagram'){
        ...
        if(live){
          ...
          action=tiktokModeSwitchHtml(meta)+'<a class="social-center-r495-open is-tiktok" href="https://www.tiktok.com/@andrikmetal" target="_blank" rel="noopener noreferrer">Открыть TikTok ↗</a>';
        }else if(meta.configured){
          detail='OAuth готов · подключи аккаунт';
          action=tiktokModeSwitchHtml(meta)+'<button type="button" class="social-center-r503-connect" data-tiktok-connect-r503>Подключить TikTok</button>';
        }else{detail=state.tiktokMode==='sandbox'?'нужны TIKTOK_SANDBOX_CLIENT_KEY и SECRET':'нужны TIKTOK_CLIENT_KEY и SECRET';action=tiktokModeSwitchHtml(meta)};
    ...
    box.querySelector('[data-tiktok-connect-r503]')?.addEventListener('click',connectTikTok);box.querySelectorAll('[data-tiktok-mode-r503]').forEach(btn=>btn.addEventListener('click',()=>switchTikTokMode(btn.dataset.tiktokModeR503)));
  }
  ...
  function renderTikTok(data){
    ...
    const note=$('socialCenterTikTokNoteR503');if(note){const count=Number(v.fetchedVideos||0);note.textContent=(tt.mode==='sandbox'?'SANDBOX · ':'PRODUCTION · ')+(v.allVideosFetched?`срез по всем ${count} публичным роликам. Дневной прирост строится отдельно для этой среды.`:`срез по последним ${count} роликам. Дневной прирост строится отдельно для этой среды.`)}
  }
