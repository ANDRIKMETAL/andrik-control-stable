/* ANDRIK Control R482 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R482__)return;
  window.__ANDRIK_VERSION_SYNC_R482__=true;
  const RELEASE='R482';
  const RUNTIME='55.00-r482';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R482 R481 BASE + HOME CLIPS R2-ONLY + NO BUNDLED PROSNIS MP4';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:482,version:'55.00'}}));
    }finally{busy=false;}
  };
