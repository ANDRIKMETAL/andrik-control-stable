(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_PLATFORM_SWIPE_R512__)return;
  window.__ANDRIK_SOCIAL_PLATFORM_SWIPE_R512__=true;

  const params=new URLSearchParams(location.search);
  const path=location.pathname.toLowerCase();
  const source=String(params.get('source')||'').toLowerCase();
  const routes=[
    {name:'Обзор',url:'/social-center-admin.html?source=social-center&v=55.00-r512'},
    {name:'Сайт · GA4',url:'/analytics-admin.html?page=site&source=social-center&v=55.00-r512'},
    {name:'YouTube Studio',url:'/youtube-admin.html?source=social-center&v=55.00-r512'},
    {name:'Instagram',url:'/instagram-admin.html?source=social-center&v=55.00-r512'},
    {name:'TikTok',url:'/tiktok-admin.html?source=social-center&v=55.00-r512'}
  ];
  ...
  document.documentElement.classList.add('social-platform-swipe-r512-active');
  document.body?.setAttribute('data-social-platform-index',String(index));

  const toast=document.createElement('div');
  toast.className='social-platform-swipe-r512-toast is-hidden';
  ...
  const setDrag=dx=>{
    ...
    document.documentElement.style.setProperty('--social-swipe-r512-x',`${px}px`);
  };
  const clearDrag=()=>document.documentElement.style.removeProperty('--social-swipe-r512-x');
  const go=delta=>{
    if(navigating)return;
    navigating=true;
    const target=(index+delta+routes.length)%routes.length;
    showToast(`${target+1}/5 · ${routes[target].name}`);
    document.documentElement.classList.add('social-platform-swipe-r512-leaving');
