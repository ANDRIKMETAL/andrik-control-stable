/* ANDRIK R496 — Social Center: one-time YouTube owner-account correction.
   Reconnect control appears only for the confirmed wrong-owner OAuth state and
   disappears automatically after the correct channel owner is authorized. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R496__)return;
  window.__ANDRIK_SOCIAL_CENTER_R496__=true;
  const RELEASE='R496';
  ...
  async function youtubeOwnerReconnectR496(button){
    const original=button?.textContent||'Выбрать владельца YouTube';
    ...
    try{
      ...
      const warning=$('socialCenterWarningR488');
      if(warning){warning.hidden=false;warning.textContent=`YouTube: ${error?.message||'ошибка авторизации'}`}
    }
  }
  function youtubeNeedsOwnerCorrectionR496(meta){
    const error=String(meta?.error||'');
    return Boolean(meta?.canReconnect && meta?.oauthConnected && /не возвращает ни одного собственного YouTube-канала|не владеет каналом/i.test(error));
  }
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      const def=platformDefs[key];const future=key==='tiktok';const live=!future&&connected(key,data);
      ...
      if(key==='instagram'){
        ...
        if(youtubeNeedsOwnerCorrectionR496(meta)){
          action='<button class="social-center-r496-owner-fix" type="button" data-youtube-owner-fix-r496>Разово выбрать владельца YouTube</button>' +
            '<span class="social-center-r496-owner-note">Выбери аккаунт владельца канала. После успешного входа эта кнопка исчезнет.</span>';
        }
    ...
    box.querySelectorAll('[data-youtube-owner-fix-r496]').forEach(button=>button.addEventListener('click',()=>youtubeOwnerReconnectR496(button)));
  }
  function renderToggles(data){
    const box=$('socialCenterTogglesR488');if(!box)return;
    const liveKeys=['site','youtube','instagram'];
    const futureKeys=['tiktok'];
  ...
  function render(data){
    ...
    const total=$('socialCenterTotalR488');if(total){const igLabel=data?.platforms?.instagram?.metricLabel||'просмотры';total.innerHTML=`<span>Общий итог</span><strong>${esc(full(grand))}</strong><small>${keys.length} из 4 · Instagram: ${esc(igLabel)}</small>`;}
