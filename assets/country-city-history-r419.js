/* ANDRIK R419 — all-time city list by default + optional calendar day mode. */
(()=>{
  'use strict';
  if(window.__ANDRIK_COUNTRY_CITY_HISTORY_R419__)return;
  window.__ANDRIK_COUNTRY_CITY_HISTORY_R419__=true;
  ...
  const load=async()=>{
    ...
    try{
      ...
      const qs=new URLSearchParams({country,mode,v:'55.00-r419'});if(mode==='daily')qs.set('date',date);
