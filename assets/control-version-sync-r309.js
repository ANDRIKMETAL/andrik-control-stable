/* Control ANDRIK R309 — safe display-version synchronization.
   Never writes text into <html> or <body>. */
(() => {
  'use strict';
  if (window.__ANDRIK_VERSION_SYNC_R309__) return;
  window.__ANDRIK_VERSION_SYNC_R309__ = true;

  const RELEASE = 'R309';
  const RUNTIME = '55.00-r309';
  const FULL = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R309';
  let applying = false;
  ...
  const apply = () => {
    ...
    try {
      ...
      window.ANDRIK_CONTROL_RELEASE = Object.freeze({
        short: RELEASE, number: 309, version: '55.00',
        full: 'v55.00 LIVE WEB AI FINAL R309',
        build: 'R309 SUBSCRIBER PUSH BASELINE FIX',
        date: '08.08.2026'
      });
      window.ANDRIK_CONTROL_VERSION = '55.00 LIVE WEB AI FINAL R309';
      window.ANDRIK_CONTROL_BUILD = 'R309 SUBSCRIBER PUSH BASELINE FIX';
