/* Control ANDRIK R279 — one-time cache generation switch.
   Uses a dedicated cache key and never fights the visible release number. */
(() => {
  'use strict';
  const VERSION = '55.00-r279';
  const KEY = 'andrik-control-cache-generation';
  const ONCE = 'andrik-control-cache-reload-r279';
  ...
  (async () => {
    try {
      ...
      console.warn('Control cache generation R279:', error);
    }
  })();
})();
