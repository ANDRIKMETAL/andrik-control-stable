(()=>{
  ...
  function hasR665Agent(data=lastRemote){return /R665/i.test(agentVersion(data))}
  ...
  async function startSequence({recover=false}={}){
    ...
    try{
      setMsg(recover?'Восстанавливаю эфир через OVH…':'Запускаю эфир через OVH…','work');
      const before=await youtubeState().catch(()=>null);
      if(before&&ytLife(before)==='live'&&!recover){
      ...
      if(modern){
        setMsg('R665: временно останавливаю OVH encoder…','work');
        await agentAction('encoder-stop');
        await sleep(2500);
        const stoppedState=await youtubeState().catch(()=>before);
        if(stoppedState)await ensureAutoStartWhenEditable(stoppedState);
        setMsg('R665: запускаю OVH encoder…','work');
        await agentAction('encoder-start');
      }else{
        // Backward-safe fallback for the current R658 agent: restart is native and
        // does not call the missing /usr/local/sbin/andrik-youtube helper.
        if(before)await ensureAutoStartWhenEditable(before).catch(()=>null);
        setMsg('Старый агент: безопасно перезапускаю OVH encoder…','work');
        await agentAction('restart');
      }

      setMsg('OVH encoder активен. Перевожу YouTube в LIVE…','work');
      await sleep(3500);
      const live=await transitionLive();
      const url=live.watchUrl||'';if(url)setWatch(url);
      setResult(`R665 OVH START ✅\nYouTube: ${live.lifeCycleStatus||'LIVE'}\nStream: ${live.streamStatus||'active'}${url?`\n${url}`:''}`);
      setMsg('YouTube LIVE ✅ · OVH encoder работает','ok');
    }catch(e){
      setResult(`R665 START ERROR\n${String(e.message||e)}`);
      setMsg(`Ошибка запуска: ${e.message||e}`,'bad');
  ...
  window.AndrikRadioRemoteR665={refresh,start:()=>startSequence(),recover:()=>startSequence({recover:true}),stop:stopSequence,status:statusSequence,saveTicker};
  const boot=()=>{refresh();timer=setInterval(refresh,5000)};
