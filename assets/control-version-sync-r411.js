/* ANDRIK Control R411 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R411__)return;
  window.__ANDRIK_VERSION_SYNC_R411__=true;
  const RELEASE='R411';
  const RUNTIME='55.00-r411';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R411 VERSION SYNC + R409 CRON SAFE + SUMMARY LAYOUT';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:411,version:'55.00'}}));
    }finally{busy=false;}
  };
