(() => {
  if (window.__ANDRIK_PLAYER_SHELL_R124__) return;
  window.__ANDRIK_PLAYER_SHELL_R124__ = true;
  ...
  const setupOwnerControlEye = async () => {
    ...
    try {
      ...
      link.href = 'https://control.andrikmetal.com/control-home.html?v=55.00-r124';
      ...
      if (!document.getElementById('andrik-owner-control-eye-style')) {
        ...
        style.textContent = `
          ...
          #andrik-owner-control-eye span{position:absolute;inset:-8px;border-radius:50%;background:radial-gradient(circle,rgba(96,255,157,.42),rgba(28,218,92,.13) 52%,transparent 73%);animation:andrikOwnerHaloR124 3.05s ease-in-out infinite;z-index:0}
          #andrik-owner-control-eye img{position:relative;z-index:1;width:42px;height:42px;display:block;object-fit:cover;border-radius:50%;clip-path:circle(49% at 50% 50%);animation:andrikOwnerEyeR124 3.05s ease-in-out infinite;will-change:transform,filter;box-shadow:0 0 0 1px rgba(119,255,177,.22)}
          #andrik-owner-control-eye:active img{transform:scale(.92)!important}
          @keyframes andrikOwnerEyeR124{0%,100%{transform:scale(.93);filter:brightness(1.02) saturate(1.28) drop-shadow(0 0 7px rgba(80,255,143,.55)) drop-shadow(0 0 18px rgba(22,218,87,.38))}50%{transform:scale(1.08);filter:brightness(1.48) saturate(1.6) drop-shadow(0 0 14px rgba(146,255,187,.96)) drop-shadow(0 0 34px rgba(32,239,105,.82))}}
          @keyframes andrikOwnerHaloR124{0%,100%{transform:scale(.70);opacity:.18}50%{transform:scale(1.32);opacity:.86}}
