(() => {
  'use strict';
  if (window.__ANDRIK_SITE_UPDATE_HEADER_R266__) return;
  window.__ANDRIK_SITE_UPDATE_HEADER_R266__ = true;
  ...
  function derive() {
    ...
    if ([check, backup, commit, release].some(item => has(item,'running') || has(item,'warn'))) return 'green';
    ...
    if (completed) {
      successLatched = false;
      goldUntil = 0;
      try { sessionStorage.removeItem('andrik-header-gold-until'); } catch (_) {}
      return 'green';
    }
    return 'green';
  }
