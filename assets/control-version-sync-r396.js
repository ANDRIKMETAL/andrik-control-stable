/* ANDRIK Control R396 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R396__)return;
  window.__ANDRIK_VERSION_SYNC_R396__=true;
  const RELEASE='R396';
  const RUNTIME='55.00-r396';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R396 SUMMARY UPDATE CLOCKS + CITY MAP PARITY + VERSION LOCK';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:396,version:'55.00'}}));
    }finally{busy=false;}
  };
