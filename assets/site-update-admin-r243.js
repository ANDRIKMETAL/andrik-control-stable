(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r243';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r243';
  ...
  window.AndrikSiteUpdateR243={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR243;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR243;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
