/* Control ANDRIK R292 — safe display-version synchronization.
   Never writes text into <html> or <body>. */
(() => {
  'use strict';
  if (window.__ANDRIK_VERSION_SYNC_R292__) return;
  window.__ANDRIK_VERSION_SYNC_R292__ = true;

  const RELEASE = 'R292';
  const RUNTIME = '55.00-r292';
  const FULL = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R292';
  let applying = false;
  ...
  const apply = () => {
    ...
    try {
      ...
      window.ANDRIK_CONTROL_VERSION = '55.00 LIVE WEB AI FINAL R292';
