(() => {
  'use strict';
  if (window.__ANDRIK_UNIFIED_GLOW_R134__) return;
  window.__ANDRIK_UNIFIED_GLOW_R134__ = true;

  const RELEASE = 'R134';
  const KEY_SESSION = 'andrik-comments-admin-key';
  const KEY_LOCAL = 'andrik-comments-admin-key-persistent';
  const OWNER_SYNC_STAMP = 'andrik-owner-session-sync-r134';
  ...
  const loadPlayerBridge = () => {
    ...
    // R134: плеер работает и на странице обновления сайта.
    // Исключаются только технические cache-reset страницы.
    if (isReset) return;

    if (!document.querySelector('link[data-andrik-player-r134]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = '/assets/control-player-bridge-r134.css?v=55.00-r134';
      link.dataset.andrikPlayerR134 = 'css';
      document.head.appendChild(link);
    }

    if (!document.querySelector('script[data-andrik-player-r134]')) {
      const script = document.createElement('script');
      script.src = '/assets/control-player-bridge-r134.js?v=55.00-r134';
      script.defer = true;
      script.dataset.andrikPlayerR134 = 'js';
  ...
  const apply = () => {
    ...
    body.classList.add('unified-glow-ready', 'andrik-ui-r134');
    const params = new URLSearchParams(location.search);
    // R134: embed-класс нужен только настоящему вложенному iframe.
