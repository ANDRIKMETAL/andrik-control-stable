/* Control ANDRIK R297 — exact completed-period daily summary opened from push. */
(() => {
  ...
  const HOME_CACHE_KEY_PUSH_BASE='andrik-control-home-push-r297';
  ...
  const shell=$('controlSwipeShell');
    ...
    try{
      const query=new URLSearchParams({v:'55.00-r297'});
  ...
  $('controlSummaryNext')?.addEventListener('click',()=>{
    ...
    location.assign(`/control-home.html?page=activity${pushQuery}&v=55.00-r297&t=${Date.now()}`);
  });
