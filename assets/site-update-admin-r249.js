(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r249';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r249';
  ...
  async function fetchBackupZip(){
    ...
    try{
      ...
      let response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r249`,{credentials:'include',cache:'no-store',headers,signal:controller.signal});
      if(response.status===401&&raw){
        await window.AndrikOwnerSession?.establish?.(raw).catch(()=>null);
        response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r249-retry`,{credentials:'include',cache:'no-store',headers:{accept:'application/zip',authorization:`Bearer ${raw}`},signal:controller.signal});
      }
      if(!response.ok){
      ...
      if(error?.name==='AbortError')throw new Error('GitHub не отдал ZIP за 3 минуты. Нажмите «Повторить ZIP-бэкап».');
      throw error;
    }finally{clearTimeout(timer)}
  }
  ...
  async function shareZipBackup(){
    if(!siteBackupZipFile){setText('siteUpdateBackupZipState','Сначала создайте ZIP текущего сайта.');return;}
    const safeName=`ANDRIK-BACKUP-R249-${new Date().toISOString().slice(0,10)}.zip`;
    const variants=[
      new File([siteBackupZipFile],safeName,{type:'application/zip',lastModified:Date.now()}),
      new File([siteBackupZipFile],safeName,{type:'application/x-zip-compressed',lastModified:Date.now()}),
      new File([siteBackupZipFile],safeName,{type:'application/octet-stream',lastModified:Date.now()})
    ];
    let lastError=null;
    if(navigator.share){
      for(const shareFile of variants){
        const payload={files:[shareFile],title:'ANDRIK — ZIP-бэкап сайта'};
        let supported=true;
        try{supported=!navigator.canShare||navigator.canShare({files:[shareFile]})}catch(_){supported=true}
        if(!supported)continue;
        try{
          await navigator.share(payload);
          setText('siteUpdateBackupZipState','Системное меню «Поделиться» открыто.');
          return;
        }catch(error){
    ...
    const originalFile=siteBackupZipFile,originalUrl=siteBackupZipUrl;
    const fallbackFile=variants[0];
    try{
      siteBackupZipFile=fallbackFile;
      siteBackupZipUrl=URL.createObjectURL(fallbackFile);
      downloadZipBackup();
    }finally{
      if(siteBackupZipUrl&&siteBackupZipUrl!==originalUrl)try{URL.revokeObjectURL(siteBackupZipUrl)}catch(_){}
      siteBackupZipFile=originalFile;siteBackupZipUrl=originalUrl;
    }
    setText('siteUpdateBackupZipState',lastError?`Android не разрешил прямую передачу ZIP (${lastError.message||lastError.name}). Архив сохранён в «Загрузки» — откройте его и выберите «Поделиться».`:'На этом устройстве прямой обмен ZIP не поддерживается. Архив сохранён в «Загрузки».');
  }
  ...
  const bind=(id,type,handler,options)=>{const node=typeof id==='string'?byId(id):id;if(node)node.addEventListener(type,handler,options)};
  bind('siteUpdateVerify','click',loadStatus);
  bind('siteUpdateRefresh','click',loadStatus);
  bind('siteUpdateBackupNow','click',backupNow);
  bind('siteUpdateBackupZipCreate','click',createDownloadableZipBackup);
  bind('siteUpdateBackupZipDownload','click',downloadZipBackup);
  bind('siteUpdateBackupZipShare','click',shareZipBackup);
  bind('siteUpdateHealthCheck','click',manualHealthCheck);
  bind('siteUpdateCacheClear','click',()=>clearControlRuntimeCaches({reload:true,release:'R249',manual:true}));
  if(autoRecoveryInput)autoRecoveryInput.addEventListener('change',()=>{try{localStorage.setItem(AUTO_RECOVERY_KEY,autoRecoveryInput.checked?'1':'0')}catch(_){};setText('siteUpdateHealthMessage',autoRecoveryInput.checked?'Автооткат включён: критический сбой вернёт предыдущий backup.':'Автооткат выключен: проверка останется активной.');});
  ...
  if(publishButton)publishButton.addEventListener('click',startPublishFromGesture,{capture:true});
  if(publishButton)publishButton.addEventListener('pointerup',event=>{
    if(event.pointerType==='touch'||event.pointerType==='pen')startPublishFromGesture(event);
  },{capture:true,passive:false});
  if(publishButton)publishButton.addEventListener('touchend',startPublishFromGesture,{capture:true,passive:false});
  window.AndrikSiteUpdateR249={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR247=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR244=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR243=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR249;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR249;bind('siteUpdateHistoryRefresh','click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));bind('siteUpdateCheckDeploy','click',()=>checkDeployNow());if(keyInput){keyInput.addEventListener('input',()=>{captureKey();if(previewButton)previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});}resetStages();
