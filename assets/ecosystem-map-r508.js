/* ANDRIK R934 — technical traffic layer hard-open fix; R932 split preserved. */
(() => {
  'use strict';
  if (window.__ANDRIK_ECOSYSTEM_MAP_R934__) return;
  window.__ANDRIK_ECOSYSTEM_MAP_R934__ = true;
  ...
  const rebuildAll = () => {
    ...
    const add = (name, rows) => {
      for (const row of cleanRows(rows)) {
        ...
        const item = breakdown.get(key) || {youtube:0,site:0,instagram:0,music:0,push:0};
    ...
    state.all.points = aggregatePoints([state.site.points,state.instagram.points,state.music.points,state.push.points]);
    state.all.weekly = mergeWeekly([state.youtube.weekly,state.site.weekly,state.instagram.weekly,state.music.weekly,state.push.weekly]);
    state.all.previous = mergeWeekly([state.youtube.previous,state.site.previous,state.instagram.previous,state.music.previous,state.push.previous]);
  ...
  const decorateCountry = () => {
    ...
    if (active === 'all') {
      ...
      detail.innerHTML = `<b>Экосистема:</b> ▶ ${fmt(b.youtube)} · 🌐 ${fmt(b.site)} · ◎ ${fmt(b.instagram)}<span class="is-muted"> · ♫ ${fmt(b.music)} · 🔔 ${fmt(b.push)}${cityText}</span>`;
    } else {
  ...
  const bindTechnicalHardOpenR934 = () => {
    if (map.dataset.technicalHardOpenR934 === '1') return;
    map.dataset.technicalHardOpenR934 = '1';
    map.addEventListener('click',event=>{
      const button=event.target?.closest?.('.ecosystem-layer-switcher [data-ecosystem-layer="technical"]');
      if(!button || !map.contains(button)) return;
      // Capture phase prevents a stale pre-R932 handler from swallowing the new layer.
      ...
      window.__andrikEcosystemActiveLayer=active;
      applyLayer(true);
      // Always refresh the classified first-party payload when the gear is tapped.
      void loadEcosystem(true).then(()=>{ if(active==='technical') applyLayer(true); });
    },true);
  };

  const bindButtons = () => {
    map.querySelectorAll('.ecosystem-layer-switcher [data-ecosystem-layer]').forEach(button => {
      if (button.dataset.ecosystemBoundR934 === '1') return;
      button.dataset.ecosystemBoundR934 = '1';
  ...
  readCaches();
  rememberHistoryR452([...state.youtube.rows,...state.site.rows,...state.instagram.rows]);
  rebuildAll();
  syncOptionalLayerButtonsR506();
  bindTechnicalHardOpenR934();
  bindButtons();
  rememberLayerR382(active);
  applyLayer();
  updateAccessoryLabels();
  ...
  window.andrikEcosystemMap = { setLayer, openTechnical:()=>{setLayer('technical');return loadEcosystem(true)}, refresh:()=>loadEcosystem(true), state:()=>state };
})();
