/* Control ANDRIK R301 — stable portrait action portal.
...
(() => {
  'use strict';
  if (window.__ANDRIK_COUNTRY_ACTIONS_R301__) return;
  window.__ANDRIK_COUNTRY_ACTIONS_R301__ = true;
  ...
  const ensureConnected = () => {
    ...
    if (!actions.classList.contains('r296-country-actions')) {
      actions.classList.add('r296-country-actions');
    }
    return true;
  };
  ...
  const setVisible = visible => {
    ...
    if (visible) {
      ...
      document.body.classList.add('r296-country-actions-visible');
    } else {
      if (!actions.hidden) actions.hidden = true;
      if (actions.getAttribute('aria-hidden') !== 'true') actions.setAttribute('aria-hidden', 'true');
      document.body.classList.remove('r296-country-actions-visible');
    }
  };
