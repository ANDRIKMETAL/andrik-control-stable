/* Control ANDRIK R277 — one-time cache generation switch.
   Uses a dedicated cache key and never fights the visible release number. */
(() => {
  'use strict';
  const VERSION = '55.00-r277';
  const KEY = 'andrik-control-cache-generation';
  const ONCE = 'andrik-control-cache-reload-r276';
  ...
  (async () => {
    try {
      ...
      console.warn('Control cache generation R277:', error);
    }
  })();
})();
