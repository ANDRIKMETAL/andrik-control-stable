(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r107';
  ...
  async function finalizeDeployment(operationId='',release='',mode='publish'){
    ...
    try{
      ...
      if(data.health?.status==='ok'){
        ...
        setText('siteUpdateDeployMessage','Сайт работает. Есть некритические предупреждения — автоматический откат не требуется.');
      }else{
        stage('protect','error');
