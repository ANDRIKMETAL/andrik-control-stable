/* ANDRIK Control R451 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R451__)return;
  window.__ANDRIK_VERSION_SYNC_R451__=true;
  const RELEASE='R451';
  const RUNTIME='55.00-r451';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R451 R450 BASE + COMPLETE COUNTRY MEMORY + LOWER LARGER ADMIN GLOBE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:451,version:'55.00'}}));
    }finally{busy=false;}
  };
