(() => {
  ...
  const OWNER_SYNC_STAMP='andrik-owner-session-sync-r120';
  ...
  mainButton.addEventListener('click',()=>{
    if(!insideMainPlayer&&!dock.classList.contains('is-open')&&!state.title){location.href=playerUrl(true);return}
  ...
  document.addEventListener('click',event=>{
    ...
    if(insideMainPlayer&&url.origin===CONTROL_ORIGIN){
      ...
      url.searchParams.set('v','55.00-r120');
