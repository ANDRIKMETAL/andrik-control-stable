/* ANDRIK Control R402 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R402__)return;
  window.__ANDRIK_VERSION_SYNC_R402__=true;
  const RELEASE='R402';
  const RUNTIME='55.00-r402';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R402 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:402,version:'55.00'}}));
    }finally{busy=false;}
  };
