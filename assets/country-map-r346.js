/* ANDRIK R346 — second tap opens the standalone country silhouette. */
(() => {
  'use strict';
  if (window.__ANDRIK_COUNTRY_MAP_R346__) return;
  window.__ANDRIK_COUNTRY_MAP_R346__ = true;
  ...
  function ensureOverlay() {
    ...
    let overlay = stage.querySelector('.country-map-r346');
    if (!overlay) {
      overlay = document.createElement('div');
      overlay.className = 'country-map-r346';
  ...
  function finishWorldReturn(){
    ...
    transitionTimer = 0;
    const overlay = map.querySelector('.country-map-r346');
    ...
    if (overlay.dataset.signature !== signature) {
      ...
      if (shape?.path) {
        ...
        let dots = points.map(point => {
          ...
          return `<circle class="country-point-r346" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}"/>`;
        }).join('');
        if (!dots) dots = '<circle class="country-point-halo-r346" cx="500" cy="300" r="34"/><circle class="country-point-r346 is-empty" cx="500" cy="300" r="15"/>';
        svg = `<svg viewBox="0 0 1000 600" role="img" aria-label="Карта страны ${esc(country)}"><path class="country-shape-r346" fill-rule="evenodd" d="${shape.path}"/>${dots}</svg>`;
      } else {
        svg = `<svg viewBox="0 0 1000 600" role="img" aria-label="Карта страны ${esc(country)}"><rect x="90" y="80" width="820" height="440" rx="80" fill="rgba(20,58,78,.55)" stroke="#92dbff" stroke-width="4" stroke-dasharray="12 12"/><circle class="country-point-halo-r346" cx="500" cy="300" r="34"/><circle class="country-point-r346 is-empty" cx="500" cy="300" r="15"/></svg>`;
      }
      overlay.innerHTML = `${svg}<button class="country-map-label-r346" type="button" aria-label="Вернуться к мировой карте">🌍 Мир · ${flag(code)} ${esc(country)}</button>`;
      overlay.querySelector('.country-map-label-r346')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();clearToWorld()},{once:true});
