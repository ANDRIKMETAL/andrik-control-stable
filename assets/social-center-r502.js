/* ANDRIK R502 — Social Center: TikTok OAuth + real Display API snapshot.
   No fabricated TikTok geography or historical analytics: daily trend is built from server snapshots. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R502__)return;
  window.__ANDRIK_SOCIAL_CENTER_R502__=true;
  const RELEASE='R502';
  ...
  const platformDefs={
    ...
    facebook:{name:'Facebook',icon:'f',label:'охвата/взаимодействий'},
    tiktok:{name:'TikTok',icon:'TT',label:'просмотров видео'}
  };
  const state={active:{site:true,youtube:true,instagram:true,facebook:true,tiktok:true},data:null,loading:false};
  ...
  async function connectTikTok(){
    const button=document.querySelector('[data-tiktok-connect-r502]');
    if(button){button.disabled=true;button.textContent='Открываем TikTok…'}
    try{
      const response=await fetch('/api/control/tiktok-oauth/start',{headers:await ownerHeaders(),credentials:'include',cache:'no-store'});
      const data=await response.json().catch(()=>({}));
      if(!response.ok||!data?.url)throw new Error(data?.error||`HTTP ${response.status}`);
      location.assign(data.url);
    }catch(error){
      const warning=$('socialCenterWarningR488');if(warning){warning.hidden=false;warning.textContent=`TikTok: ${error?.message||'не удалось начать авторизацию'}`}
  ...
  function renderPlatforms(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'];
    box.innerHTML=keys.map(key=>{
      ...
      let detail=live?(meta.metricLabel||def.label):(meta.configured?'авторизация/данные ещё не получены':'готово к подключению');
      let sub='';let href=meta.profileUrl||'';let action='';let cardValue=total;
      if(key==='instagram'){
        detail=live?(meta.metricLabel||'Instagram Insights'):(meta.configured?'токен есть · нажми обновить':'нужен Instagram OAuth/Secret');
        ...
        href='';
        if(live) detail='YouTube Analytics · OAuth подключён';
        else if(meta.oauthConnected) detail=meta.error?('OAuth готов · '+String(meta.error).slice(0,62)):'OAuth подключён · обновляем аналитику';
        else if(meta.reconnectRequired) detail='Доступ Google требует обновления';
        else if(meta.error) detail='Studio: '+String(meta.error).slice(0,75);
        else detail='Нужна авторизация YouTube Studio';
        action+='<a class="social-center-r495-open" href="https://www.youtube.com/@andrikmetal" target="_blank" rel="noopener noreferrer">Открыть канал ↗</a>';
      }else if(key==='facebook'){
        sub=meta.pageName?`<span class="social-center-r488-account">${esc(meta.pageName)}</span>`:'';
        detail=live?(meta.metricLabel||'Facebook Page Insights'):(meta.configured?'Meta подключается · ждём Page Insights':'готово к Meta OAuth');
      }else if(key==='tiktok'){
        ...
        if(live){
          ...
          const sample=Number(summary.fetchedVideos||0);const exact=summary.allVideosFetched?'все публичные ролики':`последние ${sample||'—'} роликов`;
          detail=meta.trendConnected?'прирост просмотров · история собирается автоматически':`текущий срез просмотров · ${exact}`;
          action='<a class="social-center-r495-open is-tiktok" href="https://www.tiktok.com/@andrikmetal" target="_blank" rel="noopener noreferrer">Открыть TikTok ↗</a>';
        }else if(meta.configured){
          detail='OAuth готов · подключи аккаунт';
          action='<button type="button" class="social-center-r502-connect" data-tiktok-connect-r502>Подключить TikTok</button>';
        }else detail='нужны TIKTOK_CLIENT_KEY и SECRET';
      }else if(key==='site') href='https://andrikmetal.com/';
      const inner=`<div class="social-center-r488-platform-top"><span class="social-center-r488-platform-icon">${esc(def.icon)}</span><span class="social-center-r488-status">${esc(status)}</span></div><h3>${esc(def.name)}</h3>${sub}<strong>${esc(cardValue===null?'—':num(cardValue))}</strong><small>${esc(detail)}</small>${action}${href?'<em>Открыть ↗</em>':''}`;
      return href?`<a class="social-center-r488-platform is-${key}" href="${esc(href)}" target="_blank" rel="noopener noreferrer">${inner}</a>`:`<article class="social-center-r488-platform is-${key}${live?'':' is-future'}">${inner}</article>`;
    }).join('');
    box.querySelector('[data-tiktok-connect-r502]')?.addEventListener('click',connectTikTok);
  }
  function renderToggles(data){
    const box=$('socialCenterTogglesR488');if(!box)return;
    const keys=['site','youtube','instagram','facebook','tiktok'];
    box.innerHTML=keys.map(key=>{const ready=graphConnected(key,data);const meta=data?.platforms?.[key]||{};const wait=key==='tiktok'&&meta.oauthConnected?' · копим историю':' · ждёт';return `<button type="button" class="social-center-r488-toggle is-${key}${state.active[key]?' is-active':''}" data-social-toggle="${key}" ${ready?'':'disabled'}>${esc(platformDefs[key].name)}${ready?'':wait}</button>`}).join('');
  ...
  function renderChart(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'].filter(key=>state.active[key]&&graphConnected(key,data));
  ...
  function renderShares(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'].filter(key=>connected(key,data)&&state.active[key]);
  ...
  function renderRecord(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'].filter(key=>graphConnected(key,data));
  ...
  function renderTikTok(data){
    const section=$('socialCenterTikTokR502');const grid=$('socialCenterTikTokGridR502');if(!section||!grid)return;
    const tt=data?.platforms?.tiktok||{};if(!tt.oauthConnected||!tt.connected){section.hidden=true;return}section.hidden=false;
    const v=tt.summary||{};
    const metrics=[
    ...
    grid.innerHTML=metrics.map(([label,value])=>`<div class="social-center-r502-tt-metric${present(value)?'':' is-unavailable'}"><span>${esc(label)}</span><strong>${esc(full(value))}</strong></div>`).join('');
    const note=$('socialCenterTikTokNoteR502');if(note){const count=Number(v.fetchedVideos||0);note.textContent=v.allVideosFetched?`Срез по всем ${count} публичным роликам. Дневной прирост строится из серверных снимков.`:`Срез по последним ${count} роликам. Дневной прирост строится из серверных снимков.`}
  }
  function renderWarning(data){
    ...
    const fb=data?.platforms?.facebook||{};const tt=data?.platforms?.tiktok||{};
    if(fb.configured&&!fb.connected&&fb.error)messages.push(`Facebook: ${fb.error}`);
    if(tt.configured&&!tt.oauthConnected)messages.push('TikTok: OAuth настроен. Нажми «Подключить TikTok» в карточке.');
    else if(tt.oauthConnected&&!tt.connected)messages.push(`TikTok: ${tt.error||'OAuth есть, но API пока не вернул данные'}`);
    else if(tt.oauthConnected&&tt.connected&&!tt.trendConnected)messages.push('TikTok подключён. Текущие счётчики уже видны; линия прироста появится после накопления ежедневных серверных снимков.');
    if(Array.isArray(tt.partialErrors)&&tt.partialErrors.length)messages.push(`TikTok: часть данных пока недоступна · ${tt.partialErrors[0]}`);
    box.hidden=!messages.length;box.textContent=messages.join(' · ');
  }
  function render(data){
    state.data=data;const totals=data?.totals||{};const keys=['site','youtube','instagram','facebook','tiktok'].filter(key=>connected(key,data));const grand=keys.reduce((sum,key)=>sum+n(totals[key]),0);
    const total=$('socialCenterTotalR488');if(total){const igLabel=data?.platforms?.instagram?.metricLabel||'просмотры';total.innerHTML=`<span>Общий итог</span><strong>${esc(full(grand))}</strong><small>${keys.length} из 5 · Instagram: ${esc(igLabel)}</small>`;}
    const period=$('socialCenterPeriodR488');if(period)period.textContent=`${day(data?.period?.startDate)} — ${day(data?.period?.endDate)}`;
    renderPlatforms(data);renderToggles(data);renderChart(data);renderShares(data);renderRecord(data);renderInstagram(data);renderTikTok(data);renderWarning(data);const note=document.querySelector('.social-center-r488-chart-note');if(note){const igLabel=data?.platforms?.instagram?.metricLabel||'просмотры';const tt=data?.platforms?.tiktok||{};note.textContent=`Сайт — визиты, YouTube — просмотры, Instagram — ${igLabel}, TikTok — ${tt.trendConnected?'прирост просмотров':'линия появится после накопления снимков'}, Facebook — после Meta OAuth. Цвета: зелёный / красный / жёлтый / синий / розовый.`;}
  }
  ...
  const boot=()=>{
    $('socialCenterRefreshR488')?.addEventListener('click',()=>load(true));
    const params=new URLSearchParams(location.search);const youtubeConnected=params.get('youtube')==='connected';const tiktokState=params.get('tiktok')||'';const oauthEvent=youtubeConnected||Boolean(tiktokState);
    ...
    if(tiktokState){const warning=$('socialCenterWarningR488');const reason=params.get('reason')||params.get('sync')||'';if(warning){warning.hidden=false;warning.textContent=tiktokState==='connected'?'TikTok OAuth подключён. Загружаем первый реальный срез…':`TikTok OAuth: ${reason||tiktokState}`}}
