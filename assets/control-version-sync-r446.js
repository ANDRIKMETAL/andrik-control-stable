/* ANDRIK Control R446 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R446__)return;
  window.__ANDRIK_VERSION_SYNC_R446__=true;
  const RELEASE='R446';
  const RUNTIME='55.00-r446';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R446 R444 BASE + PLANET TO WORLD MAP PORTAL + REVERSE MAP RETURN';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:445,version:'55.00'}}));
    }finally{busy=false;}
  };
