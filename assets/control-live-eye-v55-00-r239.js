(() => {
  ...
  const init = () => {
    ...
    style.textContent = `
      ...
      /* R238: section color lock is based on the actual body page class. */
      ...
      /* R238 hard page locks: higher specificity than ordinary green/check styles. */
      html[data-andrik-section-lock="blue"] body.control-page.protection-page .control-topbar.control-topbar{
        border-bottom-color:rgba(91,204,255,.72)!important;
        background:radial-gradient(ellipse at 50% 52%,rgba(55,184,255,.37) 0%,rgba(24,104,208,.17) 24%,transparent 58%),linear-gradient(90deg,#01070b 0%,#031226 36%,#071f3a 50%,#031226 64%,#01070b 100%)!important;
        box-shadow:0 3px 0 rgba(102,210,255,.12),0 12px 38px rgba(37,145,255,.28)!important
      }
      html[data-andrik-section-lock="yellow"] body.control-page.attack-page .control-topbar.control-topbar{
        border-bottom-color:rgba(255,222,89,.78)!important;
        background:radial-gradient(ellipse at 50% 52%,rgba(255,214,64,.38) 0%,rgba(205,130,12,.18) 24%,transparent 58%),linear-gradient(90deg,#090701 0%,#1d1503 36%,#332406 50%,#1d1503 64%,#090701 100%)!important;
        box-shadow:0 3px 0 rgba(255,224,104,.13),0 12px 38px rgba(255,177,30,.28)!important
      }
      html[data-andrik-section-lock="blue"] body.control-page.protection-page .control-topbar::before{background:radial-gradient(ellipse at center,rgba(99,211,255,.39),rgba(31,126,232,.16) 36%,transparent 70%)!important}
      html[data-andrik-section-lock="yellow"] body.control-page.attack-page .control-topbar::before{background:radial-gradient(ellipse at center,rgba(255,229,105,.40),rgba(226,154,28,.17) 36%,transparent 70%)!important}
      html[data-andrik-section-lock="blue"] body.control-page.protection-page .control-topbar::after{background:linear-gradient(90deg,transparent,rgba(91,204,255,.94) 18%,#9ceaff 50%,rgba(91,204,255,.94) 82%,transparent)!important;box-shadow:0 0 9px rgba(91,204,255,.98),0 0 27px rgba(40,139,255,.68)!important}
      html[data-andrik-section-lock="yellow"] body.control-page.attack-page .control-topbar::after{background:linear-gradient(90deg,transparent,rgba(255,218,80,.96) 18%,#fff2a9 50%,rgba(255,218,80,.96) 82%,transparent)!important;box-shadow:0 0 9px rgba(255,224,101,.99),0 0 27px rgba(255,174,34,.70)!important}
    ...
    const pageGlowState = document.body?.classList.contains('protection-page')
      ? 'blue'
      : (document.body?.classList.contains('attack-page')
          ? 'yellow'
          : (currentPath.includes('protection-admin')
              ? 'blue'
              : (currentPath.includes('attack-map') ? 'yellow' : 'auto')));

    const enforceSectionGlow = () => {
      if (pageGlowState === 'auto') return;
      document.documentElement.dataset.andrikSectionLock = pageGlowState;
      document.documentElement.dataset.andrikHeaderState = pageGlowState;
      logo.dataset.andrikPageGlow = pageGlowState;
    };
    enforceSectionGlow();
    const sectionGlowTimer = pageGlowState === 'auto' ? 0 : window.setInterval(enforceSectionGlow, 300);
    ...
    const frame = (time) => {
      if (visible) {
        ...
        if (pageGlowState !== 'auto') enforceSectionGlow();
    ...
    window.addEventListener('pagehide', () => {
      ...
      if (sectionGlowTimer) window.clearInterval(sectionGlowTimer);
    }, { once: true });
  };
