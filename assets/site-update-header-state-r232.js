(() => {
  'use strict';
  if (window.__ANDRIK_SITE_UPDATE_HEADER_R232__) return;
  window.__ANDRIK_SITE_UPDATE_HEADER_R232__ = true;
  ...
  function derive() {
    ...
    if (has(release,'running')) return 'yellow';
    if (items.some(item => has(item,'running')) || has(deploy,'warn')) return 'blue';
