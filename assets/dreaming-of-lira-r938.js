...
function questText(){if(eyes<5)return `Собери осколки глаза · ${eyes}/5`;if(!guitar)return 'Найди гитару Лиры';if(eyes<7)return `Собери последние знаки · ${eyes}/7`;return 'Иди к порталу за иллюзией'}
...
$('dolStartBtn')?.addEventListener('click',startGame);$('dolRestart')?.addEventListener('click',startGame);$('dolPause')?.addEventListener('click',togglePause);$('dolSound')?.addEventListener('click',()=>{sound=!sound;$('dolSound').classList.toggle('dol-sound-off',!sound);$('dolSound').textContent=sound?'♪':'×♪';if(sound)beep(440,.06)});
