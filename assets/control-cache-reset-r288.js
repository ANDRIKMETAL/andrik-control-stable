/* Control ANDRIK R288 — one-time cache generation switch.
   Uses a dedicated cache key and never fights the visible release number. */
(() => {
  'use strict';
  const VERSION = '55.00-r288';
  const KEY = 'andrik-control-cache-generation';
  const ONCE = 'andrik-control-cache-reload-r288';
  ...
  (async () => {
    try {
      ...
      console.warn('Control cache generation R288:', error);
    }
  })();
})();
