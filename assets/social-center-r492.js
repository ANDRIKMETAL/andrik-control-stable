/* ANDRIK R492 — Social Center controller: accurate Instagram metrics + YouTube OAuth recovery. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R492__)return;
  window.__ANDRIK_SOCIAL_CENTER_R492__=true;
  const RELEASE='R492';
  ...
  async function youtubeReconnectR492(button){
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      if(key==='instagram'){
        ...
        action=(meta.canReconnect!==false?'<button class="social-center-r492-reconnect" type="button" data-youtube-reconnect-r492>Переподключить YouTube</button>':'')+
          '<a class="social-center-r492-open" href="https://www.youtube.com/@andrikmetal" target="_blank" rel="noopener noreferrer">Открыть канал ↗</a>';
    ...
    box.querySelectorAll('[data-youtube-reconnect-r492]').forEach(button=>button.addEventListener('click',()=>youtubeReconnectR492(button)));
  }
