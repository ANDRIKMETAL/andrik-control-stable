...
function buildSiteUrl({ frame = false } = {}){
  const url = new URL(activeReturnPath || PLAYER_HOME_PATH, location.origin);
  ['player-shell','returnScroll','_updated','v'].forEach(key => url.searchParams.delete(key));
  if (frame) {
    url.searchParams.set('player-shell','1');
    url.searchParams.set('v','52.03');
    if (activeReturnScroll !== null) url.searchParams.set('returnScroll', String(activeReturnScroll));
  }
  url.hash = activeReturnHash || '';
  return frame ? url.href : `${url.pathname}${url.search}${url.hash}`;
}
...
if ('serviceWorker' in navigator) navigator.serviceWorker.register('/service-worker.js?v=54.36', { updateViaCache: 'none' }).catch(() => {});
window.setTimeout(() => {
