/* Control ANDRIK R285 — selected-country portal without black filler. */
(() => {
  'use strict';
  if (window.__ANDRIK_COUNTRY_PORTAL_R285__) return;
  window.__ANDRIK_COUNTRY_PORTAL_R285__ = true;
  ...
  function removeLegacyOverlay() {
    ['r283CountryBottomFill', 'r284CountryBottomFill', 'r285CountryBottomFill'].forEach(id => {
  ...
  function remember() {
    ...
    marker = document.createComment('andrik-r285-map-actions-home');
  ...
  function lockSelectedCountry() {
    if (!pane) return;
    document.body.classList.add('r285-country-scroll-locked');
    if (!wasActive) {
      pane.scrollTop = 0;
      pane.scrollLeft = 0;
  ...
  function unlockSelectedCountry() {
    document.body.classList.remove('r285-country-scroll-locked');
  }

  function restore() {
    if (!actions || !originalParent) return;
    actions.classList.remove('r285-country-actions-portal');
    document.body.classList.remove('r285-country-actions-active');
  ...
  function apply() {
    ...
    actions.classList.add('r285-country-actions-portal');
    document.body.classList.add('r285-country-actions-active');
    wasActive = true;
  }
