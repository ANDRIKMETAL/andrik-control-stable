(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r246';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r246';
  ...
  async function fetchBackupZip(){
    ...
    try{
      ...
      let response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r246`,{credentials:'include',cache:'no-store',headers,signal:controller.signal});
      if(response.status===401&&raw){
        await window.AndrikOwnerSession?.establish?.(raw).catch(()=>null);
        response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r246-retry`,{credentials:'include',cache:'no-store',headers:{accept:'application/zip',authorization:`Bearer ${raw}`},signal:controller.signal});
      }
      if(!response.ok){
  ...
  window.AndrikSiteUpdateR246={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR244=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR243=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR246;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR246;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
