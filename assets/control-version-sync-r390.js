/* ANDRIK Control R390 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R390__)return;
  window.__ANDRIK_VERSION_SYNC_R390__=true;
  const RELEASE='R390';
  const RUNTIME='55.00-r390';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R390 MORNING SUMMARY SNAPSHOT + VERSION LOCK';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:390,version:'55.00'}}));
    }finally{busy=false;}
  };
