/* ANDRIK R506 — instant cached Social Center + live Facebook Page Insights.
...
(()=>{
  ...
  const state={active:{site:true,youtube:true,instagram:true,facebook:true,tiktok:true},data:null,loading:false,background:false,tiktokMode:readTikTokMode()};
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      if(key==='site'){
        ...
        href=live?(meta.profileUrl||''):'';
        if(live){
          if(!present(cardValue)) cardValue=present(summary.interactions)?summary.interactions:(present(summary.followers)?summary.followers:null);
          detail=meta.trendConnected?`28 дней · ${meta.metricLabel||'взаимодействия'}`:'Page API · живой срез';
        }else detail=meta.configured?'токен есть · обновляем':'ждём Meta token';
      }else if(key==='tiktok'){
        const summary=meta.summary||{};
        sub=`<span class="social-center-r488-account">${esc(meta.handle||'@andrikmetal')}</span>`;
        if(live){
  ...
  function renderToggles(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'];
    box.innerHTML=keys.map(key=>{const ready=graphConnected(key,data);const meta=data?.platforms?.[key]||{};let wait=' · ждёт';if(key==='tiktok'&&meta.oauthConnected)wait=' · копим историю';if(key==='facebook'&&meta.connected)wait=' · копим ряд';return `<button type="button" class="social-center-r488-toggle is-${key}${state.active[key]?' is-active':''}" data-social-toggle="${key}" ${ready?'':'disabled'}>${esc(platformDefs[key].name)}${ready?'':wait}</button>`}).join('');
  ...
  function renderShares(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'].filter(key=>state.active[key]&&(connected(key,data)||(key==='tiktok'&&accountConnected(key,data))));
  ...
  function renderFacebook(data){
    const section=$('socialCenterFacebookR506');const grid=$('socialCenterFacebookGridR506');if(!section||!grid)return;
    const fb=data?.platforms?.facebook||{};const next=$('socialCenterNextR506');
    if(!fb.connected){section.hidden=true;if(next)next.hidden=false;return}section.hidden=false;if(next)next.hidden=true;
    const v=fb.summary||{};const a=fb.summaryAvailability||{};
    const metrics=[
      ['Подписчики',a.followers?v.followers:null],['Нравится странице',a.fans?v.fans:null],
      ['Охват',a.reach?v.reach:null],['Просмотры страницы',a.views?v.views:null],['Взаимодействия',a.interactions?v.interactions:null],
      ['Реакции',a.reactions?v.reactions:null],['Комментарии',a.comments?v.comments:null],['Репосты',a.shares?v.shares:null],['Публикации',a.posts?v.posts:null]
    ].filter(([,value])=>present(value));
    grid.innerHTML=metrics.length?metrics.map(([label,value])=>`<div class="social-center-r506-fb-metric"><span>${esc(label)}</span><strong>${esc(full(value))}</strong></div>`).join(''):'<div class="social-center-r488-empty">Facebook подключён. Meta пока не вернула дополнительные счётчики.</div>';
    const note=$('socialCenterFacebookNoteR506');if(note){const parts=[];if(fb.metricLabel)parts.push(`График: ${fb.metricLabel}`);if(fb.updatedAt)parts.push(`обновлено ${new Date(fb.updatedAt).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}`);note.textContent=parts.join(' · ')||'Facebook Page Insights подключён.'}
  }
  ...
  function render(data){
    ...
    renderPlatforms(data);renderToggles(data);renderChart(data);renderShares(data);renderRecord(data);renderInstagram(data);renderFacebook(data);renderTikTok(data);renderWarning(data);saveCachedData(data);const note=document.querySelector('.social-center-r488-chart-note');if(note){const igLabel=data?.platforms?.instagram?.metricLabel||'просмотры';const tt=data?.platforms?.tiktok||{};const fb=data?.platforms?.facebook||{};note.textContent=`Сайт — визиты, YouTube — просмотры, Instagram — ${igLabel}, Facebook — ${fb.trendConnected?(fb.metricLabel||'взаимодействия'):'линия появится после первого Page Insights'}, TikTok — ${tt.trendConnected?'прирост просмотров':'линия появится после накопления снимков'}. Цвета: зелёный / красный / жёлтый / синий / розовый.`;}
  }
  ...
  function staleSources(data){
    ...
    push('facebook',Boolean(p.facebook?.configured),p.facebook?.updatedAt,15);
  ...
  async function manualRefresh(){
    ...
    try{
      ...
      if(p.facebook?.configured)sources.push('facebook');
