(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_DETAIL_R524__)return;window.__ANDRIK_SOCIAL_DETAIL_R524__=true;
  ...
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',CACHE='andrik-social-center-data-r524';
  let lastGood=null;
  ...
  function renderInstagram(ig){
    ...
    const d=ig.demographics||{},box=$('instagramAudienceR513');if(box){const html=[audienceCard('Возраст',d.age,'age'),audienceCard('Пол',d.gender,'gender'),audienceCard('Страны',d.country,'country'),audienceCard('Города',d.city,'city')].filter(Boolean).join('');const section=box.closest('.sd-card');if(!html){if(section)section.hidden=true;return}if(section)section.hidden=false;box.innerHTML=html;const note=$('instagramAudienceNoteR513');if(note){const source={reached_audience_demographics:'охваченная аудитория',engaged_audience_demographics:'вовлечённая аудитория',follower_demographics:'подписчики'}[d.metric]||'аудитория';note.textContent=`Источник: ${source} · последние 30 дней.`}}
  }
  function renderTikTok(tt){
    ...
    chartState.rows=Array.isArray(tt.dailyTrend)?tt.dailyTrend:[];
    chartState.options=[
      {field:'views',label:'Просмотры'},{field:'followers',label:'Подписчики'},{field:'likes',label:'Лайки'},
      {field:'comments',label:'Комменты'},{field:'shares',label:'Репосты'},{field:'videos',label:'Видео'}
    ];
    chartState.field='views';renderChart();
