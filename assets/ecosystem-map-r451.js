/* ANDRIK R451 — ecosystem map + complete persistent historical country anchors on the ALL map. */
(() => {
  'use strict';
  if (window.__ANDRIK_ECOSYSTEM_MAP_R451__) return;
  window.__ANDRIK_ECOSYSTEM_MAP_R451__ = true;
  ...
  const historyCountryKeyR451 = row => String(row?.country || row?.name || row?.code || '').trim().toLowerCase();
  const readHistoryR451 = () => {
    try {
      const raw = JSON.parse(localStorage.getItem(HISTORY_STORAGE_KEY_R451) || localStorage.getItem(HISTORY_STORAGE_KEY_R450) || '[]');
  ...
  const rememberHistoryR451 = rows => {
    const merged = new Map();
    for (const row of state.historyRows || []) {
      const key = historyCountryKeyR451(row);
    ...
    try {
      localStorage.setItem(HISTORY_STORAGE_KEY_R451, JSON.stringify(state.historyRows.map(row=>({country:row.country,code:row.code||'',lastSeen:row.lastSeen||''}))));
    } catch (_) {}
  };
  const allMarkerRowsR451 = () => {
    const merged = new Map();
    for (const row of state.historyRows || []) {
      const key = historyCountryKeyR451(row);
      if (key) merged.set(key,{...row,value:1,historyOnly:true});
  ...
  const applyLayer = (force=false) => {
    ...
    if (!rows.length && active !== 'youtube') {
      runtime.render([],{
        ...
        markerRows:active==='all'?allMarkerRowsR451():undefined,
        error:state.loaded?'Данных для этого слоя пока нет. Они начнут появляться после новых событий.':'Загружаем слой экосистемы…',
        ...meta
      });
    } else {
      runtime.render(rows,{source:`ecosystem-${active}`,layer:active,points:layer.points||[],livePoints:livePointsForLayer(active),markerRows:active==='all'?allMarkerRowsR451():undefined,...meta});
    }
    window.__andrikEcosystemActiveLayer = active;
    updateButtons();
  ...
  const ingestAudience = data => {
    ...
    rememberHistoryR451(state.youtube.rows);
    rebuildAll();
  ...
  const ingestGoogle = ga => {
    ...
    rememberHistoryR451(state.site.rows);
    rebuildAll();
  ...
  async function loadEcosystem(force=false){
    ...
    try {
      ...
      rememberHistoryR451([...serverHistory,...state.site.rows,...state.music.rows,...state.push.rows,...state.youtube.rows]);
      rebuildAll();
  ...
  state.historyRows = readHistoryR451();
  readCaches();
  rememberHistoryR451([...state.youtube.rows,...state.site.rows]);
  rebuildAll();
  bindButtons();
  rememberLayerR382(active);
  applyLayer();
