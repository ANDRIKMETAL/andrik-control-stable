(()=>{
  ...
  function hasR665Agent(data=lastRemote){return /R(?:665|687|688|689|69\d|70\d|710)/i.test(agentVersion(data))}
  ...
  async function startSequence({recover=false}={}){
    ...
    try{
      setMsg(recover?'Восстанавливаю эфир R711…':'Запускаю эфир R711…','work');
      ...
      if(modern){
        setMsg('R711: перезапускаю OVH encoder чисто…','work');
      ...
      setMsg(reconnect?'YouTube OAuth требует переподключения. Нажми кнопку ниже.':`Ошибка запуска R711: ${e.message||e}`,'bad');
  ...
  async function fullFitSequence(){
    if(busy)return;busy=true;render(lastRemote||{});
    try{
      setMsg('Включаю FULL FRAME FIT / NO CROP на OVH…','work');
      const d=await agentAction('full-fit');
      const out=String(d?.result?.output||'FULL FRAME FIT / NO CROP ✅');
      setResult(out);
      setMsg('Весь кадр включён ✅ · NO CROP','ok');
    }catch(e){setMsg(`FULL FIT: ${e.message||e}`,'bad');setResult(`FULL FIT ERROR\n${e.message||e}`)}finally{busy=false;await refresh()}
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(b){e.preventDefault();const a=b.dataset.radioAction;if(a==='start')startSequence();else if(a==='recover')startSequence({recover:true});else if(a==='stop')stopSequence();else if(a==='status')statusSequence();else if(a==='auto-safe')autoSequence();else if(a==='restart')restartOnly();else if(a==='full-fit')fullFitSequence();return}
