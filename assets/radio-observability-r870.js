(()=>{
  ...
  function ensureUi(){
    ...
    if(!document.getElementById('r813Diagnostics')){
      ...
      card.innerHTML=`
        <div class="r813-diag-head"><h2>🧾 Журнал эфира</h2><span class="r813-diag-badge" id="r813DiagBadge">R870 · ждём данные</span></div>
        ...
        <div class="r813-diag-actions"><button type="button" class="r813-diag-copy" id="r813CopyLog">📋 СКОПИРОВАТЬ ВЕСЬ ЛОГ</button><button type="button" class="r813-diag-refresh" id="r813RefreshLog">↻ Обновить</button><button type="button" class="r813-diag-clear" id="r813ClearLog">Очистить вид</button></div>
        <p class="small" style="margin:9px 2px 0">На экране — события за 2 часа. «Скопировать весь лог» сохраняет всю доступную историю (до 30 событий). Если аварий нет, журнал может не получать новых записей — текущий LIVE-статус всё равно показывается.</p>`;
  ...
  function buildText(data){
    ...
    const hdr=['ANDRIK RADIO R870 DIAGNOSTIC COPY',`captured: ${new Date().toISOString()}`,`agent: ${txt(data?.agent?.version)||'—'} · radio: ${txt(s.version)||'—'}`,
  ...
  function renderDiagnostics(data){
    ...
    if(badge){badge.textContent=`R870 · ${fresh.length} свежих / ${allEvents.length} в истории`;const laneWarn=exp>1&&rt>0&&rt<exp;badge.className='r813-diag-badge '+(healthy?(laneWarn?'r813-warn':'r813-ok'):'r813-bad')}
  ...
  async function refresh(manual=false){
    ensureUi();const badge=document.getElementById('r813DiagBadge');if(manual&&badge)badge.textContent='R870 · обновляю…';
    try{const d=await api(STATUS_API);const s=d?.agent?.status||{};renderProfile(s);renderDiagnostics(d);tunePanelR870();return d}
    catch(error){if(badge){badge.textContent='R870 · нет данных';badge.className='r813-diag-badge r813-bad'}const log=document.getElementById('r813DiagLog');if(log)log.textContent=`Диагностика недоступна: ${error?.message||error}`;return null}
  }
