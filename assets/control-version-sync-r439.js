/* ANDRIK Control R439 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R439__)return;
  window.__ANDRIK_VERSION_SYNC_R439__=true;
  const RELEASE='R439';
  const RUNTIME='55.00-r439';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R439 R438 STABLE BASE + ECOSYSTEM MAP RECOVERY + CITY SOURCE EXACT TOTALS + R416 CPU SAFE';
