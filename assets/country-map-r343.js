/* ANDRIK R343 — second tap opens the standalone country silhouette. */
(() => {
  'use strict';
  if (window.__ANDRIK_COUNTRY_MAP_R343__) return;
  window.__ANDRIK_COUNTRY_MAP_R343__ = true;
  ...
  function ensureOverlay() {
    ...
    let overlay = stage.querySelector('.country-map-r343');
    if (!overlay) {
      overlay = document.createElement('div');
      overlay.className = 'country-map-r343';
    ...
    if (overlay.dataset.signature !== signature) {
      ...
      if (shape?.path) {
        ...
        let dots = points.map(point => {
          ...
          return `<circle class="country-point-r343" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}"/>`;
        }).join('');
        if (!dots) dots = '<circle class="country-point-r343 is-empty" cx="500" cy="300" r="12"/>';
        svg = `<svg viewBox="0 0 1000 600" role="img" aria-label="Карта страны ${esc(country)}"><path class="country-shape-r343" fill-rule="evenodd" d="${shape.path}"/>${dots}</svg>`;
      } else {
        svg = `<svg viewBox="0 0 1000 600" role="img" aria-label="Карта страны ${esc(country)}"><rect x="90" y="80" width="820" height="440" rx="80" fill="rgba(20,58,78,.55)" stroke="#92dbff" stroke-width="4" stroke-dasharray="12 12"/><circle class="country-point-r343 is-empty" cx="500" cy="300" r="12"/></svg>`;
      }
      overlay.innerHTML = `${svg}<button class="country-map-label-r343" type="button" aria-label="Вернуться к мировой карте">🌍 Мир · ${flag(code)} ${esc(country)}</button>${points.length ? '' : '<span class="country-map-empty-label-r343">пока без городских точек</span>'}`;
      overlay.querySelector('.country-map-label-r343')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();clearToWorld()},{once:true});
