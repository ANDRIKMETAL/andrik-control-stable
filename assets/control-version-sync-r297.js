/* Control ANDRIK R297 — safe display-version synchronization.
   Never writes text into <html> or <body>. */
(() => {
  'use strict';
  if (window.__ANDRIK_VERSION_SYNC_R297__) return;
  window.__ANDRIK_VERSION_SYNC_R297__ = true;

  const RELEASE = 'R297';
  const RUNTIME = '55.00-r297';
  const FULL = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R297';
  let applying = false;
  ...
  const apply = () => {
    ...
    try {
      ...
      window.ANDRIK_CONTROL_RELEASE = Object.freeze({
        ...
        full: 'v55.00 LIVE WEB AI FINAL R297',
        build: 'R297 CLEAN BASE + FIXED COUNTRY ACTION BUTTONS',
        date: '05.08.2026'
      });
      window.ANDRIK_CONTROL_VERSION = '55.00 LIVE WEB AI FINAL R297';
      window.ANDRIK_CONTROL_BUILD = 'R297 CLEAN BASE + FIXED COUNTRY ACTION BUTTONS';
