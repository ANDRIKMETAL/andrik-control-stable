/* ANDRIK R498 — Social Center: five-platform foundation + unified colors.
   Site/YouTube/Instagram stay live; Facebook/TikTok become API-ready without fake data. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R498__)return;
  window.__ANDRIK_SOCIAL_CENTER_R498__=true;
  const RELEASE='R498';
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      const def=platformDefs[key];const live=connected(key,data);
      ...
      let sub='';let href=meta.profileUrl||'';let action='';
      if(key==='instagram'){
        ...
        sub=meta.handle?`<span class="social-center-r488-account">${esc(meta.handle)}</span>`:'';
        detail=live?(meta.metricLabel||'TikTok Analytics'):(meta.configured?'TikTok OAuth готов · ждём данные':'готово к TikTok OAuth');
      }else if(key==='site') href='https://andrikmetal.com/';
      const inner=`<div class="social-center-r488-platform-top"><span class="social-center-r488-platform-icon">${esc(def.icon)}</span><span class="social-center-r488-status">${esc(status)}</span></div><h3>${esc(def.name)}</h3>${sub}<strong>${esc(total===null?'—':num(total))}</strong><small>${esc(detail)}</small>${action}${href?'<em>Открыть ↗</em>':''}`;
  ...
  function renderToggles(data){
    ...
    const keys=['site','youtube','instagram','facebook','tiktok'];
    box.innerHTML=keys.map(key=>`<button type="button" class="social-center-r488-toggle is-${key}${state.active[key]?' is-active':''}" data-social-toggle="${key}" ${graphConnected(key,data)?'':'disabled'}>${esc(platformDefs[key].name)}${graphConnected(key,data)?'':' · ждёт'}</button>`).join('');
  ...
  function renderWarning(data){
    ...
    if(tt.configured&&!tt.connected&&tt.error)messages.push(`TikTok: ${tt.error}`);
  ...
  function render(data){
    ...
    renderPlatforms(data);renderToggles(data);renderChart(data);renderShares(data);renderRecord(data);renderInstagram(data);renderWarning(data);const note=document.querySelector('.social-center-r488-chart-note');if(note){const igLabel=data?.platforms?.instagram?.metricLabel||'просмотры';note.textContent=`Сайт — визиты, YouTube — просмотры, Instagram — ${igLabel}, Facebook и TikTok включатся автоматически после OAuth. Цвета: зелёный / красный / жёлтый / синий / розовый.`;}
  }
  ...
  const boot=()=>{
    ...
    const params=new URLSearchParams(location.search);const justConnected=params.get('youtube')==='connected';
    if(justConnected){
      const warning=$('socialCenterWarningR488');if(warning){warning.hidden=false;warning.textContent='YouTube OAuth обновлён. Получаем свежую аналитику…'}
