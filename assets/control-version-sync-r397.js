/* ANDRIK Control R397 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R397__)return;
  window.__ANDRIK_VERSION_SYNC_R397__=true;
  const RELEASE='R397';
  const RUNTIME='55.00-r397';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R397 EARLY AUTO SUMMARY CHECKPOINT + UPDATE CLOCKS + VERSION LOCK';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:397,version:'55.00'}}));
    }finally{busy=false;}
  };
