/* ANDRIK Control R441 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R441__)return;
  window.__ANDRIK_VERSION_SYNC_R441__=true;
  const RELEASE='R441';
  const RUNTIME='55.00-r441';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R441 R440 BASE + ADMIN FOURTH CARD RESTORE + ARCHIVE DIRECT DAY LOAD + CALENDAR LIFT';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:441,version:'55.00'}}));
    }finally{busy=false;}
  };
