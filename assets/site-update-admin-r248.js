(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r248';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r248';
  ...
  async function fetchBackupZip(){
    ...
    try{
      ...
      let response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r248`,{credentials:'include',cache:'no-store',headers,signal:controller.signal});
      if(response.status===401&&raw){
        await window.AndrikOwnerSession?.establish?.(raw).catch(()=>null);
        response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r248-retry`,{credentials:'include',cache:'no-store',headers:{accept:'application/zip',authorization:`Bearer ${raw}`},signal:controller.signal});
      }
      if(!response.ok){
  ...
  async function shareZipBackup(){
    ...
    const safeName=`ANDRIK-BACKUP-R248-${new Date().toISOString().slice(0,10)}.zip`;
    const freshFile=new File([siteBackupZipFile],safeName,{type:'application/zip',lastModified:Date.now()});
    let lastError=null;
    try{
      if(navigator.share){
        const payload={files:[freshFile]};
        const supported=!navigator.canShare||navigator.canShare(payload);
        if(supported){
          try{
            await navigator.share(payload);
            setText('siteUpdateBackupZipState','ZIP-бэкап открыт в системном меню «Поделиться».');
            return;
          }catch(error){
      ...
      const originalFile=siteBackupZipFile;
      siteBackupZipFile=freshFile;
      try{downloadZipBackup()}finally{siteBackupZipFile=originalFile}
      setText('siteUpdateBackupZipState',lastError?`Android не передал ZIP напрямую (${lastError.message||lastError.name}). Архив сохранён в «Загрузки» — откройте его там и нажмите «Поделиться».`:'Телефон не поддерживает прямую передачу ZIP. Архив сохранён в «Загрузки» — откройте его там и нажмите «Поделиться».');
    }catch(error){
      if(error?.name!=='AbortError'){
        downloadZipBackup();
        setText('siteUpdateBackupZipState','Архив сохранён в «Загрузки». Откройте файл ANDRIK-BACKUP и выберите «Поделиться».');
      }
    }
  }
  ...
  window.AndrikSiteUpdateR248={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR247=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR244=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR243=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR248;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR248;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
