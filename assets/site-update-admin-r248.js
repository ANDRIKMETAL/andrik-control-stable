(() => {
  ...
  async function publishCommitWithRetry(buildForm,maxAttempts=1){
    ...
    for(let attempt=1;attempt<=maxAttempts;attempt++){
      try{
        return await api('/api/control/site-update/publish',{method:'POST',body:buildForm(),timeoutMs:180000});
      }catch(error){
        lastError=error;
        const retryable=error?.status===503||error?.data?.retryable===true||[429,500,502,504].includes(Number(error?.status||0));
    ...
    throw lastError||new Error('GitHub не ответил');
  }
