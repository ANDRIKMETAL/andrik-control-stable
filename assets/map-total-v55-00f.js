(() => {
  ...
  function updateTotal() {
    ...
    const formatted = format.format(total);
    valueNode.textContent = formatted;
    shell.classList.remove('is-loading');
    shell.classList.add('is-ready');
    shell.title = `Сумма просмотров по всем ${rows.length} странам: ${formatted}`;
    shell.setAttribute('aria-label', `Всего просмотров по всем странам: ${formatted}`);
  ...
  window.addEventListener('andrik:audience-data', scheduleUpdate);
