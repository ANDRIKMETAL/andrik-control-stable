// R925 AIR RESTORE CONTROL · FULLSCREEN SWITCH REMOVED
(()=>{
  ...
  function render(data){
    ...
    document.querySelectorAll('[data-radio-action]').forEach(b=>{
      ...
      b.disabled=busy||active||!online||(needsR867&&!hasR867Agent(data));
      b.title=!online?'OVH Agent offline — сначала установи/запусти R867':needsR867&&!hasR867Agent(data)?'Нужен OVH Agent R867':' ';
    });
  ...
  document.addEventListener('click',e=>{
    ...
    if(b){e.preventDefault();const a=b.dataset.radioAction;if(a==='start')startSequence();else if(a==='gold-restore')airRestore();else if(a==='cache-clean')cacheClean();else if(a==='soft-restart')softRestart();else if(a==='stop')stopSequence();else if(a==='status')statusSequence();else if(a==='auto-safe')autoSequence();return}
  ...
  window.AndrikRadioRemoteR925={refresh,start:()=>startSequence(),airRestore,goldRestore:airRestore,cacheClean,softRestart,stop:stopSequence,status:statusSequence,saveTicker};window.AndrikRadioRemoteR870=window.AndrikRadioRemoteR925;window.AndrikRadioRemoteR867=window.AndrikRadioRemoteR925;window.AndrikRadioRemoteR687=window.AndrikRadioRemoteR925;window.AndrikRadioRemoteR665=window.AndrikRadioRemoteR925;
