/* ANDRIK R506 — ecosystem map without TikTok geography. TikTok analytics remain in Social Center only. */
(() => {
  'use strict';
  if (window.__ANDRIK_ECOSYSTEM_MAP_R506__) return;
  window.__ANDRIK_ECOSYSTEM_MAP_R506__ = true;
  ...
  const VALID_LAYERS_R382 = ['all','site','youtube','instagram','facebook','music','push'];
  ...
  const rebuildAll = () => {
    ...
    const add = (name, rows) => {
      for (const row of cleanRows(rows)) {
        ...
        const item = breakdown.get(key) || {youtube:0,site:0,instagram:0,facebook:0,music:0,push:0};
    ...
    state.all.points = aggregatePoints([state.site.points,state.instagram.points,state.facebook.points,state.music.points,state.push.points]);
    state.all.weekly = mergeWeekly([state.youtube.weekly,state.site.weekly,state.instagram.weekly,state.facebook.weekly,state.music.weekly,state.push.weekly]);
    state.all.previous = mergeWeekly([state.youtube.previous,state.site.previous,state.instagram.previous,state.facebook.previous,state.music.previous,state.push.previous]);
  ...
  const decorateCountry = () => {
    ...
    if (active === 'all') {
      ...
      detail.innerHTML = `<b>Экосистема:</b> ▶ ${fmt(b.youtube)} · 🌐 ${fmt(b.site)} · ◎ ${fmt(b.instagram)} · f ${fmt(b.facebook)}<span class="is-muted"> · ♫ ${fmt(b.music)} · 🔔 ${fmt(b.push)}${cityText}</span>`;
    } else {
  ...
  const syncOptionalLayerButtonsR506 = () => {
    const fbButton=map.querySelector('.ecosystem-layer-switcher [data-ecosystem-layer="facebook"]');
    const hasFacebookGeo=cleanRows(state.facebook.rows).length>0 || (Array.isArray(state.facebook.points)&&state.facebook.points.length>0);
    if(fbButton)fbButton.hidden=!hasFacebookGeo;
    if(active==='facebook'&&!hasFacebookGeo){active='all';rememberLayerR382(active);window.__andrikEcosystemActiveLayer=active}
  };
  ...
  async function loadEcosystem(force=false){
    ...
    try {
      ...
      rememberHistoryR452([...serverHistory,...state.site.rows,...state.instagram.rows,...state.facebook.rows,...state.music.rows,...state.push.rows,...state.youtube.rows]);
      rebuildAll();
  ...
  readCaches();
  rememberHistoryR452([...state.youtube.rows,...state.site.rows,...state.instagram.rows,...state.facebook.rows]);
  rebuildAll();
  syncOptionalLayerButtonsR506();
  bindButtons();
  rememberLayerR382(active);
