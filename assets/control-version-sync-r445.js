/* ANDRIK Control R445 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R445__)return;
  window.__ANDRIK_VERSION_SYNC_R445__=true;
  const RELEASE='R445';
  const RUNTIME='55.00-r445';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R445 R444 BASE + PLANET TO WORLD MAP PORTAL + REVERSE MAP RETURN';
