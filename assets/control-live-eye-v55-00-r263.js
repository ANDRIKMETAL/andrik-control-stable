(() => {
  'use strict';
  if(window.__ANDRIK_LIVE_EYE_R263__)return;
  window.__ANDRIK_LIVE_EYE_R263__=true;
  const VERSION='55.00-r263';
  ...
  const readStage=()=>{
    ...
    // Stage 6 (Protection) is always blue while it is active.
    if(protect?.classList.contains('is-running'))return 'blue';
    if(protect?.classList.contains('is-warn'))return 'red';
    ...
    if(protect?.classList.contains('is-done')&&!result?.classList.contains('is-ready'))return 'blue';
