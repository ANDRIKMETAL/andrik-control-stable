...
async function loadTracks(){const d=await api('/api/music/downloads?ts='+Date.now());tracks=(Array.isArray(d?.tracks)?d.tracks:[]).filter(t=>{const key=String(t?.key||'');return !/^covers\//i.test(key)&&!( /^singles\//i.test(key)&&isCoverTrackR1161(t) )});groupTracks()}
