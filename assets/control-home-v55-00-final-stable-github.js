/* Control ANDRIK v55.00 FINAL STABLE GITHUB — standalone summary/activity screens. */
(() => {
  ...
  let sourcePages=[];
  const HOME_CACHE_KEY='andrik-control-home-last-good-v55-00d';
  ...
  function renderSummary(data){
    ...
    $('controlHomeSummary').innerHTML=items.map(item=>summaryCard(...item)).join('');
    renderGeoDelta(s);
    ...
    const compactTime=Number.isFinite(updatedAt)
      ...
      : 'сейчас';
    $('controlHomeUpdated').textContent=`Обновлено ${compactTime}`;
    syncCarouselClones();
  }
  ...
  function renderActivity(events=[]){
    ...
    if(!allActivityEvents.length){
      ...
      if(foot)foot.textContent=summaryMode?'← Сводка за сегодня · «24 часа» — полный список':'← Сводка · «24 часа» — полный список';
  ...
  if(summaryMode){
    ...
    if(cachedForCurrentWindow(cached)){
      ...
      try{localStorage.removeItem(HOME_CACHE_KEY)}catch(_){}
  ...
  const shell=$('controlSwipeShell');
    ...
    try{
      const data=await api(forceLive?'/api/control/home?refresh=1&v=55.00n':'/api/control/home?v=55.00n',{timeoutMs:forceLive?13000:8000});
  ...
  const requestedRaw=(new URLSearchParams(location.search).get('page')||'menu').toLowerCase();
  ...
  async function sendDailySummary(){
    ...
    try{
      ...
      load({silent:true,forceLive:true}).catch(()=>{});
      setTimeout(()=>{
