/* ANDRIK Control R477 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R477__)return;
  window.__ANDRIK_VERSION_SYNC_R477__=true;
  const RELEASE='R477';
  const RUNTIME='55.00-r477';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R477 R476 BASE + FAST NATIVE ALBUM LIBRARY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:476,version:'55.00'}}));
    }finally{busy=false;}
  };
