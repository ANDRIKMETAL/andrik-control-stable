/* ANDRIK Control R422 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R422__)return;
  window.__ANDRIK_VERSION_SYNC_R422__=true;
  const RELEASE='R422';
  const RUNTIME='55.00-r422';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R422 QUICK REPORT NAV FIX + R421 GLOBE SPACING + R420 ADMIN LAUNCH + R419 CITY LIST + R416 CRON STABLE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:422,version:'55.00'}}));
    }finally{busy=false;}
  };
