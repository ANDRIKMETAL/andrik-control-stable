/* ANDRIK Control R473 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R473__)return;
  window.__ANDRIK_VERSION_SYNC_R473__=true;
  const RELEASE='R473';
  const RUNTIME='55.00-r473';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R473 R472 BASE + DIRECT VIDEO COMMENTS + OWNER PUSH REPAIR + 5M RESCUE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:473,version:'55.00'}}));
    }finally{busy=false;}
  };
