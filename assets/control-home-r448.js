/* Control ANDRIK R448 — archive null-guard + stable summary + daily city acquisition sources. */
(() => {
  ...
  function renderCityModalR375(){
    ...
    box.innerHTML=rows.map((item,index)=>{
      ...
      return `<div class="control-city-modal-row-r375"><b>${number(index+1)}</b><span>${countryFlagR370(item?.country||'')}</span><div><strong>${escapeHtml(place)}</strong>${regionLine}<small class="${sourceClass}">${sourceText}</small></div><em>${number(item.opens)}</em></div>`;
    }).join('');
  }
  ...
  function renderSummary(data){
    ...
    const items=[
      ...
      ['▶️',number(yDelta),'Просмотров канала за день',periodNote,'youtube','/youtube-admin.html?refresh=1&v=55.00n']
    ];
