/* ANDRIK R418 — persistent per-country city history + map-home routing. */
(()=>{
  'use strict';
  if(window.__ANDRIK_COUNTRY_CITY_HISTORY_R418__)return;
  window.__ANDRIK_COUNTRY_CITY_HISTORY_R418__=true;
  ...
  let date=localDate(),country='',name='',loading=false,controller=null;
  ...
  const setOpen=value=>{
    ...
    if(value){
      ...
      date=localDate();modal.hidden=false;modal.setAttribute('aria-hidden','false');document.body.classList.add('country-city-history-open-r418');
      requestAnimationFrame(()=>load());
    }else{
      controller?.abort?.();controller=null;modal.hidden=true;modal.setAttribute('aria-hidden','true');document.body.classList.remove('country-city-history-open-r418');
    }
  };
  const render=payload=>{
    const rows=Array.isArray(payload?.rows)?payload.rows:[];
    $('countryCityHistoryTitleR418').textContent=`🏙 ${name||country}`;$('countryCityHistoryDateR418').textContent=pretty(date);
    $('countryCityHistoryNextR418').disabled=date>=localDate();
    $('countryCityHistorySummaryR418').textContent=rows.length?`${fmt(payload.total)} включений · ${fmt(rows.length)} городов / регионов`:'За эту дату городов не зафиксировано';
    $('countryCityHistoryListR418').innerHTML=rows.length?rows.map((r,i)=>{const place=String(r.city||r.region||'Город / регион'),region=String(r.region||'').trim(),sub=region&&region.toLowerCase()!==place.toLowerCase()?`<small>${esc(region)}</small>`:'';return `<div class="country-city-history-row-r418"><b>${i+1}</b><div><strong>${esc(place)}</strong>${sub}</div><em>${fmt(r.opens)}</em></div>`}).join(''):'<div class="admin-empty">Нет активности с доступной географией.</div>';
  };
  const load=async()=>{
    ...
    try{
      ...
      const r=await fetch(`/api/control/country-city-history?country=${encodeURIComponent(country)}&date=${encodeURIComponent(date)}&v=55.00-r418`,{cache:'no-store',credentials:'include',headers,signal:controller.signal});
  ...
  $('countryCityHistoryTodayR418')?.addEventListener('click',()=>{date=localDate();load()});$('countryCityHistoryYesterdayR418')?.addEventListener('click',()=>{date=shift(localDate(),-1);load()});
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('countryCityHistoryModalR418')?.hidden)setOpen(false)});
  // The Admin panel is the permanent home. Replacing the map entry means Back from Activity/Summary returns to Admin, never to the map.
  ...
  // renderWorldMap replaces #worldMap.innerHTML. Re-attach the overlay pill after every map repaint.
  new MutationObserver(()=>{if(openBtn.parentElement!==map)map.appendChild(openBtn);syncButton()}).observe(map,{childList:true});
  syncButton();setTimeout(syncButton,180);setTimeout(syncButton,700);
})();
