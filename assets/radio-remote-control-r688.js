(()=>{
  ...
  async function ensureLiveR688(){
    let last=null,lastData=null;
    for(let i=0;i<36;i++){
      try{
        const d=await api('/api/control/youtube-live-r688/ensure',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});
        lastData=d;
        ...
        if(d.pending){
          ...
          const label=life==='testing'||stage==='live'?'YouTube тест сигнала готов. Перевожу в LIVE…':
            life==='teststarting'||stage==='testing'?'YouTube принимает сигнал. Запускаю проверку…':
            life==='ready'?'Сигнал есть. Отправляю START в YouTube…':
  ...
  async function startSequence({recover=false}={}){
    ...
    try{
      setMsg(recover?'Восстанавливаю эфир R688…':'Запускаю эфир R688…','work');
      ...
      if(modern){
        setMsg('R688: перезапускаю OVH encoder чисто…','work');
        ...
        // R688 no longer depends on the obsolete /usr/local/sbin/andrik-youtube helper
      ...
      await sleep(2500);
      const live=await ensureLiveR688();
      ...
      setResult(`R688 START ERROR\n${String(e.message||e)}${e.data?.reason?`\nReason: ${e.data.reason}`:''}`);
      setMsg(reconnect?'YouTube OAuth требует переподключения. Нажми кнопку ниже.':`Ошибка запуска R688: ${e.message||e}`,'bad');
