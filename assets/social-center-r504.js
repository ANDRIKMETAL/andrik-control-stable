/* ANDRIK R504 — compact Social Center + safe TikTok refresh.
   Production credentials remain supported server-side, but the card stays focused on the active Sandbox until review. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R504__)return;
  window.__ANDRIK_SOCIAL_CENTER_R504__=true;
  const RELEASE='R504';
  ...
  const TIKTOK_MODE_KEY='andrik-tiktok-oauth-mode-r504';
  ...
  async function api(refresh=false){
    ...
    try{
      const params=new URLSearchParams({tiktok_mode:state.tiktokMode});if(refresh)params.set('refresh_source','tiktok');
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      if(key==='site'){
        ...
        detail=live?(meta.metricLabel||'Page Insights'):'ждём Meta OAuth';
      }else if(key==='tiktok'){
