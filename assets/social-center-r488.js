/* ANDRIK R488 — Social Center controller. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R488__)return;
  window.__ANDRIK_SOCIAL_CENTER_R488__=true;
  const RELEASE='R488';
  ...
  const num=value=>Number.isFinite(Number(value))?new Intl.NumberFormat('ru-RU',{notation:Math.abs(Number(value))>=10000?'compact':'standard',maximumFractionDigits:1}).format(Number(value)):'—';
  const full=value=>Number.isFinite(Number(value))?new Intl.NumberFormat('ru-RU').format(Math.round(Number(value))):'—';
  ...
  const platformDefs={
    site:{name:'Сайт',icon:'⌂',label:'просмотров страниц'},
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      const total=!future?data?.totals?.[key]:null;
      const status=future?'готовим':live?'подключено':'ожидает';
      const detail=future?def.label:(live?def.label:(key==='instagram'?'добавь Worker Secret':'данных пока нет'));
      return `<article class="social-center-r488-platform is-${key}${future?' is-future':''}"><div class="social-center-r488-platform-top"><span class="social-center-r488-platform-icon">${esc(def.icon)}</span><span class="social-center-r488-status">${esc(status)}</span></div><h3>${esc(def.name)}</h3><strong>${future?'—':esc(total===null?'—':num(total))}</strong><small>${esc(detail)}</small></article>`;
    }).join('');
  }
  function renderToggles(data){
    ...
    box.innerHTML=liveKeys.map(key=>`<button type="button" class="social-center-r488-toggle is-${key}${state.active[key]?' is-active':''}" data-social-toggle="${key}" ${connected(key,data)?'':'disabled'}>${esc(platformDefs[key].name)}</button>`).join('')+
    ...
    box.querySelectorAll('[data-social-toggle]').forEach(button=>button.addEventListener('click',()=>{
      ...
      if(!Object.entries(state.active).some(([k,v])=>v&&connected(k,data)))state.active[key]=true;
  ...
  function renderChart(data){
    ...
    const keys=['site','youtube','instagram'].filter(key=>state.active[key]&&connected(key,data));
  ...
  function renderRecord(data){
    ...
    const keys=['site','youtube','instagram'].filter(key=>connected(key,data));
  ...
  function renderInstagram(data){
    ...
    const s=ig.summary||{};const metrics=[['Охват',s.reach],['Профиль',s.profileViews],['Взаимодействия',s.totalInteractions],['Лайки',s.likes],['Комментарии',s.comments],['Репосты',s.shares],['Сохранения',s.saves],['Вовлечённые',s.accountsEngaged]];
    grid.innerHTML=metrics.map(([label,value])=>`<div class="social-center-r488-ig-metric"><span>${esc(label)}</span><strong>${esc(full(value))}</strong></div>`).join('');
  }
  function renderWarning(data){
    const box=$('socialCenterWarningR488');if(!box)return;const ig=data?.platforms?.instagram||{};
    if(!ig.configured){box.hidden=false;box.textContent='Instagram добавлен в центр, но Worker ждёт Secret INSTAGRAM_ACCESS_TOKEN.'}
    else if(!ig.connected){box.hidden=false;box.textContent=`Instagram пока не отдал данные: ${ig.error||'проверь токен и обнови центр'}`}
    else if(Array.isArray(ig.partialErrors)&&ig.partialErrors.length){box.hidden=false;box.textContent='Instagram подключён. Часть дополнительных метрик пока недоступна, основной график продолжает работать.'}
  ...
  function render(data){
    ...
    const total=$('socialCenterTotalR488');if(total)total.innerHTML=`<span>Все просмотры</span><strong>${esc(full(grand))}</strong><small>${keys.length} из 6 платформ подключено</small>`;
