/* ANDRIK Control R434 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R434__)return;
  window.__ANDRIK_VERSION_SYNC_R434__=true;
  const RELEASE='R434';
  const RUNTIME='55.00-r434';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R434 R433 RELEASE EVERY 5M + FIRST LIKE ZERO BASELINE + NEWEST VIDEO 2M PIN + R416 CPU SAFE';
