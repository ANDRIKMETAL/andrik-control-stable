(()=>{
  'use strict';
  if(window.__ANDRIK_RADIO_COUNTRIES_R870__)return;
  window.__ANDRIK_RADIO_COUNTRIES_R870__=true;
  ...
  function renderMap(cityData,countryData){
    ...
    for(const p of points){
      ...
      dot.title=`${p.isLive?'LIVE':'24 ч'}: ${p.city}${p.country?' · '+countryName(p.country):''}: ${p.value}`;dot.setAttribute('aria-label',dot.title);layer.appendChild(dot);
    }
    ...
    cityList.innerHTML=points.length?points.slice(0,20).map((p,i)=>`<div class="radio-city-row"><b>${i+1}</b><div><strong>${esc(p.city)}</strong><small>${esc([p.sourceLabel,p.region,p.country?countryName(p.country):''].filter(Boolean).join(' · ')||cityData.title)}</small></div><em>${fmt(p.value)}</em></div>`).join(''):'<div class="radio-list-empty">Городов за последние 24 часа пока нет.</div>';
  ...
  window.AndrikRadioCountriesR870={refresh,refreshMs:MAP_REFRESH_MS};window.AndrikRadioCountriesR662=window.AndrikRadioCountriesR870;
})();
