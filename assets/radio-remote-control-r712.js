(()=>{
  ...
  async function startSequence({recover=false}={}){
    ...
    try{
      setMsg(recover?'Восстанавливаю эфир R712…':'Запускаю эфир R712…','work');
      ...
      if(modern){
        setMsg('R712: перезапускаю OVH encoder чисто…','work');
      ...
      setMsg(reconnect?'YouTube OAuth требует переподключения. Нажми кнопку ниже.':`Ошибка запуска R712: ${e.message||e}`,'bad');
