/* Control ANDRIK R302 — reliable daily summary, push status and exact completed-period view. */
(() => {
  ...
  const HOME_CACHE_KEY_PUSH_BASE='andrik-control-home-push-r302';
  ...
  const shell=$('controlSwipeShell');
    ...
    try{
      const query=new URLSearchParams({v:'55.00-r302'});
  ...
  $('controlSummaryNext')?.addEventListener('click',()=>{
    ...
    location.assign(`/control-home.html?page=activity${pushQuery}&v=55.00-r302&t=${Date.now()}`);
  });
