/* ANDRIK Control R403 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R403__)return;
  window.__ANDRIK_VERSION_SYNC_R403__=true;
  const RELEASE='R403';
  const RUNTIME='55.00-r403';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R403 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:403,version:'55.00'}}));
    }finally{busy=false;}
  };
