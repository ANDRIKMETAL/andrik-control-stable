(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_PLATFORM_SWIPE_R524__)return;
  window.__ANDRIK_SOCIAL_PLATFORM_SWIPE_R524__=true;
  ...
  document.documentElement.classList.add('social-platform-swipe-r524-active');
  document.body?.setAttribute('data-social-platform-index',String(index));

  const toast=document.createElement('div');
  toast.className='social-platform-swipe-r524-toast is-hidden';
  ...
  const setDrag=dx=>{
    ...
    document.documentElement.style.setProperty('--social-swipe-r524-x',`${px}px`);
  };
  const clearDrag=()=>document.documentElement.style.removeProperty('--social-swipe-r524-x');
  const go=delta=>{
    if(navigating)return;
    navigating=true;
    const target=(index+delta+routes.length)%routes.length;
    showToast(`${target+1}/5 · ${routes[target].name}`);
    document.documentElement.classList.add('social-platform-swipe-r524-leaving');
