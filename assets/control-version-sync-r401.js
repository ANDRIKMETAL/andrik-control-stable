/* ANDRIK Control R401 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R401__)return;
  window.__ANDRIK_VERSION_SYNC_R401__=true;
  const RELEASE='R401';
  const RUNTIME='55.00-r401';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R401 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:401,version:'55.00'}}));
    }finally{busy=false;}
  };
