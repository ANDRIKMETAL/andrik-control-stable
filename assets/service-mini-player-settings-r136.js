(() => {
  ...
  const broadcast=visible=>{
    const detail={visible,source:'service-settings-r136'};
    ...
    try{
      ...
      channel.postMessage({type:'mini-player-visibility',visible,source:'service-settings-r136'});
      setTimeout(()=>channel.close(),80);
    }catch(_){}
    try{window.top?.postMessage?.({type:'andrik-mini-player-visibility',visible,source:'service-settings-r136'},location.origin)}catch(_){}
  };
