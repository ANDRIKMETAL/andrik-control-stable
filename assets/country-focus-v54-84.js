(() => {
  ...
  if (!list || !map || window.__andrikCountryFocusUiV5484Ready) return;
  window.__andrikCountryFocusUiV5484Ready = true;

  const card = map.closest('.world-map-card');
  const growthToggle = document.getElementById('countryGrowthToggle');
  const growthPanel = document.getElementById('countryGrowthPanel');
  let focusActions = document.getElementById('mapFocusActions');
  let lastFocused = null;
  let frame = 0;

  const getFocusedCountry = () => {
    const selected = list.querySelector('.world-country-button.is-selected,[aria-pressed="true"]');
    const selectedName = decodeURIComponent(selected?.dataset?.country || '').trim();
    const mapCountry = String(map.dataset.focusCountry || '').trim();
    return selectedName || mapCountry || '';
  };
  ...
  function syncNow(){
    ...
    const focused = Boolean(focusedCountry);
    ...
    if (focusedCountry) map.dataset.focusCountry = focusedCountry;
    else delete map.dataset.focusCountry;
    if (focusActions) {
    ...
    if (growthToggle) {
      growthToggle.hidden = false;
      growthToggle.setAttribute('aria-hidden', 'false');
    }
    if (growthPanel && !focused && growthPanel.hidden) {
      growthPanel.setAttribute('aria-hidden', 'true');
    }
    if (lastFocused !== focused) {
      lastFocused = focused;
      window.dispatchEvent(new CustomEvent('andrik:country-focus-changed', { detail: { focused, country: focusedCountry } }));
    }
  }
  ...
  observer.observe(list, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'aria-pressed', 'hidden'] });
