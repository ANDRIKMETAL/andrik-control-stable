(()=>{
  ...
  async function startSequence({recover=false}={}){
    ...
    try{
      setMsg(recover?'Восстанавливаю эфир R710…':'Запускаю эфир R710…','work');
      ...
      if(modern){
        setMsg('R710: перезапускаю OVH encoder чисто…','work');
      ...
      setMsg(reconnect?'YouTube OAuth требует переподключения. Нажми кнопку ниже.':`Ошибка запуска R710: ${e.message||e}`,'bad');
