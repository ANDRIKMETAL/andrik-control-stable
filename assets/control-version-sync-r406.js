/* ANDRIK Control R406 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R406__)return;
  window.__ANDRIK_VERSION_SYNC_R406__=true;
  const RELEASE='R406';
  const RUNTIME='55.00-r406';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R406 LANDSCAPE NO FULLSCREEN TOAST';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:406,version:'55.00'}}));
    }finally{busy=false;}
  };
