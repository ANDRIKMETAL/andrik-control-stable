(() => {
  'use strict';
  if (window.__ANDRIK_CONTROL_PLAYER_R139__ && document.getElementById('andrik-control-player-dock')) return;
  window.__ANDRIK_CONTROL_PLAYER_R139__ = true;
  ...
  const sameOriginControlFrame=window.self!==window.top&&(()=>{
    try{return window.top.location.origin===location.origin}catch(_){return false}
  })();
  ...
  const syncParentVisibility=()=>{
    if(!sameOriginControlFrame)return false;
    const visible=readRequestedVisibility();
    try{
      const parentWindow=window.top;
      if(typeof parentWindow.__ANDRIK_APPLY_MINI_PLAYER_VISIBILITY__==='function'){
        parentWindow.__ANDRIK_APPLY_MINI_PLAYER_VISIBILITY__(visible);
      }else{
        const parentDocument=parentWindow.document;
        const parentDock=parentDocument.getElementById('andrik-control-player-dock');
        parentDocument.documentElement.classList.toggle('andrik-mini-player-hidden',visible===false);
        if(parentDock){
          parentDock.classList.toggle('is-user-hidden',visible===false);
          if(visible===false)parentDock.classList.remove('is-open');
        }
      }
      return true;
  ...
  if(sameOriginControlFrame){
    syncParentVisibility();
    window.addEventListener('pageshow',syncParentVisibility,{passive:true});
    window.addEventListener('storage',event=>{
      if(event.key===VISIBILITY_KEY)syncParentVisibility();
    });
    try{
      const channel=new BroadcastChannel('andrik-control-ui');
      channel.addEventListener('message',event=>{
        if(event.data?.type==='mini-player-visibility')syncParentVisibility();
      });
    }catch(_){}
    return;
  }
  ...
  const existingDock=document.getElementById('andrik-control-player-dock');
  if(existingDock){
    const visible=readRequestedVisibility();
    existingDock.classList.toggle('is-user-hidden',visible===false);
    document.documentElement.classList.toggle('andrik-mini-player-hidden',visible===false);
    return;
  }
  ...
  // R139: всегда начинаем только с круглой кнопки.
  dock.classList.remove('is-open');
