/* ANDRIK Control R426 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R426__)return;
  window.__ANDRIK_VERSION_SYNC_R426__=true;
  const RELEASE='R426';
  const RUNTIME='55.00-r426';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R426 CITY LIST CLIPPED + PORTRAIT/LANDSCAPE SIMPLE LIST + R424 COUNTRY FILL + R416 CRON STABLE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:424,version:'55.00'}}));
    }finally{busy=false;}
  };
