/* ANDRIK R601 — native fast MP3 library, including BEYOND canonical order. */
(()=>{'use strict';
  const albums=[
    {id:'album-beyond',folder:'albums/beyond',expected:17},
  ...
  const beyondCanonicalR601=[null,'Просто живи','Вне сценария','Свобода','Тишина','Ты уже достоин','Внутренний Гуру','Дуккха','Мираж остаётся','WE ARE ONE','WAKE UP','JOY OF BEING','DARK NIGHT OF THE SOUL','PERSONA','BEYOND MAN','LIQUID LIKE MERCURY','YOU’RE ALREADY WORTHY'];
  ...
  const normTitle=v=>String(v||'').toLocaleLowerCase('ru-RU').replace(/ё/g,'е').replace(/[’'`]/g,'').replace(/[^\p{L}\p{N}]+/gu,' ').replace(/\s+/g,' ').trim();
  const beyondNumber=t=>{const probe=normTitle(t?.title||t?.name||t?.key?.split('/').pop()?.replace(/\.mp3$/i,'')||'').replace(/^\d{1,2}\s+/,'').trim();if(!probe)return normalizeNumber(t?.track);for(let i=1;i<beyondCanonicalR601.length;i++){if(normTitle(beyondCanonicalR601[i])===probe||probe.endsWith(' '+normTitle(beyondCanonicalR601[i])))return i}return normalizeNumber(t?.track)};
  ...
  function selectTracks(all,folder){
    ...
    for(const t of source){
      const n=folder==='albums/beyond'?beyondNumber(t):normalizeNumber(t.track);
  ...
  function renderTracks(details,def,tracks){
    ...
    tracks.forEach((t,index)=>{
      ...
      const num=document.createElement('span');num.className='album-fast-track-num-r477';const n=(def.folder==='albums/beyond'?beyondNumber(t):normalizeNumber(t.track))||index+1;num.textContent=String(n).padStart(2,'0');
      const title=document.createElement('strong');title.className='album-fast-track-title-r477';title.textContent=String(def.folder==='albums/beyond'&&beyondCanonicalR601[n]?beyondCanonicalR601[n]:(def.folder==='albums/trika'&&trikaCanonicalR517[n]?trikaCanonicalR517[n]:(t.title||t.key?.split('/').pop()?.replace(/\.mp3$/i,'').replace(/[_-]+/g,' ')||`Track ${n}`)));
