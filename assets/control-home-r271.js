/* Control ANDRIK R271 — exact completed-period daily summary opened from push. */
(() => {
  ...
  const HOME_CACHE_KEY_PUSH_BASE='andrik-control-home-push-r271';
  ...
  const shell=$('controlSwipeShell');
    ...
    try{
      const query=new URLSearchParams({v:'55.00-r271'});
  ...
  $('controlSummaryNext')?.addEventListener('click',()=>{
    ...
    location.assign(`/control-home.html?page=activity${pushQuery}&v=55.00-r271&t=${Date.now()}`);
  });
