/* ANDRIK R441 — calendar navigation for stored daily summaries. */
(()=>{
  'use strict';
  const button=document.getElementById('controlSummaryArchiveR441');
  if(!button)return;
  ...
  modal.className='control-summary-archive-modal-r441';
  modal.id='controlSummaryArchiveModalR441';
  modal.hidden=true;
  modal.setAttribute('aria-hidden','true');
  modal.innerHTML=`<button class="control-summary-archive-backdrop-r441" type="button" aria-label="Закрыть архив"></button><section class="control-summary-archive-panel-r441" role="dialog" aria-modal="true" aria-label="Архив сводок"><header class="control-summary-archive-head-r441"><button type="button" data-archive-prev aria-label="Предыдущий месяц">‹</button><div class="control-summary-archive-month-r441" data-archive-month>Архив</div><button type="button" data-archive-next aria-label="Следующий месяц">›</button><button type="button" data-archive-close aria-label="Закрыть">×</button></header><div class="control-summary-archive-week-r441"><span>Пн</span><span>Вт</span><span>Ср</span><span>Чт</span><span>Пт</span><span>Сб</span><span>Вс</span></div><div class="control-summary-archive-grid-r441" data-archive-grid></div><div class="control-summary-archive-status-r441" data-archive-status>Загружаем сохранённые дни…</div><div class="control-summary-archive-legend-r441">Зелёные даты — сохранённая сводка. Архив хранит завершённые циклы 06:05 → 06:05.</div></section>`;
  ...
  function render(){
    ...
    const cells=[];
    for(let i=0;i<start;i++)cells.push('<span class="control-summary-archive-day-r441 is-empty"></span>');
    for(let d=1;d<=days;d++){
      const key=keyFor(y,m,d),entry=available.get(key);
      if(entry){
        const cls=`control-summary-archive-day-r441 is-available${key===selectedKey?' is-selected':''}`;
        const tip=`Открыть сохранённую сводку за ${String(d).padStart(2,'0')}.${String(m+1).padStart(2,'0')}.${y}`;
        const href=`/control-home.html?page=summary&source=push&archive=1&summaryWindow=${encodeURIComponent(key)}&v=55.00-r441`;
        cells.push(`<a class="${cls}" href="${href}" title="${tip}">${d}</a>`);
      }else cells.push(`<span class="control-summary-archive-day-r441">${d}</span>`);
    }
  ...
  async function loadArchive(){
    ...
    try{
      const response=await fetch(`/api/control/daily-summary/archive?v=55.00-r441&t=${Date.now()}`,{cache:'no-store',headers:{accept:'application/json',authorization:`Bearer ${key}`}});
  ...
  modal.querySelector('.control-summary-archive-backdrop-r441').addEventListener('click',close);
