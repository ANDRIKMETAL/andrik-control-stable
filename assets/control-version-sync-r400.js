/* ANDRIK Control R400 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R400__)return;
  window.__ANDRIK_VERSION_SYNC_R400__=true;
  const RELEASE='R400';
  const RUNTIME='55.00-r400';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R400 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:400,version:'55.00'}}));
    }finally{busy=false;}
  };
