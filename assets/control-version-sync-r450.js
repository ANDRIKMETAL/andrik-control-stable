/* ANDRIK Control R450 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R450__)return;
  window.__ANDRIK_VERSION_SYNC_R450__=true;
  const RELEASE='R450';
  const RUNTIME='55.00-r450';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R450 R449 BASE + PERSISTENT MAP COUNTRY ANCHORS + REARMED PLANET PORTAL';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:450,version:'55.00'}}));
    }finally{busy=false;}
  };
