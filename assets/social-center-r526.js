/* ANDRIK R513 — Social Center platform routing: Site + YouTube + Instagram + TikTok only.
...
(()=>{
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      if(key==='site'){
        href='/analytics-admin.html?page=google&source=social-center&v=55.00-r526';
        detail=live?'28 дней · визиты':'данные сайта';
      }else if(key==='instagram'){
        sub=`<span class="social-center-r488-account">@${esc(meta.username||'andrikmetal')}</span>`;
        href='/instagram-admin.html?source=social-center&v=55.00-r526';
        ...
        href='/youtube-admin.html?source=social-center&v=55.00-r526';
        ...
        sub=`<span class="social-center-r488-account">${esc(meta.handle||'@andrikmetal')}</span>`;
        if(live){
          href='/tiktok-admin.html?source=social-center&v=55.00-r526';
      ...
      return href?`<a class="social-center-r488-platform is-${key}" href="${esc(href)}" aria-label="${esc(def.name)}">${inner}</a>`:`<article class="social-center-r488-platform is-${key}${live?'':' is-future'}">${inner}</article>`;
    }).join('');
  ...
  function renderToggles(data){
    ...
    const keys=['site','youtube','instagram','tiktok'];
    box.innerHTML=keys.map(key=>{const ready=graphConnected(key,data);return `<button type="button" class="social-center-r488-toggle is-${key}${state.active[key]?' is-active':''}" data-social-toggle="${key}" ${ready?'':'disabled'}>${esc(platformDefs[key].name)}</button>`}).join('');
