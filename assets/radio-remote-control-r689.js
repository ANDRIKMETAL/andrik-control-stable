(()=>{
  ...
  async function startSequence({recover=false}={}){
    ...
    try{
      setMsg(recover?'Восстанавливаю эфир R689…':'Запускаю эфир R689…','work');
      ...
      if(modern){
        setMsg('R689: перезапускаю OVH encoder чисто…','work');
      ...
      setMsg(reconnect?'YouTube OAuth требует переподключения. Нажми кнопку ниже.':`Ошибка запуска R689: ${e.message||e}`,'bad');
