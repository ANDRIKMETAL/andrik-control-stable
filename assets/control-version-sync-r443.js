/* ANDRIK Control R443 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R443__)return;
  window.__ANDRIK_VERSION_SYNC_R443__=true;
  const RELEASE='R443';
  const RUNTIME='55.00-r443';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R443 R442 BASE + R2 ALBUM ZIP INSPECTOR + BUILDER';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:443,version:'55.00'}}));
    }finally{busy=false;}
  };
