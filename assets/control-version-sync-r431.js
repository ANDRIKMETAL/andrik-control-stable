/* ANDRIK Control R431 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R431__)return;
  window.__ANDRIK_VERSION_SYNC_R431__=true;
  const RELEASE='R431';
  const RUNTIME='55.00-r431';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R431 R427 MAP ENTRY RESTORE + LANDSCAPE ADMIN GLOBE SCROLL + R430 SUMMARY DELIVERY + R416 CRON CPU SAFE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:431,version:'55.00'}}));
    }finally{busy=false;}
  };
