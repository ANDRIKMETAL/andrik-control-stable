...
const albumNames={'singles':'Синглы ANDRIK','albums/silent':'Silent (Тишина)','albums/beyond':'BEYOND','albums/illusion-of-life':'Illusion of Life','albums/ocean':'OCEAN','albums/trika':'ТРИКА'};
...
function humanName(name){return String(name||'').replace(/\.(?:mp3|wav)$/i,'').replace(/^\s*\d{1,3}[\s._-]+/,'').replace(/[_]+/g,' ').trim()||'Без названия'}
...
function safeName(name,title=''){let base=String(name||'').replace(/\.(?:mp3|wav)$/i,'');if(!base||/^track[_ -]?\d*$/i.test(base))base=title||base;base=translit(base).normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9._-]+/g,'_').replace(/^_+|_+$/g,'').replace(/_+/g,'_');return (base||('track_'+Date.now()))+'.mp3'}
function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function parseTrack(name,index){const m=String(name||'').match(/^\s*(\d{1,3})[\s._-]+/);return m?String(Number(m[1])):String(index+1)}
function formatSize(n){n=Number(n)||0;return n<1048576?(n/1024).toFixed(0)+' KB':(n/1048576).toFixed(1)+' MB'}
function persist(){try{localStorage.setItem('andrik-tag-editor-r317',JSON.stringify({folder:els.folder.value,album:els.album.value,artist:els.artist.value,year:els.year.value,genre:els.genre.value}))}catch(_){}}
function restore(){try{const v=JSON.parse(localStorage.getItem('andrik-tag-editor-r317')||'{}');if(v.folder&&albumNames[v.folder])els.folder.value=v.folder;if(v.album)els.album.value=v.album;if(v.artist)els.artist.value=v.artist;if(v.year)els.year.value=v.year;if(v.genre)els.genre.value=v.genre}catch(_){}}
function syncAlbum(){els.album.value=albumNames[els.folder.value]||els.album.value;persist()}
...
els.list.addEventListener('change',e=>{if(e.target.matches('.tag-r2-select-all')){els.list.querySelectorAll('.tag-r2-select').forEach(b=>b.checked=e.target.checked);syncR2DeleteState();return}if(e.target.matches('.tag-r2-select'))syncR2DeleteState()});
