/* ANDRIK Control R452 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R452__)return;
  window.__ANDRIK_VERSION_SYNC_R452__=true;
  const RELEASE='R452';
  const RUNTIME='55.00-r452';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R452 R451 BASE + INTERACTIVE ALL-TIME COUNTRY HISTORY + CENTERED MAP LABELS';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:452,version:'55.00'}}));
    }finally{busy=false;}
  };
