/* ANDRIK Control R474 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R474__)return;
  window.__ANDRIK_VERSION_SYNC_R474__=true;
  const RELEASE='R474';
  const RUNTIME='55.00-r474';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R474 R473 BASE + 5M WATCHDOG STATUS + SAFE RESCUE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:474,version:'55.00'}}));
    }finally{busy=false;}
  };
