/* ANDRIK Control R408 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R408__)return;
  window.__ANDRIK_VERSION_SYNC_R408__=true;
  const RELEASE='R408';
  const RUNTIME='55.00-r408';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R408 VERSION SYNC + R407 MAP UI FIXES';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:408,version:'55.00'}}));
    }finally{busy=false;}
  };
