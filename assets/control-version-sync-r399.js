/* ANDRIK Control R399 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R399__)return;
  window.__ANDRIK_VERSION_SYNC_R399__=true;
  const RELEASE='R399';
  const RUNTIME='55.00-r399';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R399 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:399,version:'55.00'}}));
    }finally{busy=false;}
  };
