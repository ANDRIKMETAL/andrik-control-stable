/* Control ANDRIK R301 — safe display-version synchronization.
   Never writes text into <html> or <body>. */
(() => {
  'use strict';
  if (window.__ANDRIK_VERSION_SYNC_R301__) return;
  window.__ANDRIK_VERSION_SYNC_R301__ = true;

  const RELEASE = 'R301';
  const RUNTIME = '55.00-r301';
  const FULL = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R301';
  let applying = false;
  ...
  const apply = () => {
    ...
    try {
      ...
      window.ANDRIK_CONTROL_RELEASE = Object.freeze({
        short: RELEASE, number: 301, version: '55.00',
        full: 'v55.00 LIVE WEB AI FINAL R301',
        build: 'R301 AUTOMATIC YOUTUBE SERVER COUNTERS + CRON',
        date: '06.08.2026'
      });
      window.ANDRIK_CONTROL_VERSION = '55.00 LIVE WEB AI FINAL R301';
      window.ANDRIK_CONTROL_BUILD = 'R301 AUTOMATIC YOUTUBE SERVER COUNTERS + CRON';
