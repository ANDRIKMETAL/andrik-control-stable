(() => {
  ...
  async function loadSystem(){if(!getKey()){set('serviceSystemMessage','Сначала сохраните ключ.');return}const button=document.getElementById('serviceSystemRefresh');if(button)button.disabled=true;set('serviceSystemMessage','Проверяем службы…');try{const data=await api('/api/control/system');const s=data.services||{};const cronState=document.getElementById('serviceCronState');if(cronState){const cron=s.cron||{};cronState.className=`service-access-state ${cron.status==='good'?'is-ready':cron.status==='error'?'is-error':''}`;cronState.textContent=cron.status==='good'?'Автоматизация работает':cron.health==='never'?'Ожидает первого запуска':'Требует проверки'}const ordered=[['🌐','Сайт ANDRIK',s.site],['⚙️','Cloudflare Worker',s.worker],['🗄️','D1',s.database],['🔔','OneSignal',s.oneSignal],['👥','Push-аудитория',s.pushAudience],['📨','Последний push',s.lastPush],['▶️','YouTube',s.youtube],['⏱️','Центральный Cron',s.cron],['🌅','Сводка 06:00',s.dailySummary],['🛡️','R2 / Backup',s.backups],['📊','Аналитика',s.analytics],['🔎','Search Console',s.searchConsole],['🫀','ANDRIK Monitor',s.nativeMonitor]];document.getElementById('serviceSystemGrid').innerHTML=ordered.map(([icon,title,item])=>serviceStatusCard(icon,title,item?.label||'Нет данных',item?.status||'warning')).join('');const critical=ordered;const good=critical.filter(([, ,item])=>item?.status==='good').length;const errors=critical.filter(([, ,item])=>item?.status==='error').length;const warnings=critical.length-good-errors;const summary=document.getElementById('serviceSystemSummary');if(summary){summary.className=`system-overall ${errors?'is-error':warnings?'is-warning':'is-good'}`;summary.textContent=errors?`🔴 Ошибок: ${errors}`:warnings?`🟡 Работает ${good}/${critical.length}`:`🟢 Всё работает ${good}/${critical.length}`;}lastSystemText=[`ANDRIK Control v${data.version||'54.03'} — состояние системы`,`Обновлено: ${formatDate(data.updatedAt)}`,'',...ordered.map(([,title,item])=>`${statusMeta(item?.status).symbol} ${title}: ${item?.label||'Нет данных'}`)].join('\n');set('serviceSystemMessage','Состояние обновлено. Секретные значения не выводятся ✅')}catch(error){set('serviceSystemMessage',`Ошибка: ${error.message}`)}finally{if(button)button.disabled=false}}
  ...
  document.getElementById('serviceCacheReset')?.addEventListener('click',()=>{
    ...
    location.href=`/cache-reset-v55-00-r3u.html?manual=1&force=1&fresh=${Date.now()}`;
  });
  if(IS_CONTROL_HOST&&'serviceWorker'in navigator)navigator.serviceWorker.register('/service-worker.js?v=54.36',{scope:'/',updateViaCache:'none'}).catch(()=>{});
