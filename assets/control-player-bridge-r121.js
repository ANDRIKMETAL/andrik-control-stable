(() => {
  'use strict';
  if (window.__ANDRIK_CONTROL_PLAYER_R121__) return;
  window.__ANDRIK_CONTROL_PLAYER_R121__ = true;
  ...
  const OWNER_SYNC_STAMP='andrik-owner-session-sync-r121';
  ...
  document.addEventListener('click',event=>{
    ...
    if(insideMainPlayer&&url.origin===CONTROL_ORIGIN){
      ...
      url.searchParams.set('v','55.00-r121');
  ...
  try{
    if(!sessionStorage.getItem('andrik-player-peek-r121')){
      sessionStorage.setItem('andrik-player-peek-r121','1');
