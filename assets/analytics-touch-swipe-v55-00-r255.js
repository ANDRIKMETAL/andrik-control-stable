/* ANDRIK Control v55.00 R255 — page swipe that never steals the country-list scroll. */
(() => {
  'use strict';
  ...
  const getCurrentIndex = () => {
    ...
    if (['google','site','analytics','ga','ga4'].includes(page)) return 0;
  ...
  const canHandleTarget = target => {
    ...
    if (target.closest('#worldCountries,#countryGrowthPanel,.country-growth-panel,[data-no-page-swipe]')) return false;
    if (target.closest('input,textarea,select,option,button,[contenteditable="true"],iframe,video,audio')) return false;
    return true;
  };
  ...
  document.addEventListener('touchstart', event => {
    if (tracking || event.touches.length !== 1 || !canHandleTarget(event.target)) return;
  ...
  document.addEventListener('touchend', event => {
    ...
    const threshold = Math.max(54, Math.min(90, window.innerWidth * .10));
    const valid = axis === 'horizontal' && Math.abs(dx) >= threshold && Math.abs(dx) > Math.abs(dy) * 1.1 && (velocity >= .14 || Math.abs(dx) >= threshold * 1.25);
