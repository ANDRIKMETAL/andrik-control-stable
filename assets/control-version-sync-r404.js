/* ANDRIK Control R404 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R404__)return;
  window.__ANDRIK_VERSION_SYNC_R404__=true;
  const RELEASE='R404';
  const RUNTIME='55.00-r404';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R404 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:404,version:'55.00'}}));
    }finally{busy=false;}
  };
