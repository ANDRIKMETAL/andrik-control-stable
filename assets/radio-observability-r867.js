(()=>{
  ...
  let hiddenBefore=0;
  let timer=null;
  const FRESH_EVENT_WINDOW_MS_R867=2*60*60*1000;
  const eventMs=e=>Date.parse(e?.at||'')||0;
  const freshEventsR867=events=>events.filter(e=>eventMs(e)&&Date.now()-eventMs(e)<=FRESH_EVENT_WINDOW_MS_R867);
  ...
  function ensureUi(){
    ...
    if(!document.getElementById('r813Diagnostics')){
      ...
      card.innerHTML=`
        ...
        <div class="r813-diag-summary" id="r813DiagSummary">Показываю только свежие события за 2 часа; старые остаются в копируемой диагностике.</div>
  ...
  function buildText(data){
    ...
    const allEvents=(Array.isArray(diag.events)?diag.events:[])
      .filter(e=>!hiddenBefore || Date.parse(e?.at||0)>=hiddenBefore)
      .slice(-30);
    const events=freshEventsR867(allEvents);
    const latest=allEvents.at(-1)||null;
    const latestAgeMs=latest?Date.now()-eventMs(latest):0;
    const fresh=freshEventsR867(events);
    const p=s.streamProfileR814||s.streamProfileR813||{};
    const hdr=[
      ...
      '',
      `EVENTS (${events.length}/30 · fresh2h=${fresh.length})`
    ];
  ...
  function renderDiagnostics(data){
    ...
    if(badge){
      badge.textContent=`R867 · ${events.length} свежих / ${allEvents.length} в истории`;
      const laneWarn=exp>1&&rt>0&&rt<exp;
      badge.className='r813-diag-badge '+(s.transportHealthy!==false&&rt>0?(laneWarn?'r813-warn':'r813-ok'):'r813-bad');
    }
    const summary=document.getElementById('r813DiagSummary');
    if(summary){
      const liveHealthy=(s.transportHealthy!==false&&rt>0);
      summary.innerHTML=`<b>${liveHealthy?'🟢':'🔴'} transport:</b> ${liveHealthy?'healthy':'problem'} · <b>RTMPS:</b> ${rt}/${exp}${exp>1&&rt===1?' <span class="r813-warn">(основной жив, backup не ESTAB)</span>':''} · <b>video feeder:</b> ${s.videoFeederRunning?'✅':'❌'} · <b>handoff:</b> ${esc(txt(s.videoHandoffMode)||'—')}<br><b>Журнал:</b> ${events.length?`${events.length} свежих событий за 2 ч`:(latest?`свежих нет · последняя запись ${Math.max(0,Math.round(latestAgeMs/60000))} мин назад`:'событий ещё нет')}${bad?`<br><b class="r813-bad">lastError:</b> ${esc(bad)}`:''}`;
    }
    lastFullText=buildText(data);
    const log=document.getElementById('r813DiagLog');
    if(log){
      if(!events.length)log.textContent=latest?`Свежих событий за последние 2 часа нет.\nПоследняя архивная запись: ${txt(latest.at)||'—'} · ${txt(latest.event)||'event'}\n\nТекущий статус всё равно попадает в «Скопировать весь лог».`:'Событий ещё нет. Текущий статус всё равно попадёт в «Скопировать весь лог».';
