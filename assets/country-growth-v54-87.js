(() => {
  ...
  if (!list || !toggle || !panel || !target || window.__andrikCountryGrowthV5477Ready) return;
  window.__andrikCountryGrowthV5477Ready = true;
  ...
  function detachPanelToBody(){
    ...
    if (!panel || panel.dataset.detached === 'true') return;
    try {
  ...
  function syncButtons(){
    list.querySelectorAll('.world-country-button[data-code]').forEach(button => {
      ...
      const hasKnownCurrent = hasCurrent || hasPrevious;
      ...
      const currentText = hasKnownCurrent ? `+${num(now)} за 7 дней` : 'данные за 7 дней обновляются';
      const compareText = hasPrevious ? `${arrow} ${num(before)} за предыдущие 7 дней` : 'нет данных за предыдущие 7 дней';
      const desired = `<span class="country-weekly-current ${hasKnownCurrent ? 'is-positive' : 'is-flat'}">${currentText}</span><span class="country-weekly-compare ${state}"><span class="country-weekly-marquee-track"><span>${compareText}</span><span aria-hidden="true">${compareText}</span></span></span>`;
      badge.hidden = false;
  ...
  function loadGrowth(force = false){
    ...
    request = hardFetch('/api/control/country-growth?refresh=1&v=54.87', {
