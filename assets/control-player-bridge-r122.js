(() => {
  'use strict';
  if (window.__ANDRIK_CONTROL_PLAYER_R122__) return;
  window.__ANDRIK_CONTROL_PLAYER_R122__ = true;
  ...
  const OWNER_SYNC_STAMP='andrik-owner-session-sync-r122';
  ...
  document.addEventListener('click',event=>{
    ...
    if(insideMainPlayer&&url.origin===CONTROL_ORIGIN){
      ...
      url.searchParams.set('v','55.00-r122');
  ...
  try{
    if(!sessionStorage.getItem('andrik-player-peek-r122')){
      sessionStorage.setItem('andrik-player-peek-r122','1');
      window.setTimeout(()=>dock.classList.add('is-open'),650);
      window.setTimeout(()=>dock.classList.remove('is-open'),3900);
    }
  }catch(_){}
