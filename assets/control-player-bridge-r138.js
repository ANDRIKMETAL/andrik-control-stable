(() => {
  'use strict';
  if (window.__ANDRIK_CONTROL_PLAYER_R138__ && document.getElementById('andrik-control-player-dock')) return;
  window.__ANDRIK_CONTROL_PLAYER_R138__ = true;
  ...
  const readRequestedVisibility=()=>{
    try{return localStorage.getItem('andrik-control-mini-player-visible')!=='0'}catch(_){return true}
  };

  const ensureTopControlPlayer=()=>{
    if(!nestedInsideControl)return false;
    try{
      const topWindow=window.top;
      const topDocument=topWindow.document;
      const visible=readRequestedVisibility();

      if(typeof topWindow.__ANDRIK_APPLY_MINI_PLAYER_VISIBILITY__==='function'){
        topWindow.__ANDRIK_APPLY_MINI_PLAYER_VISIBILITY__(visible);
      }

      const existingDock=topDocument.getElementById('andrik-control-player-dock');
      if(existingDock){
        topDocument.documentElement.classList.toggle('andrik-mini-player-hidden',visible===false);
        existingDock.classList.toggle('is-user-hidden',visible===false);
        return true;
      }

      if(!topDocument.querySelector('link[data-andrik-player-r138]')){
      ...
      if(!topDocument.querySelector('script[data-andrik-player-r138]')){
        const script=topDocument.createElement('script');
        script.src='/assets/control-player-bridge-r138.js?v=55.00-r138';
        script.defer=true;
        script.dataset.andrikPlayerR138='js';
        topDocument.head.appendChild(script);
      }
      ...
      try{
        window.top?.postMessage?.({
          type:'andrik-ensure-control-player',
          visible:readRequestedVisibility()
        },CONTROL_ORIGIN);
      }catch(__){}
      return false;
    }
  ...
  if(nestedInsideControl){
    ensureTopControlPlayer();
    window.addEventListener('pageshow',ensureTopControlPlayer,{passive:true});
    window.setInterval(ensureTopControlPlayer,1200);
    return;
  }
  ...
  const OWNER_SYNC_STAMP='andrik-owner-session-sync-r138';
  ...
  document.getElementById('andrik-control-player-dock')?.remove();
  ...
  // R138: всегда начинаем только с круглой кнопки.
  dock.classList.remove('is-open');
  ...
  document.addEventListener('click',event=>{
    ...
    if(insideMainPlayer&&url.origin===CONTROL_ORIGIN){
      ...
      url.searchParams.set('v','55.00-r138');
  ...
  window.addEventListener('message',event=>{
    if(event.origin===CONTROL_ORIGIN&&event.data?.type==='andrik-ensure-control-player'){
      applyPlayerVisibility(event.data.visible!==false);
      return;
    }
