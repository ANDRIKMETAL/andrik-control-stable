(()=>{
  ...
  async function ensureLiveR687(){
    let last=null;
    for(let i=0;i<16;i++){
      try{
        const d=await api('/api/control/youtube-live-r687/ensure',{method:'POST',headers:{'content-type':'application/json'},body:'{}'});
        if(d.watchUrl)setWatch(d.watchUrl);
        return d;
      }catch(e){
        last=e;
        const waiting=e.status===409&&String(e.data?.error||'')==='youtube-active-stream-not-found';
        if(!waiting)throw e;
        setMsg(`OVH encoder работает. Ищу его ACTIVE stream на YouTube… ${i+1}/16`,'work');
        await sleep(4000);
      }
    }
    throw last||new Error('YouTube не увидел ACTIVE stream за 64 секунды.');
  }

  async function startSequence({recover=false}={}){
    if(busy)return;busy=true;render(lastRemote||{});
    try{
      setMsg(recover?'Восстанавливаю эфир R687…':'Запускаю эфир R687…','work');
      ...
      if(modern){
        setMsg('R687: перезапускаю OVH encoder чисто…','work');
        ...
        // R687 no longer depends on the obsolete /usr/local/sbin/andrik-youtube helper
      ...
      setMsg('OVH encoder запущен. Нахожу ACTIVE stream и привязываю YouTube LIVE…','work');
      await sleep(2500);
      const live=await ensureLiveR687();
      ...
      setResult(`R687 START ERROR\n${String(e.message||e)}${e.data?.reason?`\nReason: ${e.data.reason}`:''}`);
      setMsg(reconnect?'YouTube OAuth требует переподключения. Нажми кнопку ниже.':`Ошибка запуска R687: ${e.message||e}`,'bad');
