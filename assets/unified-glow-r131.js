(() => {
  'use strict';
  if (window.__ANDRIK_UNIFIED_GLOW_R131__) return;
  window.__ANDRIK_UNIFIED_GLOW_R131__ = true;

  const RELEASE = 'R131';
  const KEY_SESSION = 'andrik-comments-admin-key';
  const KEY_LOCAL = 'andrik-comments-admin-key-persistent';
  const OWNER_SYNC_STAMP = 'andrik-owner-session-sync-r131';
  ...
  const loadPlayerBridge = () => {
    ...
    // R131: embed=1 сам по себе больше не отключает кнопку.
    ...
    if (!document.querySelector('link[data-andrik-player-r131]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = '/assets/control-player-bridge-r131.css?v=55.00-r131';
      link.dataset.andrikPlayerR131 = 'css';
      document.head.appendChild(link);
    }

    if (!document.querySelector('script[data-andrik-player-r131]')) {
      const script = document.createElement('script');
      script.src = '/assets/control-player-bridge-r131.js?v=55.00-r131';
      script.defer = true;
      script.dataset.andrikPlayerR131 = 'js';
  ...
  const apply = () => {
    ...
    body.classList.add('unified-glow-ready', 'andrik-ui-r131');
    const params = new URLSearchParams(location.search);
