/* ANDRIK R425 — global city list by default; landscape is always list-only. */
(()=>{
  'use strict';
  if(window.__ANDRIK_COUNTRY_CITY_HISTORY_R425__)return;
  window.__ANDRIK_COUNTRY_CITY_HISTORY_R425__=true;
  ...
  const load=async()=>{
    ...
    try{
      ...
      const qs=new URLSearchParams({country,mode,v:'55.00-r425'});if(mode==='daily')qs.set('date',date);
  ...
  addEventListener('orientationchange',()=>setTimeout(forceLandscapeList,120),{passive:true});
  matchMedia('(orientation: landscape)').addEventListener?.('change',()=>setTimeout(forceLandscapeList,60));
