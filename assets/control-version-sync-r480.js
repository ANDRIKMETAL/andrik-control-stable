/* ANDRIK Control R480 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R480__)return;
  window.__ANDRIK_VERSION_SYNC_R480__=true;
  const RELEASE='R480';
  const RUNTIME='55.00-r480';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R480 R478 BASE + APPROVED TRIKA COVER + COUNTDOWN UNDER ART + 3-COVER HOME HUB';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:478,version:'55.00'}}));
    }finally{busy=false;}
  };
