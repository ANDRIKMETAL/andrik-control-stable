/* Control ANDRIK R509 — four-platform daily summary + unique city people + calendar YouTube views. */
(() => {
  ...
  function renderPlatformSummaryR509(data={}){
    ...
    const ytViews=Number.isFinite(Number(ytToday?.views))?Number(ytToday.views):Number(s.youtubeViewDelta||s.youtubeViews||0);
    const ytSubs=Number.isFinite(Number(ytToday?.subscribers))?Number(ytToday.subscribers):Number(s.youtubeSubscribers||0);
    const ytLikes=Number.isFinite(Number(ytToday?.likes))?Number(ytToday.likes):Number(s.youtubeLikes||0);
    const ytComments=Number.isFinite(Number(ytToday?.comments))?Number(ytToday.comments):Number(s.youtubeComments||0);
    const cityCount=(Array.isArray(data?.cityActivity)?data.cityActivity:[]).filter(row=>Number(row?.visitors||row?.opens||0)>0).length;
    const cards=[
      platformCardR509({key:'site',title:'Сайт',icon:'⌂',scope:archive?'24 часа':'сегодня',href:'/analytics-admin.html?page=site',metrics:[
      ...
      platformCardR509({key:'youtube',title:'YouTube',icon:'▶',scope:archive?'точный день':'с 00:00',href:'/youtube-admin.html?refresh=1&v=55.00-r509',metrics:[
      ...
      platformCardR509({key:'instagram',title:'Instagram',icon:'◎',scope:archive?'нет архивного среза':'свежий срез · 28 дней',href:'/social-center-admin.html?source=summary&v=55.00-r509',metrics:archive?[]:[
      ...
      platformCardR509({key:'tiktok',title:'TikTok',icon:'♪',scope:archive?'нет архивного среза':'текущий срез',href:'/social-center-admin.html?source=summary&v=55.00-r509',metrics:archive?[]:[
  ...
  async function hydrateSocialSummaryR509(data={}){
    ...
    socialFetchPromiseR509=api(`/api/control/social-overview?fast=1&tiktok_mode=${encodeURIComponent(mode)}&v=55.00-r509`,{timeoutMs:9000})
  ...
  function renderSummary(data){
    ...
    if(releaseButtonR509)releaseButtonR509.hidden=releaseValueR509<=0;
    const dualR509=$('controlSummaryDualR375');
    if(dualR509)dualR509.classList.toggle('has-single-r509',releaseValueR509<=0);
    renderGeoDelta(s);
  ...
  const shell=$('controlSwipeShell');
    ...
    try{
      ...
      if(IS_ARCHIVE_SUMMARY_VIEW){
        ...
        const query=new URLSearchParams({v:'55.00-r509'});
