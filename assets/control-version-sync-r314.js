/* Control ANDRIK R314 — safe display-version synchronization.
   Never writes text into <html> or <body>. */
(() => {
  'use strict';
  if (window.__ANDRIK_VERSION_SYNC_R314__) return;
  window.__ANDRIK_VERSION_SYNC_R314__ = true;

  const RELEASE = 'R314';
  const RUNTIME = '55.00-r314';
  const FULL = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R314';
  let applying = false;
  ...
  const apply = () => {
    ...
    try {
      ...
      window.ANDRIK_CONTROL_RELEASE = Object.freeze({
        ...
        full: 'v55.00 LIVE WEB AI FINAL R314',
        build: 'R314 FAST EXIT · INSTALLER STAGE LABEL LOCK',
        date: '08.08.2026'
      });
      window.ANDRIK_CONTROL_VERSION = '55.00 LIVE WEB AI FINAL R314';
      window.ANDRIK_CONTROL_BUILD = 'R314 FAST EXIT · INSTALLER STAGE LABEL LOCK';
