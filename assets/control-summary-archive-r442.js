/* ANDRIK R442 — in-place daily summary archive. */
(()=>{
  'use strict';
  ...
  async function loadArchive(){
    if(loaded)return;
    try{
      const data=await fetchJson(`/api/control/daily-summary/archive?v=55.00-r442&t=${Date.now()}`,6500);
  ...
  async function openDay(key){
    ...
    try{
      const data=await fetchJson(`/api/control/daily-summary/archive/day?window=${encodeURIComponent(key)}&v=55.00-r442&t=${Date.now()}`,6500);
      selectedKey=key;
      if(typeof window.andrikApplyArchiveSummaryR442==='function'){
        ...
        location.href=`/control-home.html?page=summary&source=push&archive=1&summaryWindow=${encodeURIComponent(key)}&v=55.00-r442`;
      }
    }catch(error){
      status.textContent=`Не удалось открыть день: ${String(error?.message||error)}`;
    }finally{
      loadingDay=false;
    }
  }
