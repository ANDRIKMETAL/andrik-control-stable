/* ANDRIK Control R421 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R421__)return;
  window.__ANDRIK_VERSION_SYNC_R421__=true;
  const RELEASE='R421';
  const RUNTIME='55.00-r421';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R421 ADMIN GLOBE SPACING + R420 ADMIN LAUNCH + R419 CITY LIST + R416 CRON STABLE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:421,version:'55.00'}}));
    }finally{busy=false;}
  };
