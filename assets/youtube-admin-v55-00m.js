(() => {
  ...
  const CONTENT_TYPE_MAP={SHORTS:'Shorts',VIDEO_ON_DEMAND:'Обычные видео',VIDEOONDEMAND:'Обычные видео',LIVE_STREAM:'Прямые трансляции',LIVESTREAM:'Прямые трансляции',STORY:'Истории',UNSPECIFIED:'Не определено',CREATORCONTENTTYPEUNSPECIFIED:'Не определено'};
  ...
  const EVENT_ICONS={'youtube-comment':'💬','youtube-like':'👍','youtube-subscriber':'↑','youtube-subscriber-count':'↑','youtube-unsubscriber':'🔻','site-subscriber':'👥','comment-live':'💬','comment-pending':'💬'};
  ...
  async function forceArtistRefreshR910(reason='manual'){
    ...
    artistRefreshR910Promise=(async()=>{
      ...
      try{
        ...
        try{sessionStorage.setItem('andrik-r915-artist-refresh-ok',String(Date.now()))}catch(_){ }
  ...
  function maybeAutoRefreshArtistR910(studio={}){
    ...
    let last=0;try{last=Number(sessionStorage.getItem('andrik-r915-artist-auto-refresh')||0)}catch(_){ }
    if(Date.now()-last<10*60*1000)return;
    try{sessionStorage.setItem('andrik-r915-artist-auto-refresh',String(Date.now()))}catch(_){ }
  ...
  function renderArtistTracksR910(data={}){
    ...
    const note=fallback?`<div class="youtube-artist-source-note">${String(data.source||'').includes('top-shorts')?'Полка временно собрана из текущих топ Shorts, чтобы блок не пустовал.':'Последний подтверждённый список OAC из приложения YouTube · '+escapeHtml(data.confirmedAt||'04.09.2026')+'. Автоматическая полка заменит его сразу, когда YouTube отдаст её веб-интерфейсу.'}</div>`:'';
  ...
  async function loadArtistExtrasR910(force=false){
    ...
    artistExtrasR910Promise=(async()=>{
      try{
        ...
        if((!Array.isArray(shelfData.tracks)||!shelfData.tracks.length) && Array.isArray(topData.shorts) && topData.shorts.length){
          shelfData.tracks=topData.shorts.slice(0,6).map((row,index)=>({
            title: row.title || `Shorts #${index+1}`,
            thumbnail: row.thumbnail || '',
            url: row.url || topData.channelUrl || shelfData.channelUrl || 'https://www.youtube.com/@andrikmetal'
          }));
          shelfData.fallback=true;
          shelfData.source='top-shorts-fallback';
          shelfData.confirmedAt=shelfData.confirmedAt || new Date().toLocaleDateString('ru-RU');
          shelfData.channelUrl=shelfData.channelUrl || topData.channelUrl || 'https://www.youtube.com/@andrikmetal';
        }
        renderArtistTracksR910(shelfData||{});renderArtistShortsInlineR910(topData||{});artistExtrasLoadedR910=Date.now();
  ...
  function renderArtistStudio(yt={},studio={}){
    ...
    if(kpis)kpis.innerHTML=[
      kpi('🎵',number(music.views),'YouTube Music · 28 дней',`${number(music.estimatedMinutesWatched)} минут просмотра`,'youtube'),
      kpi('⚡',number(shorts.views),'Shorts · 28 дней',`${number(shorts.engagedViews)} вовлечённых просмотров`,'youtube'),
      kpi('🎬',number(vod.views),'Видео · 28 дней',`${number(vod.estimatedMinutesWatched)} минут просмотра`,'youtube'),
      kpi('🔴',number(live.views),'Прямые эфиры · 28 дней',`${number(live.estimatedMinutesWatched)} минут просмотра`,'youtube')
    ].join('');
    ...
    const link=$('youtubeArtistStudioLink');
    if(link){const channelId=String(yt.channelId||studio.channelId||'').trim();link.href=channelId?`https://studio.youtube.com/channel/${encodeURIComponent(channelId)}/analytics/tab-overview/period-default`:'https://studio.youtube.com/';}
  }
  ...
  function renderVideoList(items=[],sectionId='youtubeTopVideosSection',boxId='youtubeTopVideos',mode='views'){
    ...
    if(!list.length){
      if(sectionId==='youtubeTopShortsSection'||sectionId==='youtubeTopVideosRegularSection'){box.innerHTML='<div class="admin-empty">Данных пока нет.</div>';return}
      section.hidden=true;box.innerHTML='';return
    }
    section.hidden=false;
  ...
  let youtubeTopContentR545Promise=null,youtubeTopContentR545LoadedAt=0;
  function showTopContentUnavailableR545(message='Данные топа сейчас не получены. Основная статистика YouTube продолжает работать.'){
    const a=$('youtubeTopShorts'),b=$('youtubeTopVideosRegular');
    if(a)a.innerHTML=`<div class="admin-empty">${escapeHtml(message)}</div>`;
    if(b)b.innerHTML=`<div class="admin-empty">${escapeHtml(message)}</div>`;
  }
  async function loadTopContentR545(force=false){
    if(!getKey())return;
    if(!force&&Date.now()-youtubeTopContentR545LoadedAt<120000)return;
    if(youtubeTopContentR545Promise)return youtubeTopContentR545Promise;
    youtubeTopContentR545Promise=(async()=>{
      const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),10000);
      try{
        const response=await fetch('/api/control/youtube-top-content?v=55.00-r545',{headers:{accept:'application/json',authorization:`Bearer ${getKey()}`},cache:'no-store',signal:controller.signal});
        const data=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(data.error||`HTTP ${response.status}`);
        if(!data.available){showTopContentUnavailableR545('YouTube пока не вернул данные для этого раздела.');return;}
        ...
        youtubeTopContentR545LoadedAt=Date.now();
      }catch(_){showTopContentUnavailableR545('Не удалось загрузить топ. Закройте и откройте раздел ещё раз.');}
      finally{clearTimeout(timer);youtubeTopContentR545Promise=null;}
    })();
    return youtubeTopContentR545Promise;
  }
  ...
  function renderEvents(events=[]){const box=$('youtubeEventFeed');if(!events.length){box.innerHTML='<div class="admin-empty">Новые события появятся после первой проверки.</div>';return}box.innerHTML=events.filter(item=>String(item.type||'').startsWith('youtube-')).map(item=>{const icon=EVENT_ICONS[item.type]||'•';const title=item.title||item.videoTitle||'Событие YouTube';const message=item.message||item.videoTitle||'';const eventClass=item.type==='youtube-unsubscriber'?' is-unsubscriber':(['youtube-subscriber','youtube-subscriber-count'].includes(item.type)?' is-subscriber':'');const body=`<span class="youtube-event-icon">${icon}</span><div><strong>${escapeHtml(title)}</strong><p>${escapeHtml(message)}</p><small>${dateTime(item.createdAt)}</small></div><em>›</em>`;return item.url?`<a class="youtube-event-row${eventClass}" href="${escapeHtml(item.url)}" target="_blank" rel="noopener">${body}</a>`:`<article class="youtube-event-row${eventClass}">${body}</article>`}).join('')||'<div class="admin-empty">Событий YouTube пока нет.</div>'}
  ...
  function bindTopContentLaunchersR545(){
    ['youtubeTopShortsSection','youtubeTopVideosRegularSection'].forEach(id=>{
      const el=$(id);if(!el||el.dataset.boundR545==='1')return;
      el.dataset.boundR545='1';
      el.addEventListener('toggle',()=>{if(el.open)void loadTopContentR545(true);});
    });
  }
  function renderPayload(data={}){
    bindTopContentLaunchersR545();
    renderYoutube(data.youtube||{});
    renderYoutubeMonitor(data.youtubeMonitor||{});
    renderAlbums(data.albums||[]);
    saveCache(data);
    void loadStudioAuthStatus(true);
    // R545: топ-контент не участвует в основной загрузке YouTube; грузим только по тапу на раздел.
  }
  ...
  if(oauthState==='connected'){
    ...
    try{history.replaceState(null,'',location.pathname+(INTEGRATED?'?page=youtube&v=55.00d':'?v=55.00k-r545'))}catch(_){}
  }
