/* ANDRIK R489 — Social Center controller. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R489__)return;
  window.__ANDRIK_SOCIAL_CENTER_R489__=true;
  const RELEASE='R489';
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      const inner=`<div class="social-center-r488-platform-top"><span class="social-center-r488-platform-icon">${esc(def.icon)}</span><span class="social-center-r488-status">${esc(status)}</span></div><h3>${esc(def.name)}</h3>${sub}<strong>${future?'—':esc(total===null?'—':num(total))}</strong><small>${esc(detail)}</small>${href&&!future?'<em>Открыть ↗</em>':''}`;
  ...
  function renderWarning(data){
    ...
    if(!yt.connected)messages.push(`YouTube: ${yt.error||'Studio OAuth пока не вернул дневной ряд. Нажми ↻ для живого обновления.'}`);
  ...
  function render(data){
    ...
    renderPlatforms(data);renderToggles(data);renderChart(data);renderShares(data);renderRecord(data);renderInstagram(data);renderWarning(data);const note=document.querySelector('.social-center-r488-chart-note');if(note){const igLabel=data?.platforms?.instagram?.metricLabel||'просмотры';note.textContent=`Сайт — просмотры страниц, YouTube — просмотры видео, Instagram — ${igLabel}. Нажми на платформу выше, чтобы скрыть или вернуть линию.`;}
  }
