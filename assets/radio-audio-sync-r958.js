...
function paint(ms){ms=Number(ms)||0;current.textContent=`${fmt(ms)} ms`;buttons.forEach(b=>b.classList.toggle('active',Number(b.dataset.audioDelayR949)===ms));}
...
async function status(){const d=await api('/api/control/radio-remote-r627/status?ts='+Date.now());const ver=String(d?.agent?.version||'');const ms=d?.agent?.status?.audioDelayMsR949;if(Number.isFinite(Number(ms)))paint(Number(ms));if(!/R(?:958|95[8-9]|9[6-9]\d)/i.test(ver)){message(`VPS Agent ${ver||'—'} · сначала установи R958`,'warn');buttons.forEach(b=>b.disabled=true);}else if(!applying){buttons.forEach(b=>b.disabled=false);message(`Активно: ${fmt(ms)} ms · следующий клип/заставка · без рестарта`,'ok');}return d;}
