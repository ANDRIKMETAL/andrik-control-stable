/* ANDRIK Control R449 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R449__)return;
  window.__ANDRIK_VERSION_SYNC_R449__=true;
  const RELEASE='R449';
  const RUNTIME='55.00-r449';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R449 R448 BASE + GPU-LIGHT PLANET MAP TRANSITION';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:449,version:'55.00'}}));
    }finally{busy=false;}
  };
