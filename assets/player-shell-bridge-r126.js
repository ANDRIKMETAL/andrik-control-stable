(() => {
  if (window.__ANDRIK_PLAYER_SHELL_R126__) return;
  window.__ANDRIK_PLAYER_SHELL_R126__ = true;
  ...
  const setupOwnerControlEye = async () => {
    ...
    // Верхняя кнопка плеера на главной больше не используется.
    document.querySelectorAll('.topbar .brand-player-btn').forEach(node => node.remove());
    ...
    try {
      ...
      const cluster = document.querySelector('.topbar .brand-cluster');
      if (!cluster) return;

      const link = document.createElement('a');
      link.id = 'andrik-owner-control-eye';
      link.href = 'https://control.andrikmetal.com/control-home.html?v=55.00-r126';
      link.setAttribute('aria-label','Быстро открыть ANDRIK Control');
      link.title = 'Быстрое переключение в Control';
      link.innerHTML = '<span aria-hidden="true"></span><img src="/assets/control-topbar-eye-triangle.jpg?v=55.00-r126" alt="">';
      ...
      if (!document.getElementById('andrik-owner-control-eye-style')) {
        ...
        style.textContent = `
          ...
          .topbar .brand-player-btn{display:none!important}
          #andrik-owner-control-eye{
            position:relative!important;
          ...
          #andrik-owner-control-eye span{
            position:absolute;
            inset:-7px;
            border-radius:50%;
            background:radial-gradient(circle,rgba(96,255,157,.42),rgba(28,218,92,.13) 52%,transparent 73%);
            animation:andrikOwnerHaloR126 3.05s ease-in-out infinite;
            z-index:0;
            pointer-events:none;
          }
          #andrik-owner-control-eye img{
            position:relative;
            z-index:1;
            width:36px;
            height:36px;
            ...
            animation:andrikOwnerEyeR126 3.05s ease-in-out infinite;
            will-change:transform,filter;
            box-shadow:0 0 0 1px rgba(119,255,177,.22);
          }
          #andrik-owner-control-eye:active img{transform:scale(.90)!important}
          @keyframes andrikOwnerEyeR126{
            0%,100%{
              transform:scale(.93);
              filter:brightness(1.02) saturate(1.28)
                drop-shadow(0 0 7px rgba(80,255,143,.55))
                drop-shadow(0 0 18px rgba(22,218,87,.38));
            }
            50%{
              transform:scale(1.08);
              filter:brightness(1.48) saturate(1.6)
                drop-shadow(0 0 14px rgba(146,255,187,.96))
                drop-shadow(0 0 30px rgba(32,239,105,.78));
            }
          }
          @keyframes andrikOwnerHaloR126{
            0%,100%{transform:scale(.70);opacity:.18}
            50%{transform:scale(1.28);opacity:.82}
          }
          @media(max-width:430px){
            .topbar .brand-cluster{gap:6px!important}
            #andrik-owner-control-eye{
              flex-basis:36px!important;
              width:36px!important;
              height:36px!important;
            }
            #andrik-owner-control-eye img{width:32px;height:32px}
            #andrik-owner-control-eye span{inset:-6px}
          }
          @media(max-width:370px){
            .topbar .brand-cluster{gap:4px!important}
            #andrik-owner-control-eye{
              flex-basis:34px!important;
              width:34px!important;
              height:34px!important;
            }
            #andrik-owner-control-eye img{width:30px;height:30px}
          }
          @media(prefers-reduced-motion:reduce){
            #andrik-owner-control-eye img,
            #andrik-owner-control-eye span{animation-duration:6s}
          }
        `;
