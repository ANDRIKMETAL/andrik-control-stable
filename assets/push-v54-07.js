(() => {
  if (window.__ANDRIK_PUSH_V54_05__) return;
  window.__ANDRIK_PUSH_V54_05__ = true;
  ...
  async function subscribe(skipIntro=false){
    ...
    try{
      ...
      const currentId = OneSignal.User.PushSubscription.id || null;
      if(currentId && OneSignal.User.PushSubscription.optedIn){await clearGenericWelcomeNotifications();setTimeout(clearGenericWelcomeNotifications,450);setTimeout(clearGenericWelcomeNotifications,1400);await showSubscriptionSuccess();}
      setButtonState(OneSignal.User.PushSubscription.optedIn?'on':'off',OneSignal.User.PushSubscription.optedIn?t.on:t.off);
      return currentId;
  ...
  async function status(){const OneSignal=await init();return {configured:Boolean(config?.enabled),supported:Boolean(OneSignal?.Notifications?.isPushSupported?.()),optedIn:Boolean(OneSignal?.User?.PushSubscription?.optedIn),subscriptionId:OneSignal?.User?.PushSubscription?.id || null,originMismatch,siteOrigin:configuredOrigin || config?.siteOrigin || ''}}
  window.AndrikPush={init,subscribe,getSubscriptionId,status,syncSubscription,requestPermissionFlow,cleanupLegacyPushSubscriptions,clearGenericWelcomeNotifications};
