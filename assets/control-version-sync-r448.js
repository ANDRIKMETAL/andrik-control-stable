/* ANDRIK Control R448 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R448__)return;
  window.__ANDRIK_VERSION_SYNC_R448__=true;
  const RELEASE='R448';
  const RUNTIME='55.00-r448';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R448 R447 BASE + ARCHIVE NULL GUARD + PORTAL TRANSITION + GLOBE LOWER';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:448,version:'55.00'}}));
    }finally{busy=false;}
  };
