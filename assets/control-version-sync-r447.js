/* ANDRIK Control R447 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R447__)return;
  window.__ANDRIK_VERSION_SYNC_R447__=true;
  const RELEASE='R447';
  const RUNTIME='55.00-r447';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R447 R446 BASE + COMPACT SUMMARY + CITY SOURCES + ALBUM ORDER';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:447,version:'55.00'}}));
    }finally{busy=false;}
  };
