(() => {
  'use strict';
  if (window.__ANDRIK_CONTROL_PLAYER_R124__) return;
  window.__ANDRIK_CONTROL_PLAYER_R124__ = true;
  ...
  try{
    if(!sessionStorage.getItem('andrik-control-player-peek-r124')){
      sessionStorage.setItem('andrik-control-player-peek-r124','1');
      window.setTimeout(()=>dock.classList.add('is-open'),650);
      window.setTimeout(()=>dock.classList.remove('is-open'),4100);
    }
  }catch(_){}
  ...
  if(insideMainPlayer){
    ...
    window.setTimeout(()=>dock.classList.add('is-open'),420);
    window.setTimeout(()=>dock.classList.remove('is-open'),4200);
  }else{
