...
(() => {
  ...
  function monthCard(item){
    ...
    return `<button class="month${item.total>0?' has-comments':''}${active?' is-active current':''}" data-month="${item.month}" type="button"><strong>${MONTHS[item.month-1]}</strong><b>${item.total}</b><small><span>🌐 ${item.site}</span><span>▶️ ${item.youtube}</span><span>👍 ${Number(item.likes||0)}</span></small></button>`;
  }
  function renderMonths(){
    $('yearLabel').textContent=year;
    $('yearSummary').textContent=`Всего: ${data?.totals?.total||0} · Сайт: ${data?.totals?.site||0} · YouTube: ${data?.totals?.youtube||0} · Лайков: ${data?.totals?.likes||0}`;
  ...
  function commentCard(item){
    ...
    return `<article class="comment ${item.source}"><div class="comment-top"><span class="source">${sourceName(item.source)}</span><span class="comment-like-count">👍 ${Number(item.likeCount||0)}</span><time class="time">${dateLine}</time></div><div class="author">${escapeHtml(item.author||'Слушатель')}</div><div class="message">${escapeHtml(item.message||'')}</div><div class="target"><span>${item.source==='youtube'?'🎬':'📄'} ${escapeHtml(item.targetTitle||'ANDRIK')}</span><a href="${escapeHtml(item.url||'#')}" target="_blank" rel="noopener">Открыть</a></div></article>`;
  }
  async function loadMonth(){
    ...
    try{
      ...
      const monthMeta=(result.months||[]).find(item=>Number(item.month)===selectedMonth)||{};
      const likes=Math.max(Number(monthMeta.likes||0),comments.reduce((sum,item)=>sum+Number(item.likeCount||0),0));
      $('monthTitle').textContent=`${MONTHS[selectedMonth-1]} ${year}`;
      $('monthCounts').textContent=`Слушатели: ${comments.length} · Сайт: ${site} · YouTube: ${youtube} · Лайков за месяц: ${likes}`;
