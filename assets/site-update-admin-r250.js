(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r250';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r250';
  ...
  window.AndrikSiteUpdateR250={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR250;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR250;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR250;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
