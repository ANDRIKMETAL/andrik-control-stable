/* ANDRIK R341 — render a real standalone silhouette map for the selected country. */
(() => {
  'use strict';
  if (window.__ANDRIK_COUNTRY_MAP_R341__) return;
  window.__ANDRIK_COUNTRY_MAP_R341__ = true;
  ...
  function ensureOverlay() {
    ...
    let overlay = stage.querySelector('.country-map-r341');
    if (!overlay) {
      overlay = document.createElement('div');
      overlay.className = 'country-map-r341';
  ...
  function renderCountryMap() {
    ...
    const sameSignature = overlay.dataset.signature === signature;
    ...
    if (shape?.path) {
      ...
      let dots = points.map(point => {
        ...
        return `<circle class="country-point-r341" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}"/>`;
      }).join('');
      if (!dots) dots = '<circle class="country-point-r341 is-empty" cx="500" cy="300" r="12"/>';
      svg = `<svg viewBox="0 0 1000 600" role="img" aria-label="Карта страны ${esc(country)}"><path class="country-shape-r341" fill-rule="evenodd" d="${shape.path}"/>${dots}</svg>`;
    } else {
      svg = `<svg viewBox="0 0 1000 600" role="img" aria-label="Карта страны ${esc(country)}"><rect x="90" y="80" width="820" height="440" rx="80" fill="rgba(20,58,78,.55)" stroke="#92dbff" stroke-width="4" stroke-dasharray="12 12"/><circle class="country-point-r341 is-empty" cx="500" cy="300" r="12"/></svg>`;
    }
    if (!sameSignature) {
      overlay.innerHTML = `${svg}<span class="country-map-label-r341">${flag(code)} ${esc(country)}</span><span class="country-map-week-r341">+${fmt(weekly)} · 7 дней</span>${points.length ? '' : '<span class="country-map-empty-label-r341">пока без городских точек</span>'}`;
      overlay.dataset.signature = signature;
    }

    const totalNode = document.getElementById('worldMapTotalValue');
    if (totalNode) totalNode.textContent = fmt(total);
    const totalBox = document.getElementById('worldMapTotal');
    if (totalBox) totalBox.title = `${country} · ${total}`;
  ...
  function ensureActionsVisible() {
    const country = selectedCountry();
    if (!country) return;
    document.body.classList.add('is-country-focus-active');
    ...
    const detailNodes = list.querySelectorAll('.ecosystem-country-detail');
    detailNodes.forEach(node => node.style.setProperty('display','none','important'));
  }

  function sync() {
    ensureActionsVisible();
    requestAnimationFrame(() => requestAnimationFrame(renderCountryMap));
  }
