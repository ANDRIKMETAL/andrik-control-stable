(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r273';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r273';
  ...
  window.AndrikSiteUpdateR273={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR247=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR244=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR243=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR273;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR273;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
