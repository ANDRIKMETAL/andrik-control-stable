/* ANDRIK Control R423 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R423__)return;
  window.__ANDRIK_VERSION_SYNC_R423__=true;
  const RELEASE='R423';
  const RUNTIME='55.00-r423';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R423 ONE-TAP REPORT NAV + GLOBE SPACING + R420 ADMIN LAUNCH + R419 CITY LIST + R416 CRON STABLE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:423,version:'55.00'}}));
    }finally{busy=false;}
  };
