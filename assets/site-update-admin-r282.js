(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r282';
  ...
  async function fetchBackupZip(){
    ...
    try{
      ...
      let response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r247`,{credentials:'include',cache:'no-store',headers,signal:controller.signal});
      if(response.status===401&&raw){
        await window.AndrikOwnerSession?.establish?.(raw).catch(()=>null);
        response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r247-retry`,{credentials:'include',cache:'no-store',headers:{accept:'application/zip',authorization:`Bearer ${raw}`},signal:controller.signal});
      }
      if(!response.ok){
  ...
  function isNetworkFetchFailure(error){
    ...
    return error instanceof TypeError || /failed to fetch|networkerror|network request failed|load failed|connection (?:lost|closed)|fetch failed/i.test(message);
  }
  async function reconcileCommitAfterNetworkFailure(expectedHead='',release='',reinstall=false){
    stage('commit','warn');
    setResultState('warn','Проверяем GitHub');
    setText('siteUpdateResultTitle','Связь прервалась — проверяем результат');
    setText('siteUpdateResultText','Ответ от Worker не дошёл. Это ещё не означает, что Commit не создан. Автоматически проверяем GitHub.');
    setText('siteUpdatePublishMessage','Соединение прервалось на шаге Commit. Не запускай вторую установку, пока идёт проверка.');
    try{window.dispatchEvent(new CustomEvent('andrik:site-update-stage',{detail:{source:'network-reconcile',at:Date.now()}}))}catch(_){}
    const delays=[1800,2600,3800,5200,7000];
    for(let attempt=0;attempt<delays.length;attempt++){
      await sleep(delays[attempt]);
      try{
        const status=await api(`/api/control/site-update/status?reconcile=${Date.now()}-${attempt}`,{timeoutMs:22000,headers:{'cache-control':'no-cache'}});
        const head=String(status?.headSha||'');
        const message=String(status?.headMessage||'');
        const changed=Boolean(head&&expectedHead&&head!==expectedHead);
        const releaseFound=Boolean(release)&&message.toUpperCase().includes(String(release).toUpperCase());
        if(changed&&releaseFound){
          stage('commit','done');
          stage('release','warn');
          setResultState('warn','Commit найден');
          setText('siteUpdateResultTitle',`${release} уже находится в GitHub`);
          setText('siteUpdateResultText',`${reinstall?'Повторная установка':'Обновление'} продолжилось на сервере. Commit ${head.slice(0,7)} найден после потери соединения.`);
          setText('siteUpdatePublishMessage','Commit подтверждён. Проверяем публикацию Cloudflare — повторно нажимать «Обновить сайт» не нужно.');
          lastRelease=release;lastOperationId='';
          previewData=null;confirmInput.checked=false;setPublishMode(false);
          await Promise.allSettled([loadStatus(),loadHistory(),loadLog()]);
          await watchDeployment('',release,'publish');
          return true;
        }
        setText('siteUpdateResultText',`GitHub проверен ${attempt+1}/${delays.length}. Ожидаем завершение Commit…`);
      }catch(_){}
    }
    return false;
  }
  async function publish(){
    ...
    try{
      ...
      if(!publishData&&isNetworkFetchFailure(error)){
        const recovered=await reconcileCommitAfterNetworkFailure(previewData?.headSha||'',release,reinstall);
        if(recovered)return;
      }
      ...
      const message=isNetworkFetchFailure(error)?'Связь с Worker прервалась, а Commit в GitHub не подтвердился. Безопасно выбери тот же ZIP и повтори установку.':String(error?.message||error||'Неизвестная ошибка');
      setText('siteUpdateResultText',message);setText('siteUpdatePublishMessage',`Ошибка: ${message}`);
      try{window.dispatchEvent(new CustomEvent('andrik:site-update-stage',{detail:{source:'publish-error',at:Date.now()}}))}catch(_){}
    }finally{setBusy(false)}
  }
  ...
  window.AndrikSiteUpdateR247={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR244=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR243=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR247;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR247;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
