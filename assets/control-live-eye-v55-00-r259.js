(() => {
  'use strict';
  if(window.__ANDRIK_LIVE_EYE_R259__)return;
  window.__ANDRIK_LIVE_EYE_R259__=true;
  const VERSION='55.00-r259';
  ...
  ['andrik-live-eye-style-r244','andrik-live-eye-style-r254','andrik-live-eye-style-r255','andrik-live-eye-style-r256','andrik-live-eye-style-r258','andrik-live-eye-style-r259'].forEach(id=>document.getElementById(id)?.remove());
  const style=document.createElement('style');
  style.id='andrik-live-eye-style-r259';
  style.textContent=`
    @keyframes andrikR259EyePulse{0%,100%{transform:translate(-50%,-50%) scale(.90)}50%{transform:translate(-50%,-50%) scale(1.13)}}
    @keyframes andrikR259HaloPulse{0%,100%{transform:translate(-50%,-50%) scale(.76);opacity:.48}50%{transform:translate(-50%,-50%) scale(1.52);opacity:1}}
    @keyframes andrikR259HeaderPulse{0%,100%{filter:brightness(.96)}50%{filter:brightness(1.18)}}
    ...
    body.control-page .control-topbar,body.comment-collection-page .control-topbar,.control-topbar{
      ...
      animation:andrikR259HeaderPulse 1.65s ease-in-out infinite!important
    }
    ...
    .andrik-live-eye-motion-r244{width:100%!important;height:100%!important;z-index:3!important;animation:andrikR259EyePulse 1.18s cubic-bezier(.42,0,.58,1) infinite!important}
    .andrik-live-eye-halo-r244{width:146px!important;height:146px!important;border-radius:50%!important;z-index:1!important;background:radial-gradient(circle,var(--r244-halo) 0%,color-mix(in srgb,var(--r244-halo) 62%,transparent) 34%,transparent 74%)!important;filter:blur(12px)!important;animation:andrikR259HaloPulse 1.18s cubic-bezier(.42,0,.58,1) infinite!important}
  ...
  /* R259: independent animation engine. It keeps the eye breathing even when
     older Control CSS or Android accessibility rules suppress CSS animations. */
  let motionAnimation=null;
  let haloAnimation=null;
  const startEyeBreathing=()=>{
    try{
      ...
      motionAnimation=motion.animate([
        {transform:'translate(-50%,-50%) scale(.90)'},
        {transform:'translate(-50%,-50%) scale(1.13)'},
        {transform:'translate(-50%,-50%) scale(.90)'}
      ],{duration:1180,iterations:Infinity,easing:'ease-in-out'});
      haloAnimation=halo.animate([
        {transform:'translate(-50%,-50%) scale(.76)',opacity:.48},
        {transform:'translate(-50%,-50%) scale(1.52)',opacity:1},
        {transform:'translate(-50%,-50%) scale(.76)',opacity:.48}
      ],{duration:1180,iterations:Infinity,easing:'ease-in-out'});
      ...
      halo.style.removeProperty('animation');
    }
  };
  startEyeBreathing();
  const ensureEyeBreathing=()=>{
    try{
      if(!motionAnimation||motionAnimation.playState==='idle')startEyeBreathing();
      else if(motionAnimation.playState==='paused')motionAnimation.play();
      if(haloAnimation?.playState==='paused')haloAnimation.play();
    }catch(_){startEyeBreathing();}
  };
  window.addEventListener('pageshow',ensureEyeBreathing,{passive:true});
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)ensureEyeBreathing();});
  window.setInterval(()=>{if(!document.hidden)ensureEyeBreathing();},4000);

  const adminBluePage=(
    body?.classList.contains('control-home-page')||
    body?.classList.contains('comments-admin-page')||
