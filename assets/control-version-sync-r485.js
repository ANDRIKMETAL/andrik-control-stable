/* ANDRIK Control R483 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R483__)return;
  window.__ANDRIK_VERSION_SYNC_R483__=true;
  const RELEASE='R483';
  const RUNTIME='55.00-r485';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R483 R482 BASE + DEDICATED CLIPS PAGE + COMPACT PROSNIS + DIRECT R2 UPLOAD FIX';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:483,version:'55.00'}}));
    }finally{busy=false;}
  };
