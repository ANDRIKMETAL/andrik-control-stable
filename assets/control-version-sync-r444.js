/* ANDRIK Control R444 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R444__)return;
  window.__ANDRIK_VERSION_SYNC_R444__=true;
  const RELEASE='R444';
  const RUNTIME='55.00-r444';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R444 R443 BASE + PUBLIC ALBUMS HUB + R2 MULTIPART ZIP BUILDER + ADMIN GLOBE LOWER';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:444,version:'55.00'}}));
    }finally{busy=false;}
  };
