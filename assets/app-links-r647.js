(() => {
  ...
  // R647: Android YouTube APP-FIRST. Never let a generic https intent be the
  ...
  function vndYoutubeUrl(webUrl, explicitId = '') {
    const id = explicitId || youtubeVideoId(webUrl);
    if (id) return `vnd.youtube:${id}`;
    // Custom URI scheme is deliberately used instead of a generic https intent.
    // YouTube/ReVanced-compatible clients can claim vnd.youtube; the browser cannot.
    return `vnd.youtube:${webUrl}`;
  }

  function launchAppFirst(webUrl, id = '') {
    const didLeave = navigationWatch();
    try { window.location.href = vndYoutubeUrl(webUrl, id); } catch (_) {}
    // Browser is only a last fallback when no compatible app handled vnd.youtube.
    setTimeout(() => {
  ...
  async function openYoutubeFromRealTap(event, link) {
    ...
    if (isLiveChannel) {
      ...
      if (live?.id) {
        ...
        launchAppFirst(originalFallback);
      }
      return;
    }
