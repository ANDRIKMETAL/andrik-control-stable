(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r264';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r264';
  ...
  async function api(path,options={}){
    ...
    try{
      ...
      let data={};
      try{data=rawText?JSON.parse(rawText):{}}catch(_){data={message:''}}
      if(!response.ok){
        ...
        const fallback=(response.status===404&&data.error==='not-found')
          ...
          :(data.message||data.error||(transient?'GitHub или Cloudflare временно не ответили. Установщик остановлен безопасно — повторите установку этим же ZIP.':`HTTP ${response.status}`));
        const error=new Error(fallback);error.data=data;error.status=response.status;error.retryable=transient||Boolean(data.retryable);throw error
      }
      return data
    }catch(error){
  ...
  async function publish(){
    ...
    try{
      ...
      const retryable=Boolean(error?.retryable)||[429,500,502,503,504].includes(Number(error?.status||0));
      setText('siteUpdateResultTitle',retryable?'Временный сбой — можно повторить':(reinstall?'Перепрошивка остановлена':'Публикация остановлена'));
      const message=retryable?`${error.message} Backup уже сохранён; выбери тот же ZIP и нажми «Обновить сайт» повторно.`:error.message;
      setText('siteUpdateResultText',message);setText('siteUpdatePublishMessage',`Ошибка: ${message}`);
      byId('siteUpdateSetupCard').hidden=true;
      if(retryable)setTimeout(()=>loadStatus().catch(()=>{}),900)
    }finally{setBusy(false)}
  }
