(() => {
  ...
  function renderEvents(events=[]){const box=$('youtubeEventFeed');if(!events.length){box.innerHTML='<div class="admin-empty">Новые события появятся после первой проверки.</div>';return}box.innerHTML=events.filter(item=>String(item.type||'').startsWith('youtube-')).map(item=>{const icon=EVENT_ICONS[item.type]||'•';const title=item.title||item.videoTitle||'Событие YouTube';const message=item.message||item.videoTitle||'';const eventClass=item.type==='youtube-unsubscriber'?' is-unsubscriber':'';const body=`<span class="youtube-event-icon">${icon}</span><div><strong>${escapeHtml(title)}</strong><p>${escapeHtml(message)}</p><small>${dateTime(item.createdAt)}</small></div><em>›</em>`;return item.url?`<a class="youtube-event-row${eventClass}" href="${escapeHtml(item.url)}" target="_blank" rel="noopener">${body}</a>`:`<article class="youtube-event-row${eventClass}">${body}</article>`}).join('')||'<div class="admin-empty">Событий YouTube пока нет.</div>'}
