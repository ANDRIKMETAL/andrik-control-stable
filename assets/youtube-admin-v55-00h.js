(() => {
  ...
  const CACHE_KEY='andrik-control-youtube-pane-v55-00h';
  const MONITOR_CACHE_KEY='andrik-control-youtube-monitor-v55-00h';
  ...
  function renderYoutube(yt={}){
    ...
    renderVideoList((Array.isArray(studio.topShorts28)?studio.topShorts28:[]).slice(0,4),'youtubeTopShortsSection','youtubeTopShorts','28d');
    renderVideoList((Array.isArray(studio.topVideosRegular28)?studio.topVideosRegular28:[]).slice(0,4),'youtubeTopVideosRegularSection','youtubeTopVideosRegular','28d');
  ...
  async function connectYoutubeStudio(){
    ...
    if(!getKey()){
      ...
      location.href='/service-admin.html?return=youtube&v=55.00g';
      return;
    }
  ...
  if(oauthState==='connected'){
    ...
    try{history.replaceState(null,'',location.pathname+(INTEGRATED?'?page=youtube&v=55.00d':'?v=55.00g'))}catch(_){}
  }
