(() => {
  'use strict';
  if (window.__ANDRIK_UNIFIED_GLOW_R127__) return;
  window.__ANDRIK_UNIFIED_GLOW_R127__ = true;

  const RELEASE = 'R127';
  const KEY_SESSION = 'andrik-comments-admin-key';
  const KEY_LOCAL = 'andrik-comments-admin-key-persistent';
  const OWNER_SYNC_STAMP = 'andrik-owner-session-sync-r127';
  ...
  const loadPlayerBridge = () => {
    ...
    if (!document.querySelector('link[data-andrik-player-r127]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = '/assets/control-player-bridge-r127.css?v=55.00-r127';
      link.dataset.andrikPlayerR127 = 'css';
      document.head.appendChild(link);
    }

    if (!document.querySelector('script[data-andrik-player-r127]')) {
      const script = document.createElement('script');
      script.src = '/assets/control-player-bridge-r127.js?v=55.00-r127';
      script.defer = true;
      script.dataset.andrikPlayerR127 = 'js';
  ...
  const apply = () => {
    ...
    body.classList.add('unified-glow-ready', 'andrik-ui-r127');
    const params = new URLSearchParams(location.search);
