/* ANDRIK Control R409 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R409__)return;
  window.__ANDRIK_VERSION_SYNC_R409__=true;
  const RELEASE='R409';
  const RUNTIME='55.00-r409';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R409 VERSION SYNC + R407 MAP UI FIXES';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:409,version:'55.00'}}));
    }finally{busy=false;}
  };
