/* ANDRIK R493 — Social Center: Site + YouTube + Instagram, TikTok reserved as the only next platform. */
(()=>{
  'use strict';
  if(window.__ANDRIK_SOCIAL_CENTER_R493__)return;
  window.__ANDRIK_SOCIAL_CENTER_R493__=true;
  const RELEASE='R493';
  ...
  async function youtubeReconnectR493(button){
  ...
  function renderPlatforms(data){
    ...
    box.innerHTML=keys.map(key=>{
      ...
      if(key==='instagram'){
        ...
        else if(meta.reconnectRequired) detail='Доступ Google отозван — переподключи';
        else if(meta.error) detail='Studio: '+String(meta.error).slice(0,75);
        else detail='Нужна авторизация YouTube Studio';
        action=(meta.canReconnect!==false?'<button class="social-center-r493-reconnect" type="button" data-youtube-reconnect-r493>Переподключить YouTube</button>':'')+
          '<a class="social-center-r493-open" href="https://www.youtube.com/@andrikmetal" target="_blank" rel="noopener noreferrer">Открыть канал ↗</a>';
    ...
    box.querySelectorAll('[data-youtube-reconnect-r493]').forEach(button=>button.addEventListener('click',()=>youtubeReconnectR493(button)));
  }
  ...
  function renderWarning(data){
    ...
    if(!yt.connected){
      ...
      if(yt.reconnectRequired)messages.push('YouTube: доступ Google был отозван. Нажми «Переподключить YouTube» в карточке выше.');
      else if(yt.oauthConnected)messages.push(`YouTube OAuth подключён${realError?` · ${realError}`:' · дневная аналитика ещё не пришла'}. Можно нажать ↻ или переподключить YouTube в карточке.`);
      else messages.push(`YouTube: ${realError||'нужна авторизация Studio'}. Нажми «Переподключить YouTube» в карточке.`);
    }
  ...
  const boot=()=>{
    ...
    if(justConnected){
      const warning=$('socialCenterWarningR488');if(warning){warning.hidden=false;warning.textContent='YouTube переподключён. Получаем свежую аналитику…'}
