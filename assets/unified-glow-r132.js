(() => {
  'use strict';
  if (window.__ANDRIK_UNIFIED_GLOW_R132__) return;
  window.__ANDRIK_UNIFIED_GLOW_R132__ = true;
  ...
  const loadPlayerBridge = () => {
    ...
    // R132: embed=1 сам по себе больше не отключает кнопку.
    ...
    if (!document.querySelector('link[data-andrik-player-r132]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = '/assets/control-player-bridge-r132.css?v=55.00-r132';
      link.dataset.andrikPlayerR132 = 'css';
      document.head.appendChild(link);
    }

    if (!document.querySelector('script[data-andrik-player-r132]')) {
      const script = document.createElement('script');
      script.src = '/assets/control-player-bridge-r132.js?v=55.00-r132';
      script.defer = true;
      script.dataset.andrikPlayerR132 = 'js';
  ...
  const apply = () => {
    ...
    body.classList.add('unified-glow-ready', 'andrik-ui-r132');
    const params = new URLSearchParams(location.search);
    ...
    document.querySelectorAll('.control-version-footer strong').forEach(node => {
      node.textContent = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R132';
    });
