(() => {
  'use strict';
  if (window.__ANDRIK_UNIFIED_GLOW_R138__) return;
  window.__ANDRIK_UNIFIED_GLOW_R138__ = true;
  ...
  const loadPlayerBridge = () => {
    ...
    // R138: плеер работает и на странице обновления сайта.
    // Исключаются только технические cache-reset страницы.
    if (isReset) return;

    if (!document.querySelector('link[data-andrik-player-loader-r138]')) {
    ...
    if (!document.querySelector('script[data-andrik-player-loader-r138]')) {
      const script = document.createElement('script');
      script.src = '/assets/control-player-loader-r138.js?v=55.00-r138';
      script.defer = true;
  ...
  const apply = () => {
    ...
    body.classList.add('unified-glow-ready', 'andrik-ui-r138');
    const params = new URLSearchParams(location.search);
    // R138: embed-класс нужен только настоящему вложенному iframe.
    ...
    document.querySelectorAll('.control-version-footer strong').forEach(node => {
      node.textContent = 'Live Web AI · ANDRIK · v55.00 LIVE WEB AI FINAL R138';
    });
