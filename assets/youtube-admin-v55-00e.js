(() => {
  ...
  const CACHE_KEY='andrik-control-youtube-pane-v55-00e';
  const MONITOR_CACHE_KEY='andrik-control-youtube-monitor-v55-00e';
  ...
  const SHARING_MAP={WHATS_APP:'WhatsApp',FACEBOOK:'Facebook',COPY_TO_CLIPBOARD:'Копирование ссылки',COPY_PASTE:'Копирование ссылки',OTHER:'Другое',TEXT_MESSAGE:'SMS',SMS:'SMS',EMAIL:'Gmail',GMAIL:'Gmail',MESSENGER:'Messenger',FACEBOOK_MESSENGER:'Facebook Messenger',TELEGRAM:'Telegram',X:'X / Twitter',VKONTAKTE:'ВКонтакте',ODNOKLASSNIKI:'Одноклассники',DIRECT_SYSTEM_ACTIVITY_DIALOG:'Системное меню Android',DIRECT_SYSTEM:'Системное меню Android',UNKNOWN:'Другое'};
  ...
  function renderTopVideos(items=[]){const section=$('youtubeTopVideosSection');const box=$('youtubeTopVideos');if(!section||!box)return;const list=(Array.isArray(items)?items:[]).filter(item=>item&&item.title);if(!list.length){section.hidden=true;box.innerHTML='<div class="admin-empty">Ролики пока не загружены.</div>';return}section.hidden=false;box.innerHTML=list.slice(0,6).map((item,index)=>{const views=Math.max(0,Number(item.views||0)),likes=Math.max(0,Number(item.likes||0)),comments=Math.max(0,Number(item.comments||0));const meta=[dateLabel(item.publishedAt),`${number(views)} просмотров`].filter(Boolean).join(' · ');const open=item.url?`<a class="youtube-video-open" href="${escapeHtml(item.url)}" target="_blank" rel="noopener">Открыть ↗</a>`:'';return `<article class="youtube-video-row"><div class="youtube-video-rank">#${index+1}</div><div class="youtube-video-copy"><strong>${escapeHtml(item.title||`Видео #${index+1}`)}</strong><p>${escapeHtml(meta)}</p><div class="youtube-video-stats"><span>👍 ${number(likes)}</span><span>💬 ${number(comments)}</span></div></div>${open}</article>`}).join('')}
  ...
  async function connectYoutubeStudio(){
    ...
    if(!getKey()){
      ...
      location.href='/service-admin.html?return=youtube&v=55.00e';
      return;
    }
  ...
  if(oauthState==='connected'){
    ...
    try{history.replaceState(null,'',location.pathname+(INTEGRATED?'?page=youtube&v=55.00d':'?v=55.00e'))}catch(_){}
  }
