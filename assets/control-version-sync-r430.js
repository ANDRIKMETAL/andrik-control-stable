/* ANDRIK Control R430 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R430__)return;
  window.__ANDRIK_VERSION_SYNC_R430__=true;
  const RELEASE='R430';
  const RUNTIME='55.00-r429';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R430 DIRECT MAP ANCHOR + REDUNDANT SUMMARY DELIVERY + R427 CITY PANEL + R416 CRON CPU SAFE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:428,version:'55.00'}}));
    }finally{busy=false;}
  };
