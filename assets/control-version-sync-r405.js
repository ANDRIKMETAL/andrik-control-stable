/* ANDRIK Control R405 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R405__)return;
  window.__ANDRIK_VERSION_SYNC_R405__=true;
  const RELEASE='R405';
  const RUNTIME='55.00-r405';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R405 EXTERNAL CRON DIRECT CHECKPOINT + RETRY GATEWAY';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:405,version:'55.00'}}));
    }finally{busy=false;}
  };
