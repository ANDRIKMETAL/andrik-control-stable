...
(()=>{'use strict';
  ...
  const copy={
    ru:{ready:'⬇ Скачать альбом целиком',wait:'ZIP ещё не создан',size:'ZIP'},
    uk:{ready:'⬇ Завантажити альбом повністю',wait:'ZIP ще не створено',size:'ZIP'},
    sk:{ready:'⬇ Stiahnuť celý album',wait:'ZIP ešte nie je vytvorený',size:'ZIP'},
    en:{ready:'⬇ Download full album',wait:'ZIP not created yet',size:'ZIP'}
  }[lang]||{ready:'⬇ Download full album',wait:'ZIP not created yet',size:'ZIP'};
...
/* R446 — floating player access on the dedicated albums page. */
(()=>{
  'use strict';
  ...
  const make=()=>{
    ...
    a.id='albumsPlayerFabR446';a.className='albums-player-fab-r446';a.href=`/player.html?lang=${encodeURIComponent(lang)}&collection=ocean&from=albums`;
