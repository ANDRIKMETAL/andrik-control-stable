/* ANDRIK Control R475 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R475__)return;
  window.__ANDRIK_VERSION_SYNC_R475__=true;
  const RELEASE='R475';
  const RUNTIME='55.00-r475';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R475 R474 BASE + HOME PROMO FIRST + COUNTDOWN SECOND';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:475,version:'55.00'}}));
    }finally{busy=false;}
  };
