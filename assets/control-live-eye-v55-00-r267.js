(() => {
  'use strict';
  if(window.__ANDRIK_LIVE_EYE_R267__)return;
  window.__ANDRIK_LIVE_EYE_R267__=true;
  const VERSION='55.00-r267';
  ...
  logo.dataset.r267LiveEye='1';
  logo.dataset.r266LiveEye='1';
  ...
  style.id='andrik-live-eye-style-r267';
  style.textContent=`
    @keyframes andrikR267EyePulse{0%,100%{transform:translate(-50%,-50%) scale(.9)}50%{transform:translate(-50%,-50%) scale(1.16)}}
    @keyframes andrikR267HaloPulse{0%,100%{transform:translate(-50%,-50%) scale(.7);opacity:.42}50%{transform:translate(-50%,-50%) scale(1.62);opacity:1}}
    @keyframes andrikR267HeaderPulse{0%,100%{filter:brightness(.96)}50%{filter:brightness(1.18)}}
    ...
    body.control-page .control-topbar,body.comment-collection-page .control-topbar,.control-topbar{
      ...
      animation:andrikR267HeaderPulse 1.45s ease-in-out infinite!important
    }
    ...
    .andrik-live-eye-motion-r244{width:100%!important;height:100%!important;z-index:3!important;animation:andrikR267EyePulse 1.12s ease-in-out infinite!important;animation-play-state:running!important;animation-fill-mode:both!important}
    .andrik-live-eye-halo-r244{width:146px!important;height:146px!important;border-radius:50%!important;z-index:1!important;background:radial-gradient(circle,var(--r244-halo) 0%,color-mix(in srgb,var(--r244-halo) 62%,transparent) 34%,transparent 74%)!important;filter:blur(12px)!important;animation:andrikR267HaloPulse 1.12s ease-in-out infinite!important;animation-play-state:running!important;animation-fill-mode:both!important}
  ...
  const forceBreath=()=>{
    motion.style.setProperty('animation','none','important');
    halo.style.setProperty('animation','none','important');
    void motion.offsetWidth;
    motion.style.setProperty('animation','andrikR267EyePulse 1.12s ease-in-out infinite','important');
    motion.style.setProperty('animation-play-state','running','important');
    halo.style.setProperty('animation','andrikR267HaloPulse 1.12s ease-in-out infinite','important');
    halo.style.setProperty('animation-play-state','running','important');
    const topbar=logo.closest('.control-topbar');
    if(topbar){
      topbar.style.setProperty('animation','andrikR267HeaderPulse 1.45s ease-in-out infinite','important');
      topbar.style.setProperty('animation-play-state','running','important');
    }
  };
  forceBreath();
  window.addEventListener('pageshow',forceBreath,{passive:true});
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)forceBreath()},{passive:true});

  const adminSection=String(body?.dataset?.controlSection||'').toLowerCase();
  const adminBluePage=(
    ['service','monitor','monitoring','releases','release','discussion','comments'].includes(adminSection)||
    ...
    path.includes('release-admin')||
    path.includes('releases-admin')||
    path.includes('discussion-admin')
  );
  ...
  const readStage=()=>{
    ...
    const statusText=`${result?.textContent||''} ${document.getElementById('siteUpdateDeployMessage')?.textContent||''} ${document.getElementById('siteUpdateResultText')?.textContent||''} ${document.getElementById('siteUpdatePublishMessage')?.textContent||''}`;
    ...
    const error=items.some(x=>x.classList.contains('is-error'))||result?.classList.contains('is-error')||/ошиб|сбой|failed|failure|критичес|неуда/i.test(statusText);
    if(error)return 'red';
    ...
    if(complete){goldLatched=false;goldUntil=0;return 'green';}
  ...
  sync();
  setInterval(()=>{sync();if(getComputedStyle(motion).animationName==='none')forceBreath();},900);
