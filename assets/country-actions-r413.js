/* ANDRIK R413 — hard lock for the two portrait country buttons only. */
(()=>{
  'use strict';
  if(window.__ANDRIK_COUNTRY_ACTIONS_R413__)return;
  window.__ANDRIK_COUNTRY_ACTIONS_R413__=true;
  ...
  const apply=()=>{
    ...
    set(box,'min-height','104px');
    set(box,'height','104px');
    set(box,'max-height','104px');
    ...
    box.querySelectorAll('.map-focus-action').forEach(btn=>{
      ...
      set(btn,'width','100%');
      set(btn,'min-width','0');
      set(btn,'min-height','104px');
      set(btn,'height','104px');
      set(btn,'max-height','104px');
