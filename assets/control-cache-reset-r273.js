/* ANDRIK Control R273 — one-time runtime cache migration. */
(() => {
  'use strict';
  const VERSION = '55.00-r273';
  const KEY = 'andrik-control-runtime-version';
  const ONCE = 'andrik-control-runtime-reload-r273';
  ...
  (async () => {
    try {
      ...
      console.warn('Control runtime cache migration R273:', error);
      try { localStorage.setItem(KEY, VERSION); } catch (_) {}
    }
  })();
})();
