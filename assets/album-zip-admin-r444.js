/* ANDRIK R444 — browser-side STORE ZIP + R2 multipart uploader. Large albums are never buffered as one file. */
(()=>{'use strict';
  const root=document.getElementById('albumZipCardR444'); if(!root)return;
  const grid=document.getElementById('albumZipGridR444'), status=document.getElementById('albumZipStatusR444'), refresh=document.getElementById('albumZipRefreshR444');
  const cache=new Map();
  ...
  function renderAlbum(a){
    ...
    return `<article class="album-zip-item-r444" data-album="${esc(a.slug)}">
      <div><span class="eyeline">ANDRIK · R2</span><strong>${esc(a.label)}</strong></div>
      <div class="album-zip-stats-r444">
        <span class="album-zip-pill-r444 ok">MP3 в R2: ${sourceCount}</span>
        <span class="album-zip-pill-r444 ${archiveCount?'ok':'warn'}">В ZIP: ${archiveCount}</span>
        <span class="album-zip-pill-r444">${esc(size(a.totalBytes))}</span>
        <span class="album-zip-pill-r444 ${zip.exists?'ok':'warn'}">ZIP: ${zip.exists?esc(size(zip.size)):'нет'}</span>
      </div>
      ${dupes?`<div class="album-zip-note-r444">ℹ️ Найдено дублей по номеру трека: ${dupes}. В архив попадёт только выбранная основная версия.</div>`:''}
      <div class="album-zip-tracks-r444">${tracks.length?tracks.map((t,i)=>`<div class="album-zip-track-r444"><span>${esc(String(t.track||i+1).padStart(2,'0'))}. ${esc(t.title||t.key)}</span><span>${esc(size(t.size))}</span></div>`).join(''):'<div class="admin-empty">MP3 не найдены.</div>'}</div>
      <div class="album-zip-progress-r444" hidden><span></span></div>
      <div class="album-zip-actions-r444">
        <button class="btn album-zip-check-r444" type="button">Проверить</button>
        <button class="btn btn-primary album-zip-build-r444" type="button" ${tracks.length?'':'disabled'}>${zip.exists?'↻ Пересобрать ZIP':'📦 Собрать ZIP'}</button>
        <a class="btn album-zip-download-r444" ${zip.exists?`href="${esc(zip.downloadUrl)}"`:'aria-disabled="true"'}>${zip.exists?'⬇ Скачать готовый ZIP':'ZIP ещё не создан'}</a>
      </div>
    </article>`;
  }
  ...
  async function build(album,button){
    ...
    const card=button.closest('.album-zip-item-r444'),progress=card?.querySelector('.album-zip-progress-r444'),bar=progress?.querySelector('span');
  ...
  grid?.addEventListener('click',e=>{const card=e.target.closest('[data-album]');if(!card)return;const album=card.dataset.album;if(e.target.closest('.album-zip-check-r444'))load(album).catch(()=>{});const b=e.target.closest('.album-zip-build-r444');if(b)build(album,b)});
