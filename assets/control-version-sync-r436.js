/* ANDRIK Control R436 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R436__)return;
  window.__ANDRIK_VERSION_SYNC_R436__=true;
  const RELEASE='R436';
  const RUNTIME='55.00-r436';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R436 R434 STABLE BASE + YOUTUBE REPLY PUSH RECOVERY + REPLY HIGH-WATER + R416 CPU SAFE';
