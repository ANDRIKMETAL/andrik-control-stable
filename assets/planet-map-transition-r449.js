/* ANDRIK R449 — GPU-light planet <-> world-map portal. No giant DOM layers, no animation wait deadlocks. */
(()=>{
  'use strict';
  if(window.__ANDRIK_PLANET_MAP_TRANSITION_R449__)return;
  window.__ANDRIK_PLANET_MAP_TRANSITION_R449__=true;

  const K_RECT='andrik-portal-globe-rect-r449';
  const K_OPEN='andrik-portal-map-open-r449';
  const K_RETURN='andrik-portal-return-r449';
  ...
  const withVersion=url=>{
    try{const u=new URL(url,location.href);u.searchParams.set('v','55.00-r449');return u.pathname+u.search+u.hash}catch(_){return url}
  };
  ...
  const createPortal=()=>{
    const root=document.createElement('div');root.className='andrik-map-portal-r449';root.setAttribute('aria-hidden','true');
    const back=document.createElement('div');back.className='andrik-map-portal-backdrop-r449';
    const orb=document.createElement('div');orb.className='andrik-map-portal-orb-r449';
    const map=document.createElement('div');map.className='andrik-map-portal-map-r449';
    const flare=document.createElement('div');flare.className='andrik-map-portal-flare-r449';
  ...
  const readRect=()=>{
    try{
      const raw=sessionStorage.getItem(K_RECT) || sessionStorage.getItem('andrik-portal-globe-rect-r448');
  ...
  async function open(globe){
    ...
    const href=globe.href||'/analytics-admin.html?page=map&source=admin-globe&v=55.00-r449';
    ...
    if(clone){clone.classList.add('andrik-map-portal-source-r449');p.orb.insertBefore(clone,p.map)}
    ...
    sphere.classList.add('andrik-portal-source-hidden-r449');
  ...
  async function arriveMap(){
    let stamp=0;
    try{stamp=sessionStorage.getItem(K_OPEN)||sessionStorage.getItem('andrik-portal-map-open-r448')}catch(_){}
    if(!fresh(stamp)){
      document.documentElement.classList.remove('andrik-portal-map-arrival-pending-r449');
      return;
    }
    try{sessionStorage.removeItem(K_OPEN);sessionStorage.removeItem('andrik-portal-map-open-r448')}catch(_){}
  ...
  async function arriveAdmin(){
    let stamp=0;
    try{stamp=sessionStorage.getItem(K_RETURN)||sessionStorage.getItem('andrik-portal-return-r448')}catch(_){}
    if(!fresh(stamp)){
      document.documentElement.classList.remove('andrik-portal-return-pending-r449');
      return;
    }
    try{sessionStorage.removeItem(K_RETURN);sessionStorage.removeItem('andrik-portal-return-r448')}catch(_){}

    const sphere=document.getElementById('adminHubGlobeSphereR418');
    if(!sphere){document.documentElement.classList.remove('andrik-portal-return-pending-r449');return}
    if(reduced()){document.documentElement.classList.remove('andrik-portal-return-pending-r449');return}
    ...
    sphere.classList.remove('andrik-portal-source-hidden-r449');
    p.root.remove();
    document.documentElement.classList.remove('andrik-portal-return-pending-r449');
  }
