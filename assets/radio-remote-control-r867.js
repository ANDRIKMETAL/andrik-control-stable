(()=>{
  ...
  async function goldRestore(){
    ...
    busy=true;render(lastRemote||{});
    try{
      setMsg('🚑 Возвращаю доказанный FULLSCREEN GOLD…','work');
      const d=await agentAction('gold-restore');
      setResult(String(d?.result?.output||'FULLSCREEN GOLD RESTORE ✅'));
      setMsg('FULLSCREEN GOLD восстановлен ✅ · жду YouTube…','ok');
      await sleep(5000);
      await ensureLiveR689().catch(()=>null);
    }catch(e){setMsg(`GOLD restore: ${e.message||e}`,'bad');setResult(`GOLD RESTORE ERROR\n${e.message||e}`)}
    finally{busy=false;await refresh()}
  }
  ...
  window.AndrikRadioRemoteR867={refresh,start:()=>startSequence(),goldRestore,cacheClean,softRestart,stop:stopSequence,status:statusSequence,saveTicker};window.AndrikRadioRemoteR687=window.AndrikRadioRemoteR867;window.AndrikRadioRemoteR665=window.AndrikRadioRemoteR867;
