/* ANDRIK R452 — ecosystem map + interactive all-time historical country list on the ALL map. */
(() => {
  'use strict';
  if (window.__ANDRIK_ECOSYSTEM_MAP_R452__) return;
  window.__ANDRIK_ECOSYSTEM_MAP_R452__ = true;
  ...
  async function loadEcosystem(force=false){
    ...
    try {
      ...
      rememberHistoryR452([...serverHistory,...state.site.rows,...state.music.rows,...state.push.rows,...state.youtube.rows]);
      rebuildAll();
  ...
  readCaches();
  rememberHistoryR452([...state.youtube.rows,...state.site.rows]);
  rebuildAll();
  bindButtons();
  rememberLayerR382(active);
  applyLayer();
