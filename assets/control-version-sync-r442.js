/* ANDRIK Control R442 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R442__)return;
  window.__ANDRIK_VERSION_SYNC_R442__=true;
  const RELEASE='R442';
  const RUNTIME='55.00-r442';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R442 R441 BASE + ARCHIVE IN-PLACE LOAD + CENTER CALENDAR + GLOBE BALANCE';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:442,version:'55.00'}}));
    }finally{busy=false;}
  };
