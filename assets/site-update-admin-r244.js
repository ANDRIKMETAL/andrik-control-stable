(() => {
  'use strict';
  const SITE_UPDATE_UI_VERSION='55.00-r244';
  const KEY_SESSION='andrik-comments-admin-key',KEY_LOCAL='andrik-comments-admin-key-persistent',AUTO_RECOVERY_KEY='andrik-site-update-auto-recovery',CACHE_REFRESH_PREFIX='andrik-site-update-cache-refresh:',PENDING_DEPLOY_KEY='andrik-site-update-pending-deploy-r244';
  ...
  let statusRequestId=0,statusBusy=false;
  ...
  async function loadStatus(){
    ...
    setText('siteUpdateReleaseRepoText','Быстрая проверка · максимум 9 секунд');
    ...
    try{
      ...
      byId('siteUpdateBackupZipCreate')?.removeAttribute('disabled');
      if(selectedFile()&&hasOwnerAccess())previewButton.disabled=operation;
    }
  }
  ...
  async function fetchBackupZip(){
    ...
    try{
      ...
      let response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r243`,{credentials:'include',cache:'no-store',headers,signal:controller.signal});
      if(response.status===401&&raw){
        await window.AndrikOwnerSession?.establish?.(raw).catch(()=>null);
        response=await fetch(`/api/control/site-update/backup-zip?fresh=${Date.now()}&v=r243-retry`,{credentials:'include',cache:'no-store',headers:{accept:'application/zip',authorization:`Bearer ${raw}`},signal:controller.signal});
      }
      if(!response.ok){
  ...
  window.AndrikSiteUpdateR244={preview,publish,start:startPublishFromGesture,resume:resumePendingDeploy,checkDeploy:checkDeployNow,createZipBackup:createDownloadableZipBackup,version:SITE_UPDATE_UI_VERSION};window.AndrikSiteUpdateR243=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR242=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR241=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR240=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR239=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR238=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR237=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR232=window.AndrikSiteUpdateR244;window.AndrikSiteUpdateR195=window.AndrikSiteUpdateR244;byId('siteUpdateHistoryRefresh').addEventListener('click',()=>Promise.allSettled([loadStatus(),loadHistory(),loadLog()]));byId('siteUpdateCheckDeploy').addEventListener('click',()=>checkDeployNow());keyInput.addEventListener('input',()=>{captureKey();previewButton.disabled=operation||!selectedFile()||!hasOwnerAccess()});keyInput.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();loadStatus()}});resetStages();
  ...
  const emergencyUnlock=()=>{
    archiveInput.disabled=false;
    byId('siteUpdateBackupNow').disabled=operation;
    byId('siteUpdateBackupZipCreate')?.removeAttribute('disabled');
    const state=byId('siteUpdateState');
    if(state&&/Проверяем/i.test(state.textContent||'')){
      setState(true,'Резервный режим');
      setText('siteUpdateRepoText','Проверка GitHub не блокирует загрузчик');
      setText('siteUpdateReleaseRepoText','Выбор ZIP и backup доступны');
      setText('siteUpdateConnectionMessage','GitHub можно проверить повторно кнопкой «Проверить». Страница больше не перезагружается автоматически.');
    }
    if(selectedFile()&&hasOwnerAccess())previewButton.disabled=operation;
  };
  setTimeout(emergencyUnlock,9500);
  window.addEventListener('pageshow',()=>setTimeout(emergencyUnlock,600),{passive:true});
  window.addEventListener('error',event=>{
    if(/site-update/i.test(String(event?.filename||''))){
      setTimeout(emergencyUnlock,0);
      setText('siteUpdateUploadMessage','Локальная ошибка интерфейса перехвачена. Выбор ZIP остаётся доступным.');
    }
  });
