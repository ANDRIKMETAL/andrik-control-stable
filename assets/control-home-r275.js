/* Control ANDRIK R280 — exact completed-period daily summary opened from push. */
(() => {
  ...
  const shell=$('controlSwipeShell');
    ...
    try{
      const query=new URLSearchParams({v:'55.00-r280'});
  ...
  $('controlSummaryNext')?.addEventListener('click',()=>{
    ...
    location.assign(`/control-home.html?page=activity${pushQuery}&v=55.00-r280&t=${Date.now()}`);
  });
