/* ANDRIK Control R410 — authoritative display-version synchronization. */
(()=>{
  'use strict';
  if(window.__ANDRIK_VERSION_SYNC_R410__)return;
  window.__ANDRIK_VERSION_SYNC_R410__=true;
  const RELEASE='R410';
  const RUNTIME='55.00-r410';
  let busy=false;
  const apply=()=>{
    ...
    try{
      ...
      window.ANDRIK_CONTROL_BUILD='R410 VERSION SYNC + R409 CRON SAFE + SUMMARY LAYOUT';
      try{localStorage.setItem('andrik-control-display-version',RUNTIME)}catch(_){ }
      window.dispatchEvent(new CustomEvent('andrik-control-version-ready',{detail:{short:RELEASE,number:410,version:'55.00'}}));
    }finally{busy=false;}
  };
