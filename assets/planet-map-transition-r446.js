/* ANDRIK R446 — safe cross-page portal between Admin globe and listener world map. */
(()=>{
  'use strict';
  if(window.__ANDRIK_PLANET_MAP_TRANSITION_R446__)return;
  window.__ANDRIK_PLANET_MAP_TRANSITION_R446__=true;

  const K_RECT='andrik-portal-globe-rect-r446';
  const K_OPEN='andrik-portal-map-open-r446';
  const K_RETURN='andrik-portal-return-r446';
  ...
  const portalRoot=()=>{
    const root=document.createElement('div');root.className='andrik-map-portal-r446';root.setAttribute('aria-hidden','true');
    const back=document.createElement('div');back.className='andrik-map-portal-backdrop-r446';
    const orb=document.createElement('div');orb.className='andrik-map-portal-orb-r446';
    const map=document.createElement('div');map.className='andrik-map-portal-map-r446';
    const flare=document.createElement('div');flare.className='andrik-map-portal-flare-r446';
  ...
  async function openFromGlobe(globe,href){
    if(window.__ANDRIK_PORTAL_NAVIGATING_R446__)return;
    window.__ANDRIK_PORTAL_NAVIGATING_R446__=true;
    ...
    const p=portalRoot();
    const clone=cleanClone(sphere);if(clone){clone.classList.add('andrik-map-portal-source-r446');p.orb.insertBefore(clone,p.map)}
    orbAt(p.orb,rect.left+rect.width/2,rect.top+rect.height/2,Math.max(rect.width,rect.height));
    p.root.classList.add('is-opening');
    sphere.classList.add('andrik-portal-source-hidden-r446');
  ...
  async function arriveOnMap(){
    ...
    if(!fresh(stamp)){document.documentElement.classList.remove('andrik-portal-map-arrival-pending-r446');return}
    try{sessionStorage.removeItem(K_OPEN)}catch(_){ }
    if(reduced()){document.documentElement.classList.remove('andrik-portal-map-arrival-pending-r446');return}
    ...
    p.root.remove();document.documentElement.classList.remove('andrik-portal-map-arrival-pending-r446');
  }
  ...
  async function arriveOnAdmin(){
    ...
    if(!fresh(stamp)){document.documentElement.classList.remove('andrik-portal-return-pending-r446');return}
    ...
    if(!globe||!sphere){document.documentElement.classList.remove('andrik-portal-return-pending-r446');return}
    const rect=sphere.getBoundingClientRect();
    if(reduced()){document.documentElement.classList.remove('andrik-portal-return-pending-r446');sphere.style.opacity='';return}
    const p=portalRoot(),clone=cleanClone(sphere);if(clone){clone.classList.add('andrik-map-portal-source-r446');p.orb.insertBefore(clone,p.map)}
    orbAt(p.orb,rect.left+rect.width/2,rect.top+rect.height/2,Math.max(rect.width,rect.height));
    p.map.style.opacity='1';p.root.style.background='transparent';p.back.style.opacity='.28';sphere.classList.add('andrik-portal-source-hidden-r446');
    await Promise.all([
    ...
    sphere.classList.remove('andrik-portal-source-hidden-r446');p.root.remove();document.documentElement.classList.remove('andrik-portal-return-pending-r446');
  }

  function bindGlobe(){
    const globe=document.getElementById('adminHubGlobeR418');if(!globe||globe.dataset.portalR446==='1')return;
    globe.dataset.portalR446='1';
    globe.addEventListener('click',e=>{if(e.defaultPrevented||window.__ANDRIK_PORTAL_NAVIGATING_R446__)return;e.preventDefault();e.stopImmediatePropagation();const href=globe.href||'/analytics-admin.html?page=map&source=admin-globe&v=55.00-r446';openFromGlobe(globe,href)},true);
  }
