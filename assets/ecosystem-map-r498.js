/* ANDRIK R498 — ecosystem map with Site/YouTube/Instagram/Facebook/TikTok social layers. */
(() => {
  'use strict';
  if (window.__ANDRIK_ECOSYSTEM_MAP_R498__) return;
  window.__ANDRIK_ECOSYSTEM_MAP_R498__ = true;
  ...
  const state = {
    ...
    facebook:{rows:[],points:[],weekly:[],previous:[]},
    tiktok:{rows:[],points:[],weekly:[],previous:[]},
  ...
  const VALID_LAYERS_R382 = ['all','site','youtube','instagram','facebook','tiktok','music','push'];
  ...
  const HISTORY_STORAGE_KEY_R452 = 'andrik-ecosystem-country-history-r452';
  const HISTORY_STORAGE_KEY_R451 = 'andrik-ecosystem-country-history-r451';
  const HISTORY_STORAGE_KEY_R450 = 'andrik-ecosystem-country-history-r450';
  const historyCountryKeyR452 = row => String(row?.country || row?.name || row?.code || '').trim().toLowerCase();
  const readHistoryR452 = () => {
    try {
      const raw = JSON.parse(localStorage.getItem(HISTORY_STORAGE_KEY_R452) || localStorage.getItem(HISTORY_STORAGE_KEY_R451) || localStorage.getItem(HISTORY_STORAGE_KEY_R450) || '[]');
  ...
  const rememberHistoryR452 = rows => {
    const merged = new Map();
    for (const row of state.historyRows || []) {
      const key = historyCountryKeyR452(row);
    ...
    try {
      localStorage.setItem(HISTORY_STORAGE_KEY_R452, JSON.stringify(state.historyRows.map(row=>({country:row.country,code:row.code||'',lastSeen:row.lastSeen||''}))));
    } catch (_) {}
  };
  const allMarkerRowsR452 = () => {
    const merged = new Map();
    for (const row of state.historyRows || []) {
      const key = historyCountryKeyR452(row);
      if (key) merged.set(key,{...row,value:1,historyOnly:true});
  ...
  const rebuildAll = () => {
    ...
    const add = (name, rows) => {
      for (const row of cleanRows(rows)) {
        ...
        const item = breakdown.get(key) || {youtube:0,site:0,instagram:0,facebook:0,tiktok:0,music:0,push:0};
    ...
    add('instagram',state.instagram.rows);
    add('facebook',state.facebook.rows);
    add('tiktok',state.tiktok.rows);
    ...
    state.all.points = aggregatePoints([state.site.points,state.instagram.points,state.facebook.points,state.tiktok.points,state.music.points,state.push.points]);
    state.all.weekly = mergeWeekly([state.youtube.weekly,state.site.weekly,state.instagram.weekly,state.facebook.weekly,state.tiktok.weekly,state.music.weekly,state.push.weekly]);
    state.all.previous = mergeWeekly([state.youtube.previous,state.site.previous,state.instagram.previous,state.facebook.previous,state.tiktok.previous,state.music.previous,state.push.previous]);
  ...
  const layerMeta = layer => {
    ...
    const common = {
      all:{
        periodText:'Общая карта за всё время!',metricLabel:'сигналов',totalTitle:'Просмотры ANDRIK на YouTube за всё время',
      ...
      instagram:{
        periodText:'Instagram ANDRIK · география аудитории · 30 дней.',metricLabel:'аудит.',totalTitle:'Instagram · аудитория по странам',
        growthEyeline:'INSTAGRAM · 7 ДНЕЙ',growthTitle:'Instagram по странам',growthSubtitle:'Появится после доступных Meta Insights',
        monthlyEyeline:'INSTAGRAM',monthlyTitle:'Instagram',monthlyDescription:'География аудитории Instagram.',monthlyMetric:'аудитории',
        growthButtonTitle:'Instagram по странам',calendarTitle:'Instagram по месяцам'
      },
      facebook:{
        periodText:'Facebook ANDRIK · география Page Insights.',metricLabel:'аудит.',totalTitle:'Facebook · аудитория по странам',
        growthEyeline:'FACEBOOK',growthTitle:'Facebook по странам',growthSubtitle:'Включится после Meta OAuth',
        monthlyEyeline:'FACEBOOK',monthlyTitle:'Facebook',monthlyDescription:'География Facebook Page.',monthlyMetric:'аудитории',
        growthButtonTitle:'Facebook по странам',calendarTitle:'Facebook по месяцам'
      },
      tiktok:{
        periodText:'TikTok ANDRIK · география аудитории.',metricLabel:'аудит.',totalTitle:'TikTok · аудитория по странам',
        growthEyeline:'TIKTOK',growthTitle:'TikTok по странам',growthSubtitle:'Включится, когда официальный API отдаст географию',
        monthlyEyeline:'TIKTOK',monthlyTitle:'TikTok',monthlyDescription:'География TikTok, когда она доступна через API.',monthlyMetric:'аудитории',
        growthButtonTitle:'TikTok по странам',calendarTitle:'TikTok по месяцам'
      },
      music:{
  ...
  const decorateCountry = () => {
    ...
    if (active === 'all') {
      ...
      detail.innerHTML = `<b>Экосистема:</b> ▶ ${fmt(b.youtube)} · 🌐 ${fmt(b.site)} · ◎ ${fmt(b.instagram)} · f ${fmt(b.facebook)} · TT ${fmt(b.tiktok)}<span class="is-muted"> · ♫ ${fmt(b.music)} · 🔔 ${fmt(b.push)}${cityText}</span>`;
    } else {
  ...
  const applyLayer = (force=false) => {
    ...
    if (!rows.length && active !== 'youtube') {
      runtime.render([],{
        ...
        markerRows:active==='all'?allMarkerRowsR452():undefined,
        error:state.loaded?'Данных для этого слоя пока нет. Они начнут появляться после новых событий.':'Загружаем слой экосистемы…',
        ...meta
      });
    } else {
      runtime.render(rows,{source:`ecosystem-${active}`,layer:active,points:layer.points||[],livePoints:livePointsForLayer(active),markerRows:active==='all'?allMarkerRowsR452():undefined,...meta});
    }
    window.__andrikEcosystemActiveLayer = active;
    updateButtons();
  ...
  const ingestAudience = data => {
    ...
    rememberHistoryR452(state.youtube.rows);
    rebuildAll();
  ...
  async function loadEcosystem(force=false){
    ...
    try {
      ...
      state.instagram.previous = cleanWeekly(data?.instagram?.previousWeekCountries || []);
      state.facebook.rows = cleanRows(data?.facebook?.countries || []);
      state.facebook.points = Array.isArray(data?.facebook?.points)?data.facebook.points:[];
      state.facebook.weekly = cleanWeekly(data?.facebook?.weeklyCountries || []);
      state.facebook.previous = cleanWeekly(data?.facebook?.previousWeekCountries || []);
      state.tiktok.rows = cleanRows(data?.tiktok?.countries || []);
      state.tiktok.points = Array.isArray(data?.tiktok?.points)?data.tiktok.points:[];
      state.tiktok.weekly = cleanWeekly(data?.tiktok?.weeklyCountries || []);
      state.tiktok.previous = cleanWeekly(data?.tiktok?.previousWeekCountries || []);
      ...
      rememberHistoryR452([...serverHistory,...state.site.rows,...state.instagram.rows,...state.facebook.rows,...state.tiktok.rows,...state.music.rows,...state.push.rows,...state.youtube.rows]);
      rebuildAll();
  ...
  state.historyRows = readHistoryR452();
  readCaches();
  rememberHistoryR452([...state.youtube.rows,...state.site.rows,...state.instagram.rows,...state.facebook.rows,...state.tiktok.rows]);
  rebuildAll();
  bindButtons();
  rememberLayerR382(active);
  applyLayer();
