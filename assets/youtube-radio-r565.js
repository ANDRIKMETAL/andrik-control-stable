...
function renderRadio(data){
  ...
  text('youtubeRadioClipsR565',number(data?.clipCount||4));
  ...
  text('youtubeRadioNowMetaR565',[safe(cur.album),cur.type==='clip'?'КЛИП':'MP3 ИЗ R2'].filter(Boolean).join(' • ')||'R2 → YouTube');
