...
TMP_DIR="$(mktemp -d /tmp/andrik-r711.XXXXXX)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
ENGINE_DROPIN="$DROPIN_DIR/r711-engine.conf"
AGENT_TARGET=/usr/local/sbin/andrik-radio-web
AGENT_LIB=/usr/local/lib/andrik-radio-web-agent-r711.mjs
...
echo '[1/10] Загружаю R711 engine: R695 YouTube transport + anti-stall + full-fit + R2 delete…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r711-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-radio-web-agent-r711.mjs?v=55.00-r711-$(date +%s)" -o "$TMP_DIR/agent.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-visual-auto-r703.sh?v=55.00-r711-$(date +%s)" -o "$TMP_DIR/auto.sh"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/force-full-fit-r711.sh?v=55.00-r711-$(date +%s)" -o "$TMP_DIR/fullfit.sh"
...
grep -q 'R711-R695-YOUTUBE-TRANSPORT-FULLFIT-R2-DELETE' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R711'; exit 3; }
grep -q 'AUDIO_INPUT_QUEUE_PACKETS = 16' "$TMP_DIR/server.mjs" || { echo 'СТОП: потерян R707 bounded PCM queue'; exit 3; }
grep -q 'VIDEO_INPUT_QUEUE_PACKETS = 512' "$TMP_DIR/server.mjs" || { echo 'СТОП: нет R711 anti-stall video queue'; exit 3; }
grep -q "VIDEO_BITRATE = '4500k'" "$TMP_DIR/server.mjs" || { echo 'СТОП: нет проверенного R695 4.5 Mbps profile'; exit 3; }
grep -q 'OUTPUT_TIMESHIFT_SECONDS = 6' "$TMP_DIR/server.mjs" || { echo 'СТОП: нет проверенного R695 6s RTMPS cushion'; exit 3; }
...
grep -q "version:'R711'" "$TMP_DIR/agent.mjs" || { echo 'СТОП: agent не R711'; exit 3; }
...
cp -a "$SERVER" "$SERVER.bak-r711-$TS"
[ -s "$AGENT_TARGET" ] && cp -a "$AGENT_TARGET" "$AGENT_TARGET.bak-r711-$TS" || true
[ -s "$AUTO_TARGET" ] && cp -a "$AUTO_TARGET" "$AUTO_TARGET.bak-r711-$TS" || true

echo '[3/10] Ставлю R711 engine + настоящий .mjs agent…'
...
#!/usr/bin/env bash
exec /usr/bin/node /usr/local/lib/andrik-radio-web-agent-r711.mjs "$@"
WRAP
...
[Unit]
Description=ANDRIK Radio Web Control Agent R711
...
[Unit]
Description=ANDRIK four visual cycles scheduler R703/R711
...
echo '[6/10] Перезапускаю именно R711 agent и проверяю, что ESM реально стартует…'
...
systemctl is-active --quiet "$AGENT_SERVICE" || {
  echo 'СТОП: R711 agent не поднялся.'
...
echo '[9/10] Чистый restart сбрасывает старый PCM backlog и применяет R711…'
systemctl restart "$SERVICE"
sleep 7
"$AUTO_TARGET" force || true

echo '[10/10] Проверяю R711 + agent + MORNING + title sync…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  sleep 2
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R711-R695-YOUTUBE-TRANSPORT-FULLFIT-R2-DELETE'; then
    if printf '%s' "$STATUS" | python3 -c 'import sys,json; d=json.load(sys.stdin); raise SystemExit(0 if d.get("publisherRunning") and d.get("audioInputQueuePackets")==16 and d.get("videoInputQueuePackets")==512 and d.get("videoBitrate")=="4500k" and d.get("videoBuffer")=="9000k" and d.get("outputTimeshiftSeconds")==6 and d.get("equalizerEngine")=="R706-GEQ-FRAME-ANIMATED" else 1)' 2>/dev/null; then OK=1; break; fi
  fi
done
[ "$OK" = 1 ] || { echo 'R711 radio не стал healthy.'; printf '%s\n' "$STATUS" | python3 -m json.tool 2>/dev/null || true; journalctl -u "$SERVICE" -n 100 --no-pager || true; exit 5; }
...
if [ -s "$MORNING" ]; then
  ...
  echo '⚠️ MORNING master пока не появился локально — нажми «Назначить + сейчас» один раз; R711 agent уже исправлен.'
fi

echo
echo '============================================'
echo '✅ R711 ГОТОВ · YOUTUBE STABILITY + SAFE R2 DELETE'
echo '✅ YouTube ingest: proven R695 4.5 Mbps CBR + 9 Mbps VBV + 6 s network cushion'
echo '✅ Local MJPEG feeder: 512-frame anti-stall cushion; q=8 lightweight feeder'
...
echo '✅ AUTO FIT / NO CROP сохранён + аварийная кнопка FULL FRAME FIT'
echo '============================================'
