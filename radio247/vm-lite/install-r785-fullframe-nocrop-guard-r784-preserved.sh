...
STAMP="$(date +%s)"
TMP="$(mktemp -d /tmp/andrik-r785.XXXXXX)"
REMOTE="$TMP/server.mjs"
BACKUP="$SERVER.bak-before-r785-$(date +%Y%m%d-%H%M%S)"
trap 'rm -rf "$TMP"' EXIT

rollback(){
  echo '⚠️ R785 live-check не прошёл — возвращаю предыдущую рабочую версию.'
...
echo '[1/8] Download R785'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r785-$STAMP" -o "$REMOTE"

echo '[2/8] Verify no-crop full-frame guard + continuous rawvideo clock + station audio guard'
node --check "$REMOTE" >/dev/null
grep -Fq "R785-FULLFRAME-NOCROP-GUARD-R784-PRESERVED" "$REMOTE" || { echo 'СТОП: remote server не R785'; exit 3; }
...
grep -Fq "R785-LIVE-CLIP-FIT-PAD-FINAL" "$REMOTE" || { echo 'СТОП: нет финального FIT+PAD для клипов'; exit 3; }
grep -Fq "probePreparedGeometryR785" "$REMOTE" || { echo 'СТОП: нет проверки prepared 1920x1080'; exit 3; }
grep -Fq "fullFrameGuardMode: 'R785-FINAL-LIVE-FIT-PAD-1920x1080+SAR1'" "$REMOTE" || { echo 'СТОП: нет no-crop guard'; exit 3; }
grep -Fq "'-vf','setsar=1'" "$REMOTE" || { echo 'СТОП: master SAR=1 не зафиксирован'; exit 3; }
...
grep -Fq "CLIP_PREP_SUFFIX_R782 = '.r785-ready.mp4'" "$REMOTE" || { echo 'СТОП: нет принудительной свежей подготовки R785'; exit 3; }
...
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/assets/subscribe-right-r767.png?v=55.00-r785-$STAMP" -o "$CTA_REMOTE"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/assets/like-right-r783.png?v=55.00-r785-$STAMP" -o "$LIKE_REMOTE"
...
# R785 geometry preflight: a 4:3 source MUST be fully visible inside 16:9 with side padding,
# never zoomed/cropped. Then verify the exact raw-frame byte size expected by the R784 relay.
ffmpeg -nostdin -hide_banner -loglevel error \
  -f lavfi -i "testsrc2=size=640x480:rate=25" -t 0.2 \
  ...
  -c:v libx264 -preset ultrafast -pix_fmt yuv420p "$TMP/nocrop.mp4"
GEO="$(ffprobe -v error -select_streams v:0 -show_entries stream=width,height,sample_aspect_ratio,display_aspect_ratio -of csv=p=0 "$TMP/nocrop.mp4")"
echo "$GEO" | grep -Fq '1920,1080,1:1,16:9' || { echo "СТОП: no-crop geometry preflight: $GEO"; exit 3; }
...
if grep -Eqi 'DTS .*out of order|timestamp discontinuity|non[- ]monoton|Could not write header|Error opening output file|Tag .* incompatible with output codec|Invalid argument' "$TMP/ffmpeg-preflight.log"; then
  echo 'СТОП: R785 rawvideo/FIFO preflight не прошёл:'
...
ok=(
 d.get('version')=='R785-FULLFRAME-NOCROP-GUARD-R784-PRESERVED' and
 ...
 d.get('fullFrameGuardMode')=='R785-FINAL-LIVE-FIT-PAD-1920x1080+SAR1' and
 d.get('permanentFullscreenFitPolicy')=='R785-FINAL-LIVE-FIT-DECREASE-PAD-NO-CROP-SAR1' and
...
PY
then
  echo '❌ R785 status не подтвердился.'
...
printf '%s\n' "$STATUS" | python3 -c 'import sys,json;d=json.load(sys.stdin);print("VERSION:",d.get("version"));print("PUBLISHER:",d.get("publisherRunning"));print("TRANSPORT:",d.get("transportHealthy"));print("CLOCK:",d.get("masterTimestampMode"));print("FULL FRAME:",d.get("fullFrameGuardMode"));print("FIT POLICY:",d.get("permanentFullscreenFitPolicy"));print("TIMESTAMP ERRORS:",d.get("masterTimestampErrorCount"));print("AUDIO BYTES:",d.get("masterAudioBytesWritten"));print("VIDEO BYTES:",d.get("masterVideoBytesWritten"));print("STATION AUDIO:",d.get("stationPreparedAudioByKey"));print("ERROR:",d.get("lastError"))'
echo '✅ R785: клипы/вставки принудительно FIT+PAD 1920x1080 SAR1 — crop/zoom запрещён'
echo '✅ R785: prepared cache ffprobe-проверяется 1920x1080 + R784 frame relay пишет только целые YUV420P кадры'
echo '✅ R784 сохранён: все 5 заставок — лучший слышимый audio stream + RMS verify'
