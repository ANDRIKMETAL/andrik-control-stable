...
TMP_DIR="$(mktemp -d /tmp/andrik-r725.XXXXXX)"
trap 'rm -rf "$TMP_DIR"' EXIT

for c in curl node ffmpeg ffprobe python3 systemctl; do command -v "$c" >/dev/null || { echo "СТОП: $c не найден"; exit 2; }; done
[ -s "$SERVER" ] || { echo "СТОП: нет $SERVER"; exit 2; }

echo '[1/8] Загружаю R725: TITLE SYNC + BUMPERS + QR RIGHT + Я ЕСТЬ…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r725-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/assets/andrik-qr-r723.png?v=55.00-r725-$(date +%s)" -o "$TMP_DIR/andrik-qr-r723.png"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/assets/subscribe-like-r722.png?v=55.00-r725-$(date +%s)" -o "$TMP_DIR/subscribe-like-r722.png"
...
echo '[2/8] Проверяю R725 функции…'
grep -q 'R725-CTA-2MIN-BUMPERS-4-6-R724-PRESERVED' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R725'; exit 3; }
grep -q 'TITLE_HANDOFF_DELAY_MS_R724' "$TMP_DIR/server.mjs" || { echo 'СТОП: title sync отсутствует'; exit 3; }
grep -q 'BUMPER_MIN_SONGS_R724 = 4' "$TMP_DIR/server.mjs" || { echo 'СТОП: bumper min != 4'; exit 3; }
grep -q 'BUMPER_MAX_SONGS_R724 = 6' "$TMP_DIR/server.mjs" || { echo 'СТОП: bumper max != 6'; exit 3; }
...
grep -q 'CTA_PERIOD_SECONDS_R722 = 120' "$TMP_DIR/server.mjs" || { echo 'СТОП: SUBSCRIBE/LIKE period != 120s'; exit 3; }
...
TS="$(date +%Y%m%d-%H%M%S)"
echo '[4/8] Резервная копия текущей версии…'
cp -a "$SERVER" "$SERVER.bak-r725-$TS"
mkdir -p "$ASSET_DIR"
[ -s "$QR_TARGET" ] && cp -a "$QR_TARGET" "$QR_TARGET.bak-r725-$TS" || true

echo '[5/8] Ставлю R725…'
...
echo '[7/8] Проверяю LIVE/status R725…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | python3 -c 'import sys,json; d=json.load(sys.stdin); raise SystemExit(0 if d.get("version")=="R725-CTA-2MIN-BUMPERS-4-6-R724-PRESERVED" and d.get("publisherRunning") and d.get("clipBoundaryReconnect") is False and d.get("qrPosition")=="top-right" and d.get("titleHandoffDelayMs")==1800 and d.get("nextBumperAfterSongs") in (4,5,6) else 1)' 2>/dev/null; then OK=1; break; fi
  sleep 2
done
[ "$OK" = 1 ] || { echo 'R725 не подтвердил статус.'; printf '%s\n' "$STATUS" | python3 -m json.tool 2>/dev/null || true; journalctl -u "$SERVICE" -n 180 --no-pager || true; exit 5; }
RECENT="$(journalctl -u "$SERVICE" --since '-20 seconds' --no-pager 2>/dev/null || true)"
if printf '%s' "$RECENT" | grep -qi 'non-monoton'; then
  echo 'СТОП: найден Non-monotonic DTS после R725'
  printf '%s\n' "$RECENT" | tail -n 100
  exit 6
fi
