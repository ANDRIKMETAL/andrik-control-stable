...
function writeTicker(text){
  ...
  fs.mkdirSync('/var/cache/andrik-radio-r622',{recursive:true});
  const tmp=TICKER_FILE+'.tmp';fs.writeFileSync(tmp,value,'utf8');fs.renameSync(tmp,TICKER_FILE);
  return value;
}
...
async function pair(code){
  ...
  writeConfig({token:d.token,pairedAt:new Date().toISOString(),base:BASE,version:'R629'});
  console.log('ГОТОВО ✅ AWS привязан к веб-контрольке.');
...
async function daemon(){
  ...
  console.log('ANDRIK Radio Web Agent R629 started');
  while(true){
    try{
      const status=await localStatus();
      const d=await jsonFetch(BASE+'/api/radio-agent-r627/poll',{method:'POST',headers,body:JSON.stringify({version:'R629',status})});
      if(d.ticker && typeof d.ticker.text==='string' && clean(d.ticker.text)!==clean(status.ticker)){
        const live=writeTicker(d.ticker.text);
        console.log(new Date().toISOString(),'ticker updated:',live.slice(0,90));
      }
      if(d.command){
