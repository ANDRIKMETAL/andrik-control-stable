...
TMP_SERVER="$(mktemp /tmp/andrik-r752.XXXXXX.mjs)"
BACKUP_SERVER="${SERVER}.bak-before-r752-$(date +%Y%m%d-%H%M%S)"
ENV_BACKUP="${ENV_FILE}.bak-before-r752-$(date +%Y%m%d-%H%M%S)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
DROPIN="$DROPIN_DIR/r752-boundary-lock.conf"
...
echo '[1/10] Скачиваю R752 server…'
STAMP="$(date +%s)"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r752-$STAMP" -o "$TMP_SERVER"

echo '[2/10] Проверяю R752 и сохранность R751/R750/R749/R748/R746…'
node --check "$TMP_SERVER" >/dev/null
grep -Fq "R752-BOUNDARY-LOCKED-CLIP-AV-FADE-R751-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: скачался не R752'; exit 3; }
...
rollback(){
  echo '⚠️ R752 не прошёл запуск — возвращаю предыдущую версию…'
...
echo '[5/10] Устанавливаю R752 и безопасные пороги…'
...
echo '[8/10] Проверяю живой R752 status…'
STATUS=''; OK=0
...
ok=(
 d.get('version')=='R752-BOUNDARY-LOCKED-CLIP-AV-FADE-R751-PRESERVED' and
 ...
 d.get('clipAvSyncMode')=='R752-CACHE-WARM-NO-MEDIA+BOTH-OUTPUTS-READY+ATOMIC-BOUNDARY' and
 d.get('insertUnhandledRejectionGuard')=='R752-UNIFIED-AV-EXIT-CATCH+R751-GUARD' and
...
if [ "$OK" != 1 ]; then
  echo '❌ R752 status не подтвердился.'
...
printf '%s\n' "$STATUS" | python3 -c 'import sys,json;d=json.load(sys.stdin);print("VERSION:",d.get("version"));print("PUBLISHER:",d.get("publisherRunning"),"healthy=",d.get("transportHealthy"));print("VIDEO:",d.get("videoFeederRunning"));print("AUDIO:",d.get("producerRunning"));print("CLIP SYNC:",d.get("clipAvSyncMode"));print("CACHE WARM:",d.get("clipCacheWarmLeadSeconds"),"sec, live preroll=",d.get("videoPipelineLeadSeconds"));print("BACKPRESSURE:",d.get("masterBackpressureDetection"),d.get("masterBackpressureWatchdogMs"),"ms");print("FADE LEAD:",d.get("videoFadeLeadSeconds"));print("ERROR:",d.get("lastError"));print("WARNING:",d.get("lastWarning"))'

echo
echo '✅ R752 ГОТОВ — CLIP A/V BOUNDARY LOCK'
echo '✅ клип/заставка НЕ МОЖЕТ появиться до конца текущей песни'
echo '✅ за 5с прогревается только локальный кэш/метаданные — НОЛЬ кадров в LIVE'
echo '✅ на границе один FFmpeg подключает видео+звук вместе'
echo '✅ затемнение R751 остаётся видимым до реальной границы'
echo '✅ SPECIAL 30/60, заставки и обычные клипы используют один безопасный механизм'
