...
CANON_DROPIN="${CANON_DROPIN_DIR}/r803d-single-agent.conf"
DUP_DROPIN_DIR="/etc/systemd/system/${DUP_UNIT}.d"
DUP_DROPIN="${DUP_DROPIN_DIR}/r803d-duplicate-disabled.conf"
DUP_ALLOW_MARKER="/run/andrik-radio-r650-duplicate-allowed"
TMP_AGENT="$(mktemp --suffix=.mjs)"
STAMP="$(date +%Y%m%d-%H%M%S)"
BACKUP="${CANON_TARGET}.bak-before-r803d-${STAMP}"
...
rollback(){
  set +e
  echo "=== R803D ROLLBACK WEB AGENTS ONLY ===" >&2
...
if [[ -n "$RADIO_PID_BEFORE" && "$RADIO_PID_BEFORE" != "0" && "$RADIO_PID_AFTER" != "$RADIO_PID_BEFORE" ]]; then
  echo "WARNING: radio MainPID changed independently; R803D never calls restart/stop/start on andrik-radio.service" >&2
fi
...
HTTP="$(curl -sS -o /tmp/r803d-public.$$ -w '%{http_code}' --max-time 8 https://andrikmetal.com/api/public/radio-diagnostics-r803 2>/dev/null || true)"
echo "HTTP ${HTTP:-unavailable}"
if [[ "$HTTP" == "200" ]]; then head -c 2200 /tmp/r803d-public.$$; echo; fi
rm -f /tmp/r803d-public.$$
