...
TMP_SERVER="$(mktemp /tmp/andrik-r758.XXXXXX.mjs)"
BACKUP_SERVER="${SERVER}.bak-before-r758-$(date +%Y%m%d-%H%M%S)"
ENV_BACKUP="${ENV_FILE}.bak-before-r758-$(date +%Y%m%d-%H%M%S)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
DROPIN="$DROPIN_DIR/r758-guaranteed-mp3-boundary-fade.conf"
...
echo '[1/10] Скачиваю R758 server…'
STAMP="$(date +%s)"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r758-$STAMP" -o "$TMP_SERVER"

echo '[2/10] Проверяю R758 и сохранность R757/R756…'
node --check "$TMP_SERVER" >/dev/null
grep -Fq "R758-GUARANTEED-MP3-BOUNDARY-FADE-R757-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: скачался не R758'; exit 3; }
grep -Fq "R757-END-BLACK-HOLD-THEN-VIDEO-FADE-IN" "$TMP_SERVER" || { echo 'СТОП: R757 MP3→clip fade отсутствует'; exit 3; }
grep -Fq "R758-OLD-MP3-FADE-TO-BLACK-HOLD-NEW-MP3-FADE-IN-0.30" "$TMP_SERVER" || { echo 'СТОП: R758 MP3→MP3 fade mode отсутствует'; exit 3; }
grep -Fq "const endFadeToBlackR758=Boolean(actualNextR736)" "$TMP_SERVER" || { echo 'СТОП: R758 boundary black hold отсутствует'; exit 3; }
grep -Fq "const mp3ToMp3BoundaryR758=previousWasTrackR758" "$TMP_SERVER" || { echo 'СТОП: R758 MP3→MP3 detector отсутствует'; exit 3; }
...
echo '[3/10] Проверяю FFmpeg drawtext/fade + boundary black…'
PREV="$(mktemp /tmp/r758-prev.XXXXXX.txt)"; NEXT="$(mktemp /tmp/r758-next.XXXXXX.txt)"
...
rollback(){
  echo '⚠️ R758 не прошёл запуск — возвращаю предыдущую версию…'
...
echo '[5/10] Устанавливаю R758: гарантированное MP3→MP3 затемнение, остальное без изменений…'
...
wanted={
 ...
 'MP3_BOUNDARY_FADE_IN_SECONDS_R758':'0.30',
}
...
echo '[8/10] Проверяю R758 status…'
STATUS=''; OK=0
...
ok=(
 d.get('version')=='R758-GUARANTEED-MP3-BOUNDARY-FADE-R757-PRESERVED' and
 ...
 d.get('mp3BoundaryFadeMode')=='R758-OLD-MP3-FADE-TO-BLACK-HOLD-NEW-MP3-FADE-IN-0.30' and
...
if [ "$OK" != 1 ]; then
  echo '❌ R758 status не подтвердился.'
...
if journalctl -u "$SERVICE" --since '-20 sec' --no-pager | grep -Eq 'Main process exited.*status=(75|76)|R751 STREAM STALL'; then
  echo '❌ После R758 обнаружен transport restart.'
  rollback
  exit 6
fi
...
printf '%s\n' "$STATUS" | python3 -c 'import sys,json;d=json.load(sys.stdin);print("VERSION:",d.get("version"));print("PUBLISHER:",d.get("publisherRunning"),"healthy=",d.get("transportHealthy"));print("VIDEO:",d.get("videoFeederRunning"));print("MP3 FADE:",d.get("mp3BoundaryFadeMode"),"in=",d.get("mp3BoundaryFadeInSeconds"));print("VIDEO FADE:",d.get("mp3ToVideoFadeMode"));print("CLIP PREVIEW:",d.get("clipPreviewMode"));print("MASTER:",d.get("masterVideoMode"));print("ERROR:",d.get("lastError"));print("WARNING:",d.get("lastWarning"))'

echo
echo '✅ R758 ГОТОВ — гарантированное MP3→MP3 затемнение'
echo '✅ старый MP3 уходит в чёрный, новый MP3 проявляется за 0.30с'
echo '✅ PREVIOUS/NEXT, названия и тайминги R757/R756 не изменены'
