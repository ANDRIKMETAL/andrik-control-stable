...
async function downloadVisualR650(slot,headers,{force=false}={}){
  ...
  const r=await fetch(`${BASE}/api/radio-agent-r650/visual?slot=${encodeURIComponent(slot)}&download=1`,{headers:{authorization:headers.authorization,'user-agent':'ANDRIK-Radio-Web-Agent-R715'},signal:AbortSignal.timeout(180000)});
  if(!r.ok){
...
async function execute(action,command={},headers={}){
  ...
  if(action==='full-fit'){
    const helper='/usr/local/sbin/andrik-radio-force-fullfit';
    const r=fs.existsSync(helper)
      ? await runAsync(helper,[],90000)
      : await runAsync('systemctl',['restart','andrik-radio.service'],90000);
    return {ok:r.ok,output:`FULL FRAME FIT / NO CROP ${r.ok?'✅':'❌'}\n${r.output}`};
  }
  if(action==='status'){
...
async function daemon(){
  while(true){
    try{
      ...
      const headers={'content-type':'application/json','authorization':'Bearer '+cfg.token,'user-agent':'ANDRIK-Radio-Web-Agent-R715'};
      const status=await localStatus();
      // Heartbeat always continues, even while start/recover is running.
      const d=await jsonFetch(BASE+'/api/radio-agent-r627/poll',{method:'POST',headers,body:JSON.stringify({version:'R715',status})});
      if(d.ticker && typeof d.ticker.text==='string' && clean(d.ticker.text)!==clean(status.ticker))writeTicker(d.ticker.text);
      await maybeEnsureYoutubeLiveR715(headers,status);
      if(d.command && !busy){
...
async function main(){
  ...
  console.log('ANDRIK Radio Web Agent R715 · commands: daemon | status | bootstrap-visuals | visual-sync | visual-now <morning|day|evening|night> | visual-auto');
}
