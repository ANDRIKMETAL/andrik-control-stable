...
TMP_SERVER="$(mktemp /tmp/andrik-r751.XXXXXX.mjs)"
BACKUP_SERVER="${SERVER}.bak-before-r751-$(date +%Y%m%d-%H%M%S)"
ENV_BACKUP="${ENV_FILE}.bak-before-r751-$(date +%Y%m%d-%H%M%S)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
DROPIN="$DROPIN_DIR/r751-crash-proof.conf"
...
echo '[1/10] Скачиваю R751 server…'
STAMP="$(date +%s)"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r751-$STAMP" -o "$TMP_SERVER"

echo '[2/10] Проверяю R751 и сохранность R750/R749/R748/R746…'
node --check "$TMP_SERVER" >/dev/null
grep -Fq "R751-CRASH-PROOF-INSERT-PROGRESS-GUARD-FADE-LATE-R750-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: скачался не R751'; exit 3; }
grep -Fq "clipExitPromise.catch(()=>{})" "$TMP_SERVER" || { echo 'СТОП: crash-proof guard вставок отсутствует'; exit 3; }
grep -Fq "R751 master pipe NO-PROGRESS" "$TMP_SERVER" || { echo 'СТОП: progress-aware watchdog отсутствует'; exit 3; }
grep -Fq "VIDEO_FADE_LEAD_SECONDS_R735 = 0.40" "$TMP_SERVER" || { echo 'СТОП: затемнение +2 секунды не применено'; exit 3; }
...
rollback(){
  echo '⚠️ R751 не прошёл запуск — возвращаю предыдущую версию…'
...
echo '[5/10] Устанавливаю R751 и безопасные пороги…'
...
echo '[8/10] Проверяю живой R751 status…'
STATUS=''; OK=0
...
ok=(
 d.get('version')=='R751-CRASH-PROOF-INSERT-PROGRESS-GUARD-FADE-LATE-R750-PRESERVED' and
 ...
 d.get('insertUnhandledRejectionGuard')=='R751-IMMEDIATE-CLIP-EXIT-CATCH' and
 abs(float(d.get('videoFadeLeadSeconds') or 0)-0.40)<0.01 and
 int(d.get('insertAudioStartTimeoutMs') or 0)==4000
)
...
if [ "$OK" != 1 ]; then
  echo '❌ R751 status не подтвердился.'
...
printf '%s\n' "$STATUS" | python3 -c 'import sys,json;d=json.load(sys.stdin);print("VERSION:",d.get("version"));print("PUBLISHER:",d.get("publisherRunning"),"healthy=",d.get("transportHealthy"));print("VIDEO:",d.get("videoFeederRunning"));print("AUDIO:",d.get("producerRunning"));print("INSERT GUARD:",d.get("insertUnhandledRejectionGuard"),"audio timeout=",d.get("insertAudioStartTimeoutMs"));print("BACKPRESSURE:",d.get("masterBackpressureDetection"),d.get("masterBackpressureWatchdogMs"),"ms");print("FADE LEAD:",d.get("videoFadeLeadSeconds"));print("ERROR:",d.get("lastError"));print("WARNING:",d.get("lastWarning"))'

echo
echo '========================================================'
echo '✅ R751 ГОТОВ — CRASH-PROOF INSERT + PROGRESS GUARD'
echo '✅ пойманный SPECIAL 30 audio-timeout больше НЕ может уронить Node'
echo '✅ неудачная вставка безопасно пропускается, эфир продолжает MP3'
echo '✅ старт аудио вставки: до 4с вместо слишком жёстких 1.5с'
echo '✅ обычный writableNeedDrain больше НЕ считается падением'
echo '✅ restart только при 30с реального нулевого прогресса pipe'
echo '✅ затемнение начинается ровно на 2с позже'
echo '✅ R750/R749/R748/R746 сохранены'
echo '========================================================'
