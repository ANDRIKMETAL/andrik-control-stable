...
STAMP="$(date +%s)"
TMP="$(mktemp -d /tmp/andrik-r778.XXXXXX)"
REMOTE="$TMP/server.mjs"
BACKUP="$SERVER.bak-before-r778-$(date +%Y%m%d-%H%M%S)"
trap 'rm -rf "$TMP"' EXIT

rollback(){
  echo '⚠️ R778 live-check не прошёл — возвращаю предыдущую рабочую версию.'
...
for c in curl node python3 ffmpeg ffprobe systemctl journalctl grep stat install cp; do
  command -v "$c" >/dev/null || { echo "СТОП: $c не найден"; exit 2; }
done
[ -s "$SERVER" ] || { echo "СТОП: нет $SERVER"; exit 2; }

echo '[1/7] Download R778'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r778-$STAMP" -o "$REMOTE"

echo '[2/7] Verify MPEG-TS timestamp bridge before restart'
node --check "$REMOTE" >/dev/null
grep -Fq "R778-MPEGTS-TIMESTAMP-BRIDGE-R777-PRESERVED" "$REMOTE" || { echo 'СТОП: remote server не R778'; exit 3; }
grep -Fq "R778-MPEGTS-PTS-DTS-BRIDGE-NO-SETTS-NO-SECOND-ENCODE" "$REMOTE" || { echo 'СТОП: нет R778 timestamp marker'; exit 3; }
...
# Test the exact compatibility primitive on THIS VPS FFmpeg: two independent x264
# feeder segments, each restarting timestamps at zero, are concatenated into one
# persistent MPEG-TS demuxer and copied to FLV without a second video encode.
for n in 1 2; do
...
cat "$TMP/seg1.ts" "$TMP/seg2.ts" | \
  ffmpeg -hide_banner -loglevel warning -fflags +genpts+discardcorrupt -f mpegts -i pipe:0 \
...
ok=(d.get('version')=='R778-MPEGTS-TIMESTAMP-BRIDGE-R777-PRESERVED' and d.get('publisherRunning') is True and d.get('transportHealthy') is True and str(d.get('masterTimestampMode','')).startswith('R778-MPEGTS-PTS-DTS-BRIDGE'))
raise SystemExit(0 if ok else 1)
PY
then
  echo '❌ R778 status не подтвердился.'
...
echo '✅ single x264 encode preserved; MPEG-TS only supplies PTS/DTS to persistent master'
echo '✅ no setts BSF and no timestamp-less raw-H264 FLV packets'
