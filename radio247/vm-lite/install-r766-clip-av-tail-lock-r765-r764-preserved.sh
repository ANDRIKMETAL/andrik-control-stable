...
TMP_SERVER="$(mktemp /tmp/andrik-r766.XXXXXX.mjs)"
BACKUP_SERVER="${SERVER}.bak-before-r766-$(date +%Y%m%d-%H%M%S)"
ENV_BACKUP="${ENV_FILE}.bak-before-r766-$(date +%Y%m%d-%H%M%S)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
DROPIN="$DROPIN_DIR/r766-stable.conf"
...
echo '[1/9] Скачиваю R766…'
STAMP="$(date +%s)"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r766-$STAMP" -o "$TMP_SERVER"

echo '[2/9] Проверяю: R766 A/V tail-lock + R765 push + R764/R763 сохранены…'
node --check "$TMP_SERVER" >/dev/null
grep -Fq "R766-PER-OUTPUT-T+VIDEO-TPAD-TRIM+AUDIO-APAD-ATRIM" "$TMP_SERVER" || { echo 'СТОП: R766 A/V tail lock marker отсутствует'; exit 3; }
grep -Fq "tpad=stop_mode=clone:stop_duration=" "$TMP_SERVER" || { echo 'СТОП: video tail pad отсутствует'; exit 3; }
grep -Fq "apad=pad_dur=" "$TMP_SERVER" || { echo 'СТОП: audio tail pad отсутствует'; exit 3; }
grep -Fq "R766-CLIP-AV-TAIL-LOCK-R765-PUSH-R764-R763-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: скачался не R766'; exit 3; }
...
rollback(){
  echo '⚠️ R766 не прошёл запуск — возвращаю предыдущий server…'
...
echo '[4/9] Устанавливаю R766…'
install -m 0644 "$TMP_SERVER" "$SERVER"
chmod 600 "$ENV_FILE"

echo '[5/9] Старые prepared-клипы не удаляю: R766 защищает их LIVE tail-lock на чтении.'
...
ok=(
 d.get('version')=='R766-CLIP-AV-TAIL-LOCK-R765-PUSH-R764-R763-PRESERVED' and
...
if [ "$OK" != 1 ]; then
  echo '❌ R766 status не подтвердился.'
...
echo
echo '✅ R766 ГОТОВ — VIDEO/AUDIO TAIL LOCK'
...
echo '✅ R766: видео вставки теперь держат последний кадр до конца их аудио'
echo '✅ R766: одинаковый -t стоит отдельно на H264 и PCM outputs'
