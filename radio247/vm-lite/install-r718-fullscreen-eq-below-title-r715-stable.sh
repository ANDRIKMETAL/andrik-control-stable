...
TMP_DIR="$(mktemp -d /tmp/andrik-r718.XXXXXX)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
ENGINE_DROPIN="$DROPIN_DIR/r718-engine.conf"
...
echo '[1/11] Загружаю R718: FULLSCREEN + EQ ПОД НАЗВАНИЕМ + стабильный transport R715…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r718-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-radio-web-agent-r715.mjs?v=55.00-r718-$(date +%s)" -o "$TMP_DIR/agent.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-visual-auto-r703.sh?v=55.00-r718-$(date +%s)" -o "$TMP_DIR/auto.sh"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/force-full-fit-r712.sh?v=55.00-r718-$(date +%s)" -o "$TMP_DIR/fullfit.sh"

for p in morning day evening night; do
  curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
    "$SITE_BASE/assets/equalizer-${p}-r717.mov?v=55.00-r718-$(date +%s)" -o "$TMP_DIR/equalizer-${p}.mov"
done
...
grep -q 'R718-FULLSCREEN-INSET-FIX-EQ-BELOW-TITLE-R715-STABLE' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R718'; exit 3; }
...
grep -q "R718-R706-PRERENDERED-QTRLE-4-SLOT-DIRECT" "$TMP_DIR/server.mjs" || { echo 'СТОП: R718 EQ engine отсутствует'; exit 3; }
...
grep -q "const insetCrop=await detectInsetBlackFrameCrop(clipPath).catch(()=> ''); // R718 SAFE BLACK-INSET ONLY" "$TMP_DIR/server.mjs" || { echo 'СТОП: safe inset crop для клипов не закреплён'; exit 3; }
...
cp -a "$SERVER" "$SERVER.bak-r718-$TS"
[ -s "$AGENT_TARGET" ] && cp -a "$AGENT_TARGET" "$AGENT_TARGET.bak-r718-$TS" || true
...
[Unit]
Description=ANDRIK four visual cycles scheduler R703/R718
...
echo '[10/11] Проверяю R718 + стабильный YouTube transport…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  sleep 2
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R718-FULLSCREEN-INSET-FIX-EQ-BELOW-TITLE-R715-STABLE'; then
    if printf '%s' "$STATUS" | python3 -c 'import sys,json; d=json.load(sys.stdin); raise SystemExit(0 if d.get("publisherRunning") and d.get("videoBitrate")=="4500k" and d.get("outputTimeshiftSeconds")==6 and d.get("engine")=="R678-R695-DIRECT-FFMPEG" and d.get("equalizerEngine")=="R718-R706-PRERENDERED-QTRLE-4-SLOT-DIRECT" else 1)' 2>/dev/null; then OK=1; break; fi
  fi
done
[ "$OK" = 1 ] || {
  echo 'R718 radio не поднялся как stable DIRECT.'
