...
async function autoSafe(){
  ...
  // R633: never wait on an old created/unknown broadcast again.
  // Stop the encoder first, then create a brand-new broadcast bound to the exact reusable stream key.
  if(serviceActive()){
    console.log('Останавливаю encoder на несколько секунд для чистого старта…');
    systemctl('stop',RADIO_SERVICE);
    const target=await chooseRadioStream(token).catch(()=>null);
    if(target)await waitSpecificStream(token,target.id,'inactive',30000).catch(()=>sleep(5000));
    else await sleep(5000);
  }

  console.log('R633: создаю новый чистый broadcast вместо зависшего created/unknown…');
  ...
  if(!streamId)throw new Error('R633: у нового broadcast нет boundStreamId.');
...
async function recover(){
  ...
  console.log('R633 recovery: старый created/unknown не используем → выполняю чистый старт.');
  return autoSafe();
}
...
async function main(){
  ...
  console.log('ANDRIK YouTube Console R633');
