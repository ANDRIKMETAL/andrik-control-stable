...
need_youtube(){ command -v andrik-youtube >/dev/null 2>&1 || { echo "Нет andrik-youtube. Выполни installer R629."; return 1; }; }
launch(){ need_youtube || return; echo; echo "🔴 ЗАПУСК СТРИМА БЕЗ YOUTUBE STUDIO · SAFE AUTO-START"; line; andrik-youtube auto-safe; }
recover(){ need_youtube || return; echo; echo "🛠 ВОССТАНОВЛЕНИЕ ЭФИРА"; line; andrik-youtube recover; }

while true; do
  clear 2>/dev/null || true
  echo "ANDRIK RADIO CONTROL · R629"
  line
  ...
  case "$choice" in
    ...
    8) masters; pause ;;
    9) errors; pause ;;
    10) andrik-radio-web status 2>/dev/null || echo "Web agent не установлен"; pause ;;
    0) exit 0 ;;
