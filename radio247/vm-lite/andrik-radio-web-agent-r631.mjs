...
const CONFIG_CANDIDATES=[CONFIG,'/etc/andrik-radio-web.json','/etc/andrik-radio-web-r629.json','/etc/andrik-radio-web-r630.json'];
...
function migrateConfig(){
  ...
  for(const candidate of CONFIG_CANDIDATES.slice(1)){
    try{
      const d=JSON.parse(fs.readFileSync(candidate,'utf8'));
      if(clean(d?.token)){
        fs.copyFileSync(candidate,CONFIG);fs.chmodSync(CONFIG,0o600);return;
      }
    }catch(_){}
  }
}
...
function execute(action){
  ...
  if(action==='status'){const y=run('/usr/local/sbin/andrik-youtube',['status'],60000);const s=run('systemctl',['is-active','andrik-radio.service'],10000);return {ok:y.ok,output:`RADIO SERVICE: ${s.output}\n\n${y.output}`};}
...
async function daemon(){
  while(true){
    try{
      ...
      const headers={'content-type':'application/json','authorization':'Bearer '+cfg.token,'user-agent':'ANDRIK-Radio-Web-Agent-R631'};
      const status=await localStatus();
      const d=await jsonFetch(BASE+'/api/radio-agent-r627/poll',{method:'POST',headers,body:JSON.stringify({version:'R631',status})});
      if(d.ticker && typeof d.ticker.text==='string' && clean(d.ticker.text)!==clean(status.ticker))writeTicker(d.ticker.text);
      if(d.command){const {id,action}=d.command;const result=execute(action);const after=await localStatus();await jsonFetch(BASE+'/api/radio-agent-r627/result',{method:'POST',headers,body:JSON.stringify({id,ok:result.ok,output:result.output,status:after})});}
    }catch(e){console.error(new Date().toISOString(),'agent:',e.message||e)}
    await sleep(4000);
  }
}
async function main(){const cmd=clean(process.argv[2]||'status').toLowerCase();if(cmd==='pair')return pair(process.argv[3]);if(cmd==='daemon')return daemon();if(cmd==='status'){const cfg=readConfig();console.log(cfg.token?'PAIRED ✅':'NOT PAIRED ❌');console.log(await localStatus());return}console.log('ANDRIK Radio Web Agent R631 · commands: daemon | status');}
