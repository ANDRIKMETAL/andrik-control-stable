...
BACKUP="${LIVE}.bak-before-r818-${STAMP}"
AGENT_BACKUP="${AGENT}.bak-before-r818-${STAMP}"
TMP_SERVER="$(mktemp --suffix=.mjs)"
TMP_AGENT="$(mktemp --suffix=.mjs)"
STATUS_TMP="$(mktemp)"
EVIDENCE="/var/cache/andrik-radio-r622/diagnostics/r818-before-${STAMP}.log"
...
cat <<'HDR'
================================================================
 R818 • RAWVIDEO QUEUE24 + DIRECT-STRETCH FULLSCREEN GUARD
 R817/R816 ONE PERSISTENT X264 PRESERVED
 DIRECT SCALE 1920x1080 NO PAD + RAWVIDEO QUEUE 24 + SAR1/DAR16:9
...
echo
echo "=== DOWNLOAD R818 CANONICAL FILES ==="
...
echo
echo "=== R818 STATIC GUARD ==="
python3 - "$TMP_SERVER" <<'PY'
from pathlib import Path
import sys
s=Path(sys.argv[1]).read_text(encoding='utf-8')
required=[
 'R818-RAWVIDEO-QUEUE24-STRETCH-FULLFRAME',
 'R818-RAWVIDEO-QUEUE24-STRETCH-FULLFRAME-R817-R816-R814-PRESERVED',
 ...
 "LIVE_FULL_FRAME_STRETCH_R818",
 "scale=1920:1080:flags=fast_bilinear,setsar=1",
]
missing=[x for x in required if x not in s]
if missing:
 print('ERROR: missing R818 markers:')
...
print('R818 STATIC ARCHITECTURE ✅')
print('  raster : 1920x1080 DIRECT-STRETCH, NO PAD')
...
grep -q "streamProfileR818" "$TMP_AGENT" || { echo "ERROR: R818 web-agent profile alias missing" >&2; exit 32; }
...
echo
echo "=== WAIT FOR VALID R818 API (UP TO 120s) ==="
API_READY=0
for N in $(seq 1 120); do
...
assert 'R818-RAWVIDEO-QUEUE24-STRETCH-FULLFRAME' in str(s.get('version') or '')
assert bool(s.get('publisherRunning'))
p=s.get('streamProfileR818') or {}
g=p.get('geometry') or {}
...
assert 'DIRECT-STRETCH' in str(s.get('liveGeometryModeR818') or '')
PY
    then
      API_READY=1
      echo "R818 API READY ${N}/120 ✅"
      break
    fi
  fi
  sleep 1
...
if [ "$API_READY" != "1" ]; then
  echo "ERROR: valid R818 API did not recover within 120 seconds" >&2
...
echo
echo "=== INSTALL / RESTART R803 WEB AGENT WITH R818 PROFILE ==="
if [ -f "$AGENT" ]; then
  cat "$TMP_AGENT" > "$AGENT"
...
if ! node --check "$AGENT"; then
  echo "WARNING: R818 web agent syntax failed; restoring ONLY agent" >&2
  restore_agent
else
  systemctl restart "$AGENT_UNIT" || true
  sleep 4
  if ! systemctl is-active --quiet "$AGENT_UNIT"; then
    echo "WARNING: R818 web agent did not stay active; restoring ONLY agent" >&2
    restore_agent
  else
...
echo
echo "=== FINAL R818 STATUS ==="
if curl -fsS --max-time 4 "$BASE/status" -o "$STATUS_TMP" 2>/dev/null; then
python3 - "$STATUS_TMP" <<'PY'
import json,sys
s=json.load(open(sys.argv[1],encoding='utf-8'))
p=s.get('streamProfileR818') or {}; v=p.get('video') or {}; a=p.get('audio') or {}; g=p.get('geometry') or {}
...
echo "FIXED LIVE GEOMETRY:"
echo "  live feeder    : DIRECT scale=1920x1080, NO contain/pad"
echo "  rawvideo demux : pixel_format=yuv420p video_size=1920x1080 framerate=25"
echo "  master metadata: setsar=1,setdar=16/9"
