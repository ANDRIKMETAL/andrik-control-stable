...
STAMP="$(date +%s)"
TMP="$(mktemp -d /tmp/andrik-r789.XXXXXX)"
REMOTE="$TMP/server.mjs"
BACKUP="$SERVER.bak-before-r789-$(date +%Y%m%d-%H%M%S)"
trap 'rm -rf "$TMP"' EXIT

rollback(){
  echo '⚠️ R789 live-check не прошёл — возвращаю предыдущий server.'
...
echo '[1/7] Download R789 server'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r789-$STAMP" -o "$REMOTE"
...
required=[
 "R789-YOUTUBE-STEADY-STATE-GUARD-R788-PRESERVED",
 "const MASTER_QUEUE_STARTUP_GRACE_MS_R789 = 30000",
 "masterInputQueueStartupBlockCount",
 "masterInputQueueSteadyBlockCount",
...
if missing:
    print('СТОП: missing R789 markers:', ', '.join(missing)); raise SystemExit(3)
if found:
    print('СТОП: forbidden regression:', ', '.join(found)); raise SystemExit(3)
print('R789 static audit OK')
PY

# Re-prove immutable CONTAIN/FIT+PAD without printing confusing cropdetect coordinates.
...
check_fit(){
  ...
  [ "$got" = "$expected" ] || { echo "СТОП: FULL-FRAME test failed for ${src}"; exit 3; }
...
echo '[4/7] Install R789 + one restart'
...
ok=(
 d.get('version')=='R789-YOUTUBE-STEADY-STATE-GUARD-R788-PRESERVED' and
 ...
 int(d.get('masterTimestampErrorCount') or 0)==0 and
 int(d.get('masterInputQueueStartupGraceMs') or 0)==30000
)
raise SystemExit(0 if ok else 1)
PY
then
  echo '❌ R789 status не подтвердился.'
...
echo '[6/7] Steady-state live guard'
# We are now past the 30s startup grace. Two startup queue warnings (one per tiny input queue)
# are acceptable; what matters is that NO queue saturation occurs once the stream is armed.
STATUS_BASE="$(curl -fsS --max-time 4 http://127.0.0.1:8080/status 2>/dev/null || true)"
STARTUP_BLOCKS="$(STATUS_JSON="$STATUS_BASE" python3 -c 'import os,json;d=json.loads(os.environ.get("STATUS_JSON","{}"));print(int(d.get("masterInputQueueStartupBlockCount") or 0))' 2>/dev/null || echo 999)"
STEADY_BASE="$(STATUS_JSON="$STATUS_BASE" python3 -c 'import os,json;d=json.loads(os.environ.get("STATUS_JSON","{}"));print(int(d.get("masterInputQueueSteadyBlockCount") or 0))' 2>/dev/null || echo 999)"
if [ "$STARTUP_BLOCKS" -gt 4 ]; then echo "❌ excessive startup queue blocks=$STARTUP_BLOCKS"; rollback; exit 6; fi
if [ "$STEADY_BASE" -ne 0 ]; then echo "❌ queue saturation already exists after startup grace=$STEADY_BASE"; rollback; exit 6; fi
STEADY_TS="$(date '+%Y-%m-%d %H:%M:%S')"
sleep 60
if ! systemctl is-active --quiet "$SERVICE"; then rollback; exit 6; fi
journalctl -u "$SERVICE" --since "$STEADY_TS" --no-pager > "$TMP/steady.log" 2>/dev/null || true
if grep -Eqi 'DTS .*out of order|timestamp discontinuity|non[- ]monoton|Broken pipe|Could not write header|Tag .*incompatible|master pipe NO-PROGRESS|Main process exited.*status=[1-9]' "$TMP/steady.log"; then
  echo '❌ Найдена steady-state transport/timestamp regression:'
  grep -Ei 'DTS .*out of order|timestamp discontinuity|non[- ]monoton|Broken pipe|Could not write header|Tag .*incompatible|master pipe NO-PROGRESS|Main process exited.*status=[1-9]' "$TMP/steady.log" | tail -n 60 || true
  rollback; exit 6
fi
...
STEADY_END="$(STATUS_JSON="$STATUS3" python3 -c 'import os,json;d=json.loads(os.environ.get("STATUS_JSON","{}"));print(int(d.get("masterInputQueueSteadyBlockCount") or 0))' 2>/dev/null || echo 999)"
...
[ "$STEADY_END" -eq "$STEADY_BASE" ] || { echo "❌ steady-state master queue blocked: $STEADY_BASE->$STEADY_END"; rollback; exit 6; }
[ "$LOUD" -eq 0 ] || { echo "❌ background loudness jobs active=$LOUD"; rollback; exit 6; }
