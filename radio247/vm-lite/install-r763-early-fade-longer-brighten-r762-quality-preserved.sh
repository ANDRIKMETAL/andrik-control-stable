...
TMP_SERVER="$(mktemp /tmp/andrik-r763.XXXXXX.mjs)"
BACKUP_SERVER="${SERVER}.bak-before-r763-$(date +%Y%m%d-%H%M%S)"
ENV_BACKUP="${ENV_FILE}.bak-before-r763-$(date +%Y%m%d-%H%M%S)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
DROPIN="$DROPIN_DIR/r763-safe-quality.conf"
...
echo '[1/9] Скачиваю R763…'
STAMP="$(date +%s)"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r763-$STAMP" -o "$TMP_SERVER"

echo '[2/9] Проверяю: качество поднято, стабильность R761/R760/R753 сохранена…'
node --check "$TMP_SERVER" >/dev/null
grep -Fq "R763-EARLY-FADE-LONGER-BRIGHTEN-R762-QUALITY-R761-STABILITY-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: скачался не R763'; exit 3; }
...
grep -Fq "R763-R753-SAME-FEEDER-BLACK-ALPHA-0.65-HOLD-0.05-LEAD-1.40-RECOVER-0.80" "$TMP_SERVER" || { echo 'СТОП: идеальный fade R753 потерян'; exit 3; }
...
rollback(){
  echo '⚠️ R763 не прошёл запуск — возвращаю R761/предыдущий server…'
...
ok=(
 d.get('version')=='R763-EARLY-FADE-LONGER-BRIGHTEN-R762-QUALITY-R761-STABILITY-PRESERVED' and
 ...
 d.get('mp3BoundaryFadeMode')=='R763-R753-SAME-FEEDER-BLACK-ALPHA-0.65-HOLD-0.05-LEAD-1.40-RECOVER-0.80' and
 d.get('videoFadeLeadSeconds')==1.4 and
 d.get('videoFadeInSeconds')==0.8
)
...
if [ "$OK" != 1 ]; then
  echo '❌ R763 status не подтвердился.'
...
echo
echo '✅ R763 ГОТОВ — затемнение начинается на 1.0с раньше, осветление 0.80с'
...
echo '✅ R760/R753 геометрия сохранена; fade R753 расширен по просьбе: lead 1.40s / recover 0.80s'
