...
while true; do
  ...
  echo "ANDRIK RADIO CONTROL · R625"
  line
  ...
  echo "6) 🔐 АВТОРИЗАЦИЯ YOUTUBE — КОД С САЙТА"
  ...
  case "$choice" in
    ...
    6)
      if need_youtube; then
        echo
        echo "Открой: https://andrikmetal.com/youtube-device-pair-admin.html"
        echo "Вставь Client ID + Secret на странице и получи короткий одноразовый код."
        printf "Код с сайта: "
        read -r pair_code || true
        if [ -n "${pair_code:-}" ]; then andrik-youtube pair "$pair_code"; else echo "Код пустой."; fi
      fi
      pause ;;
