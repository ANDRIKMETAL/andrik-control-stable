...
const DEFAULT_TITLE='🔴 ANDRIK METAL RADIO 24/7 | Heavy Metal • Female Vocals';
const DEFAULT_DESCRIPTION=`🔴 ANDRIK METAL RADIO — LIVE 24/7

Непрерывный эфир оригинальной музыки ANDRIK.
Heavy Metal • Industrial Metal • Epic Metal • Female Vocals

Здесь звучат альбомы, синглы и новые композиции ANDRIK: мощный женский вокал, тяжёлые риффы, мелодичные гитарные соло, индустриальный бас и философские тексты о сознании, свободе и пробуждении.

🎵 Название текущего трека отображается прямо в эфире.
🌐 Официальный сайт: https://andrikmetal.com
▶️ YouTube: https://youtube.com/@andrikmetal

────────────
ENGLISH
24/7 original ANDRIK metal radio.
Heavy Metal • Industrial Metal • Epic Metal • Female Vocals.
Albums, singles and new tracks in one continuous stream.
...
async function metadata(){
  ...
  const x=await current(token);
  const b=x.broadcast;
  if(!b){console.log('Активного/готового broadcast нет — метаданные применятся при следующем создании.');return null}
  const sn=b.snippet||{};
  const snippet={title:DEFAULT_TITLE,description:DEFAULT_DESCRIPTION,scheduledStartTime:sn.scheduledStartTime};
  if(sn.scheduledEndTime)snippet.scheduledEndTime=sn.scheduledEndTime;
  if(sn.categoryId)snippet.categoryId=String(sn.categoryId);
  const updated=await apiFetch(token,'/liveBroadcasts',{part:'snippet'},{method:'PUT',body:JSON.stringify({id:b.id,snippet})});
  console.log('ГОТОВО ✅ Название и полное описание ANDRIK Radio восстановлены.');
...
async function stopRadioFast(){
  ...
  console.log('Encoder ещё завершает буфер → повторяю мягкий SIGTERM…');
  spawnSync('systemctl',['kill','--kill-who=all','--signal=SIGTERM',RADIO_SERVICE],{stdio:'ignore'});
  for(let i=0;i<8;i++){await sleep(500);if(['inactive','failed','unknown'].includes(serviceState()))break;}
  if(!['inactive','failed','unknown'].includes(serviceState())){
    console.log('Только аварийный fallback: SIGKILL после полного graceful timeout.');
...
async function endLive(){
  const token=await accessToken();
  let x=await current(token);
  const broadcastId=clean(x?.broadcast?.id);
  const streamId=clean(x?.broadcast?.contentDetails?.boundStreamId || x?.stream?.id);

  // R637 correct shutdown order: encoder first. The Node radio closes the current
  // decoder, sends EOF to the persistent PCM pipe and lets the one AAC/FLV master
  // flush normally before systemd reports inactive. Only then complete YouTube.
  if(serviceActive() || ['active','activating','deactivating','reloading'].includes(serviceState())){
    console.log('1/2 Мягко завершаю encoder и выгружаю AAC/FLV буфер…');
    await stopRadioFast();
  }
  if(streamId){
    await waitSpecificStream(token,streamId,'inactive',20000).catch(()=>sleep(2000));
  }
  x=broadcastId?await broadcastById(token,broadcastId):x;
  if(x.broadcast && ['live','livestarting','testing','teststarting'].includes(life(x.broadcast))){
    console.log('2/2 Сигнал закрыт → завершаю broadcast на YouTube…');
    await transition(token,x.broadcast.id,'complete');
    console.log('YouTube broadcast завершён корректно ✅');
  }else console.log('Активного LIVE broadcast уже нет ✅');
}
async function main(){
  ...
  if(cmd==='metadata'||cmd==='meta'||cmd==='restore-metadata')return metadata();
  ...
  console.log('ANDRIK YouTube Console R637');
  console.log('Команды: sync | pair CODE | auth | status | metadata | create | launch | auto | auto-safe | start | recover | end');
}
