...
STAMP="$(date +%s)"
TMP="$(mktemp -d /tmp/andrik-r783.XXXXXX)"
REMOTE="$TMP/server.mjs"
BACKUP="$SERVER.bak-before-r783-$(date +%Y%m%d-%H%M%S)"
trap 'rm -rf "$TMP"' EXIT

rollback(){
  echo '⚠️ R783 live-check не прошёл — возвращаю предыдущую рабочую версию.'
...
echo '[1/8] Download R783'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r783-$STAMP" -o "$REMOTE"

echo '[2/8] Verify exact VPS FIFO→FLV compatibility before restart'
node --check "$REMOTE" >/dev/null
grep -Fq "R783-ALTERNATING-SUBSCRIBE-LIKE-R782-PRESERVED" "$REMOTE" || { echo 'СТОП: remote server не R783'; exit 3; }
...
grep -Fq "OUTPUT_FATAL_REGEX_R780" "$REMOTE" || { echo 'СТОП: нет R783 egress guard'; exit 3; }
grep -Fq "TITLE_SWITCH_BEFORE_BOUNDARY_R781" "$REMOTE" || { echo 'СТОП: нет R783 title boundary clock'; exit 3; }
grep -Fq "R781-NEXT-MP3-DURING-BLACK-BEFORE-RECOVERY" "$REMOTE" || { echo 'СТОП: нет R783 title handoff marker'; exit 3; }
grep -Fq "dynamicTitle:true,showPreview:true" "$REMOTE" || { echo 'СТОП: CURRENT не reloadable для R783'; exit 3; }
grep -Fq "rightSubscribeMode:'R767-TRANSPARENT-420PX-BOTTOM-RIGHT'" "$REMOTE" || { echo 'СТОП: потеряна правая SUBSCRIBE'; exit 3; }
grep -Fq "CTA_LIKE_OVERLAY_R783" "$REMOTE" || { echo 'СТОП: LIKE asset не подключён'; exit 3; }
grep -Fq "compactCtaChainR783" "$REMOTE" || { echo 'СТОП: нет R783 alternating CTA chain'; exit 3; }
grep -Fq "R783-SUBSCRIBE-LIKE-ALTERNATE-EVERY-120S" "$REMOTE" || { echo 'СТОП: нет R783 alternating CTA status'; exit 3; }
grep -Fq "CLIP_PREP_SUFFIX_R782 = '.r783-ready.mp4'" "$REMOTE" || { echo 'СТОП: нет R783 fresh prepared cache'; exit 3; }
grep -Fq "probeStationLeadingSilenceR782" "$REMOTE" || { echo 'СТОП: нет R783 PCM station sync'; exit 3; }
grep -Fq "R783-PREBAKED-ALTERNATING-SUBSCRIBE-LIKE-RIGHT-CTA" "$REMOTE" || { echo 'СТОП: нет R783 clip CTA'; exit 3; }
...
# R783: clip CTA is baked only in background prepared files. Fetch/validate the exact
# transparent right-side asset before touching the live service.
CTA_REMOTE="$TMP/subscribe-right-r767.png"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/assets/subscribe-right-r767.png?v=55.00-r783-$STAMP" -o "$CTA_REMOTE"
...
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/assets/like-right-r783.png?v=55.00-r783-$STAMP" -o "$LIKE_REMOTE"
[ "$(stat -c%s "$LIKE_REMOTE")" -gt 10000 ] || { echo 'СТОП: LIKE asset слишком маленький'; exit 3; }
python3 - "$LIKE_REMOTE" <<'PYLIKE'
import sys
p=sys.argv[1]
b=open(p,'rb').read(8)
raise SystemExit(0 if b==b'\x89PNG\r\n\x1a\n' else 1)
...
# Verify the VPS can execute the only audio filter R783 adds OFFLINE. This deliberately
...
ffmpeg -nostdin -hide_banner -loglevel error -f lavfi -i 'anullsrc=r=44100:cl=stereo' -t 0.35 \
  ...
  -c:a pcm_s16le -f s16le "$TMP/r783-audio-preflight.pcm"
[ -s "$TMP/r783-audio-preflight.pcm" ] || { echo 'СТОП: R783 offline audio prep test failed'; exit 3; }
...
ok=(d.get('version')=='R783-ALTERNATING-SUBSCRIBE-LIKE-R782-PRESERVED' and d.get('publisherRunning') is True and d.get('transportHealthy') is True and d.get('masterFlvTagMode')=='R780-VTAG7-ATAG10-OLD-FFMPEG-FIFO-COMPAT' and d.get('currentTitleHandoff')=='R781-NEXT-MP3-DURING-BLACK-BEFORE-RECOVERY' and d.get('rightSubscribeMode')=='R767-TRANSPARENT-420PX-BOTTOM-RIGHT' and d.get('rightCtaMode')=='R783-SUBSCRIBE-LIKE-420PX-BOTTOM-RIGHT-SMOOTH-ALTERNATING' and d.get('ctaAlternateMode')=='R783-SUBSCRIBE-LIKE-ALTERNATE-EVERY-120S' and d.get('clipSubscribeOverlay')=='R783-PREBAKED-ALTERNATING-SUBSCRIBE-LIKE-RIGHT-CTA' and d.get('stationInsertSync')=='R782-ALL-3-BUMPERS+SPECIAL30+SPECIAL60-OFFLINE-PCM-LEAD-TRIM')
raise SystemExit(0 if ok else 1)
PY
then
  echo '❌ R783 status не подтвердился.'
...
echo '✅ R783: all 3 bumpers + SPECIAL 30/60 use the same offline PCM sync normalizer'
echo '✅ R783: normal clips alternate right SUBSCRIBE / LIKE offline: first at 20s, then every 120s, 8s each'
echo '✅ R781 title switch at black preserved'
echo '✅ R780 transport preserved: FLV tag7/tag10 + hard egress guard'
echo '✅ MP3 CTA alternates SUBSCRIBE / LIKE: first SUBSCRIBE at 20s, then every 120s, 8s each, smooth fade'
