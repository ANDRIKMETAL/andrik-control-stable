...
STAMP="$(date +%s)"
TMP="$(mktemp -d /tmp/andrik-r781.XXXXXX)"
REMOTE="$TMP/server.mjs"
BACKUP="$SERVER.bak-before-r781-$(date +%Y%m%d-%H%M%S)"
trap 'rm -rf "$TMP"' EXIT

rollback(){
  echo '⚠️ R781 live-check не прошёл — возвращаю предыдущую рабочую версию.'
...
echo '[1/8] Download R781'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r781-$STAMP" -o "$REMOTE"

echo '[2/8] Verify exact VPS FIFO→FLV compatibility before restart'
node --check "$REMOTE" >/dev/null
grep -Fq "R781-MP3-TITLE-SWITCH-AT-BLACK-R780-PRESERVED" "$REMOTE" || { echo 'СТОП: remote server не R781'; exit 3; }
...
grep -Fq "OUTPUT_FATAL_REGEX_R780" "$REMOTE" || { echo 'СТОП: нет R781 egress guard'; exit 3; }
grep -Fq "TITLE_SWITCH_BEFORE_BOUNDARY_R781" "$REMOTE" || { echo 'СТОП: нет R781 title boundary clock'; exit 3; }
grep -Fq "R781-NEXT-MP3-DURING-BLACK-BEFORE-RECOVERY" "$REMOTE" || { echo 'СТОП: нет R781 title handoff marker'; exit 3; }
grep -Fq "dynamicTitle:true,showPreview:true" "$REMOTE" || { echo 'СТОП: CURRENT не reloadable для R781'; exit 3; }
...
ok=(d.get('version')=='R781-MP3-TITLE-SWITCH-AT-BLACK-R780-PRESERVED' and d.get('publisherRunning') is True and d.get('transportHealthy') is True and d.get('masterFlvTagMode')=='R780-VTAG7-ATAG10-OLD-FFMPEG-FIFO-COMPAT' and d.get('currentTitleHandoff')=='R781-NEXT-MP3-DURING-BLACK-BEFORE-RECOVERY' and d.get('rightSubscribeMode')=='R767-TRANSPARENT-420PX-BOTTOM-RIGHT')
raise SystemExit(0 if ok else 1)
PY
then
  echo '❌ R781 status не подтвердился.'
...
echo '✅ R781: MP3→MP3 CURRENT switches during black phase before recovery'
