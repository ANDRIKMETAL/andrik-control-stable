...
TMP="$(mktemp /tmp/andrik-r738.XXXXXX.mjs)"
BACKUP="${SERVER}.bak-before-r738-$(date +%Y%m%d-%H%M%S)"
trap 'rm -f "$TMP"' EXIT

for c in curl node python3 systemctl ffmpeg; do command -v "$c" >/dev/null || { echo "СТОП: $c не найден"; exit 2; }; done
[ -s "$SERVER" ] || { echo "СТОП: нет $SERVER"; exit 2; }

echo '[1/8] Скачиваю R738…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r738-$(date +%s)" -o "$TMP"

echo '[2/8] Проверяю R738 и НЕИЗМЕННЫЙ MP3 транспорт…'
node --check "$TMP" >/dev/null
grep -Fq "R738-AV-LOCK-INSERT-AUDIO-EARLY-TITLE-FADE-R737-PRESERVED" "$TMP" || { echo 'СТОП: скачался не R738'; exit 3; }
...
rollback(){
  echo '⚠️ R738 не прошёл запуск — возвращаю предыдущий server.mjs…'
  cp -a "$BACKUP" "$SERVER"
  systemctl restart "$SERVICE" || true
  sleep 5
}

echo '[6/8] Устанавливаю R738. MP3 звук / 1024-8 / publisher НЕ меняются…'
...
if [ "$OK" != 1 ]; then
  echo '❌ R738 status не подтвердился.'
  printf '%s\n' "$STATUS" | python3 -m json.tool 2>/dev/null || true
  rollback
  exit 5
fi

printf '%s\n' "$STATUS" | python3 -c 'import sys,json; d=json.load(sys.stdin); print("VERSION:",d.get("version")); print("PUBLISHER:",d.get("publisherRunning")); print("PRODUCER:",d.get("producerRunning")); print("VIDEO QUEUE:",d.get("videoInputQueuePackets")); print("AUDIO QUEUE:",d.get("audioInputQueuePackets")); print("FADE LEAD:",d.get("videoFadeLeadSeconds")); print("TITLE LEAD:",d.get("titleVisualLeadSeconds")); print("CLIP A/V:",d.get("clipAvSyncMode")); print("PRE-DRAIN:",d.get("clipPreDrainMs")); print("POST-DRAIN:",d.get("clipPostDrainMs")); print("INSERT AUDIO REQUIRED:",d.get("stationInsertAudioRequired")); print("NEXT:",d.get("nextPreviewSource"))'

echo
echo '========================================================'
echo '✅ R738 ГОТОВ'
echo '✅ MP3 транспорт сохранён: VIDEO 1024 / AUDIO 8'
echo '✅ SAFE fade сдвинут раньше: lead 3.80s'
