...
async function downloadVisualR650(slot,headers,{force=false}={}){
  ...
  const r=await fetch(`${BASE}/api/radio-agent-r650/visual?slot=${encodeURIComponent(slot)}&download=1`,{headers:{authorization:headers.authorization,'user-agent':'ANDRIK-Radio-Web-Agent-R710'},signal:AbortSignal.timeout(180000)});
  if(!r.ok){
...
async function daemon(){
  while(true){
    try{
      ...
      const headers={'content-type':'application/json','authorization':'Bearer '+cfg.token,'user-agent':'ANDRIK-Radio-Web-Agent-R710'};
      const status=await localStatus();
      // Heartbeat always continues, even while start/recover is running.
      const d=await jsonFetch(BASE+'/api/radio-agent-r627/poll',{method:'POST',headers,body:JSON.stringify({version:'R710',status})});
...
async function main(){
  ...
  console.log('ANDRIK Radio Web Agent R710 · commands: daemon | status | bootstrap-visuals | visual-sync | visual-now <morning|day|evening|night> | visual-auto');
}
