...
STAMP="$(date +%s)"
TMP="$(mktemp -d /tmp/andrik-r792.XXXXXX)"
REMOTE="$TMP/server.mjs"
BACKUP="$SERVER.bak-before-r792-$(date +%Y%m%d-%H%M%S)"
trap 'rm -rf "$TMP"' EXIT

rollback(){
  echo '⚠️ R792 live-check не прошёл — возвращаю предыдущий server.'
...
echo '[1/8] Download R792 server'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r792-$STAMP" -o "$REMOTE"

echo '[2/8] Static audit: NO-CROP + station no-gap + dual RTMPS'
...
required=[
 "R792-STATION-NOGAP-DUAL-RTMPS-R791-R790-PRESERVED",
...
print('R792 static ordering audit OK')
PY
...
echo 'R792 local dual-ingest packet identity proof OK'

echo '[3/8] Backup current server'
cp -a "$SERVER" "$BACKUP"

echo '[4/8] Install R792 + one restart'
...
ok=(
 d.get('version')=='R792-STATION-NOGAP-DUAL-RTMPS-R791-R790-PRESERVED' and
...
if grep -Fq '[loudness-r750-background]' "$TMP/live.log"; then
  echo '❌ Background loudness unexpectedly ran'; rollback; exit 7
fi
