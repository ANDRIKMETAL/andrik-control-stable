...
TMP_DIR="$(mktemp -d /tmp/andrik-r709.XXXXXX)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
ENGINE_DROPIN="$DROPIN_DIR/r709-engine.conf"
AGENT_TARGET=/usr/local/sbin/andrik-radio-web
AGENT_LIB=/usr/local/lib/andrik-radio-web-agent-r709.mjs
...
echo '[1/10] Загружаю R709 engine + ESM-safe agent + 4-slot scheduler…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r709-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-radio-web-agent-r709.mjs?v=55.00-r709-$(date +%s)" -o "$TMP_DIR/agent.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-visual-auto-r703.sh?v=55.00-r709-$(date +%s)" -o "$TMP_DIR/auto.sh"

node --check "$TMP_DIR/server.mjs" >/dev/null
node --check "$TMP_DIR/agent.mjs" >/dev/null
bash -n "$TMP_DIR/auto.sh"
grep -q 'R709-VISUAL-LIBRARY-MORNING-FINAL-R708-PRESERVED' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R709'; exit 3; }
...
grep -q "version:'R709'" "$TMP_DIR/agent.mjs" || { echo 'СТОП: agent не R709'; exit 3; }
...
cp -a "$SERVER" "$SERVER.bak-r709-$TS"
[ -s "$AGENT_TARGET" ] && cp -a "$AGENT_TARGET" "$AGENT_TARGET.bak-r709-$TS" || true
[ -s "$AUTO_TARGET" ] && cp -a "$AUTO_TARGET" "$AUTO_TARGET.bak-r709-$TS" || true

echo '[3/10] Ставлю R709 engine + настоящий .mjs agent…'
...
#!/usr/bin/env bash
exec /usr/bin/node /usr/local/lib/andrik-radio-web-agent-r709.mjs "$@"
WRAP
...
[Unit]
Description=ANDRIK Radio Web Control Agent R709
...
[Unit]
Description=ANDRIK four visual cycles scheduler R703/R709
...
echo '[6/10] Перезапускаю именно R709 agent и проверяю, что ESM реально стартует…'
...
systemctl is-active --quiet "$AGENT_SERVICE" || {
  echo 'СТОП: R709 agent не поднялся.'
...
echo '[9/10] Чистый restart сбрасывает старый PCM backlog и применяет R709…'
systemctl restart "$SERVICE"
sleep 7
"$AUTO_TARGET" force || true

echo '[10/10] Проверяю R709 + agent + MORNING + title sync…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  sleep 2
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R709-VISUAL-LIBRARY-MORNING-FINAL-R708-PRESERVED'; then
...
if [ -s "$MORNING" ]; then
  ...
  echo '⚠️ MORNING master пока не появился локально — нажми «Назначить + сейчас» один раз; R709 agent уже исправлен.'
fi

echo
echo '============================================'
echo '✅ R709 ГОТОВ · VISUAL LIBRARY + MORNING FINAL'
...
echo '✅ MORNING/DAY/EVENING/NIGHT используют одинаковый upload → R2 library → assign → OVH путь'
