...
TMP_DIR="$(mktemp -d /tmp/andrik-r717.XXXXXX)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
ENGINE_DROPIN="$DROPIN_DIR/r717-engine.conf"
...
echo '[1/11] Загружаю R717: рабочее движение R706 + стабильный transport R715…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r717-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-radio-web-agent-r715.mjs?v=55.00-r717-$(date +%s)" -o "$TMP_DIR/agent.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-visual-auto-r703.sh?v=55.00-r717-$(date +%s)" -o "$TMP_DIR/auto.sh"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/force-full-fit-r712.sh?v=55.00-r717-$(date +%s)" -o "$TMP_DIR/fullfit.sh"

for p in morning day evening night; do
  curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
    "$SITE_BASE/assets/equalizer-${p}-r717.mov?v=55.00-r717-$(date +%s)" -o "$TMP_DIR/equalizer-${p}.mov"
done
...
grep -q 'R717-R715-STABLE-DIRECT-R706-VISIBLE-4-EQ-AUTO-LIVE-R2' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R717'; exit 3; }
...
grep -q "R717-R706-PRERENDERED-QTRLE-4-SLOT-DIRECT" "$TMP_DIR/server.mjs" || { echo 'СТОП: R717 EQ engine отсутствует'; exit 3; }
grep -q "'-thread_queue_size','64','-re','-stream_loop','-1','-i',eq.path" "$TMP_DIR/server.mjs" || { echo 'СТОП: tiny EQ input отсутствует'; exit 3; }
grep -q "overlay=x=(W-w)/2:y=H-h-210" "$TMP_DIR/server.mjs" || { echo 'СТОП: EQ не закреплён над названием'; exit 3; }
grep -q "'-map','\\[outv\\]','-map','2:a:0'" "$TMP_DIR/server.mjs" || { echo 'СТОП: звук перестал идти напрямую'; exit 3; }
grep -q "const insetCrop=''; // R712 NO CROP — FULL FRAME FIT" "$TMP_DIR/server.mjs" || { echo 'СТОП: NO CROP не закреплён'; exit 3; }
...
cp -a "$SERVER" "$SERVER.bak-r717-$TS"
[ -s "$AGENT_TARGET" ] && cp -a "$AGENT_TARGET" "$AGENT_TARGET.bak-r717-$TS" || true
mkdir -p "$ASSET_DIR" "$DROPIN_DIR" "$VISUAL_DIR"
for p in morning day evening night; do
  [ -s "$ASSET_DIR/equalizer-${p}-r717.mov" ] && cp -a "$ASSET_DIR/equalizer-${p}-r717.mov" "$ASSET_DIR/equalizer-${p}-r717.mov.bak-$TS" || true
done

echo '[5/11] Ставлю server + 4 EQ loops…'
install -m 0644 "$TMP_DIR/server.mjs" "$SERVER"
for p in morning day evening night; do
  install -m 0644 "$TMP_DIR/equalizer-${p}.mov" "$ASSET_DIR/equalizer-${p}-r717.mov"
done
...
[Unit]
Description=ANDRIK four visual cycles scheduler R703/R717
...
echo '[10/11] Проверяю R717 + стабильный YouTube transport…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  sleep 2
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R717-R715-STABLE-DIRECT-R706-VISIBLE-4-EQ-AUTO-LIVE-R2'; then
    if printf '%s' "$STATUS" | python3 -c 'import sys,json; d=json.load(sys.stdin); raise SystemExit(0 if d.get("publisherRunning") and d.get("videoBitrate")=="4500k" and d.get("outputTimeshiftSeconds")==6 and d.get("engine")=="R678-R695-DIRECT-FFMPEG" and d.get("equalizerEngine")=="R717-R706-PRERENDERED-QTRLE-4-SLOT-DIRECT" else 1)' 2>/dev/null; then OK=1; break; fi
  fi
done
[ "$OK" = 1 ] || {
  echo 'R717 radio не поднялся как stable DIRECT.'
...
echo '✅ НО старый R706 MJPEG/GEQ 1080p producer НЕ возвращён'
...
echo '✅ EQ закреплён НАД названием; красной линии и подложки нет'
echo '✅ MP3 hotfix 234 + R2 клипы/delete + FULL FIT + AUTO LIVE сохранены'
echo '======================================================'
