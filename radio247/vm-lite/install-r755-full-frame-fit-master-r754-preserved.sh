...
TMP_SERVER="$(mktemp /tmp/andrik-r755.XXXXXX.mjs)"
BACKUP_SERVER="${SERVER}.bak-before-r755-$(date +%Y%m%d-%H%M%S)"
ENV_BACKUP="${ENV_FILE}.bak-before-r755-$(date +%Y%m%d-%H%M%S)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
DROPIN="$DROPIN_DIR/r755-full-frame-fit-master.conf"
...
echo '[1/10] Скачиваю R755 server…'
STAMP="$(date +%s)"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r755-$STAMP" -o "$TMP_SERVER"

echo '[2/10] Проверяю R755 + сохранность R754/R753…'
node --check "$TMP_SERVER" >/dev/null
grep -Fq "R755-FULL-FRAME-FIT-MASTER-R754-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: скачался не R755'; exit 3; }
grep -Fq "R755-PERSISTENT-X264-FULL-FRAME-FIT-NO-CROP-R754-PRESERVED" "$TMP_SERVER" || { echo 'СТОП: master video isolation отсутствует'; exit 3; }
...
rollback(){
  echo '⚠️ R755 не прошёл запуск — возвращаю предыдущую версию…'
...
echo '[5/10] Устанавливаю R755 FULL-FRAME FIT без изменения таймингов…'
...
echo '[8/10] Проверяю R755 status…'
STATUS=''; OK=0
...
ok=(
 d.get('version')=='R755-FULL-FRAME-FIT-MASTER-R754-PRESERVED' and
 d.get('publisherRunning') is True and
 d.get('masterVideoMode')=='R755-PERSISTENT-X264-FULL-FRAME-FIT-NO-CROP-R754-PRESERVED' and
...
if [ "$OK" != 1 ]; then
  echo '❌ R755 status не подтвердился.'
...
if journalctl -u "$SERVICE" --since '-20 sec' --no-pager | grep -Eq 'Main process exited.*status=(75|76)|R751 STREAM STALL'; then
  echo '❌ Сразу после R755 обнаружен transport restart.'
  rollback
  exit 6
fi
...
echo
echo '✅ R755 ГОТОВ — FULL FRAME FIT / NO CROP + R754 STABILITY'
echo '✅ весь кадр FIT 1920x1080, crop/cover OFF, SAR=1'
