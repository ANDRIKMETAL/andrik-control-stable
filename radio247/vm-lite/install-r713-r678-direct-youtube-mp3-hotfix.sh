...
TMP_DIR="$(mktemp -d /tmp/andrik-r713.XXXXXX)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
ENGINE_DROPIN="$DROPIN_DIR/r713-engine.conf"
AGENT_TARGET=/usr/local/sbin/andrik-radio-web
AGENT_LIB=/usr/local/lib/andrik-radio-web-agent-r713.mjs
...
echo '[1/10] Загружаю R713: настоящий R678/R695 DIRECT YouTube engine…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r713-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-radio-web-agent-r713.mjs?v=55.00-r713-$(date +%s)" -o "$TMP_DIR/agent.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-visual-auto-r703.sh?v=55.00-r713-$(date +%s)" -o "$TMP_DIR/auto.sh"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/force-full-fit-r712.sh?v=55.00-r713-$(date +%s)" -o "$TMP_DIR/fullfit.sh"
...
grep -q 'R713-R678-R695-DIRECT-YOUTUBE-MP3-HOTFIX-FULLFIT-R2' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R713'; exit 3; }
...
if grep -q "'-c:v','mjpeg'\|'pipe:4'\|normalVideoProducerArgs\|startNormalVisualProducer" "$TMP_DIR/server.mjs"; then
  echo 'СТОП: найден новый MJPEG feeder — R713 должен быть прямым.'; exit 3
fi
grep -q "const insetCrop=''; // R713 NO CROP" "$TMP_DIR/server.mjs" || { echo 'СТОП: NO CROP не закреплён'; exit 3; }
...
grep -q 'ensureNextTrackReadyR713' "$TMP_DIR/server.mjs" || { echo 'СТОП: защита clip→MP3 отсутствует'; exit 3; }
grep -q "version:'R713'" "$TMP_DIR/agent.mjs" || { echo 'СТОП: agent не R713'; exit 3; }
...
cp -a "$SERVER" "$SERVER.bak-r713-$TS"
[ -s "$AGENT_TARGET" ] && cp -a "$AGENT_TARGET" "$AGENT_TARGET.bak-r713-$TS" || true
[ -s "$AUTO_TARGET" ] && cp -a "$AUTO_TARGET" "$AUTO_TARGET.bak-r713-$TS" || true

echo '[4/10] Ставлю прямой engine + R713 agent + FULL FIT helper…'
...
rm -f "$DROPIN_DIR"/r70{1,2,3,4,5,6,7,8,9}-engine.conf "$DROPIN_DIR"/r71{0,1,2}-engine.conf 2>/dev/null || true
...
#!/usr/bin/env bash
exec /usr/bin/node /usr/local/lib/andrik-radio-web-agent-r713.mjs "$@"
WRAP
chmod 0755 "$AGENT_TARGET"

cat > "/etc/systemd/system/$AGENT_SERVICE" <<'UNIT'
[Unit]
Description=ANDRIK Radio Web Control Agent R713
...
[Unit]
Description=ANDRIK four visual cycles scheduler R703/R713
...
echo '[6/10] Перезапускаю R713 agent…'
...
echo '[9/10] Проверяю R713 DIRECT status…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  sleep 2
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R713-R678-R695-DIRECT-YOUTUBE-MP3-HOTFIX-FULLFIT-R2'; then
...
[ "$OK" = 1 ] || {
  echo 'R713 radio не поднялся как DIRECT engine.'
...
echo
echo '======================================================'
echo '✅ R713 ГОТОВ · R678/R695 DIRECT YOUTUBE ENGINE'
