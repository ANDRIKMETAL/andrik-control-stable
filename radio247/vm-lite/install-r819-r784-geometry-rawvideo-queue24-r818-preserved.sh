...
BACKUP="${LIVE}.bak-before-r819-${STAMP}"
AGENT_BACKUP="${AGENT}.bak-before-r819-${STAMP}"
TMP_SERVER="$(mktemp --suffix=.mjs)"
TMP_AGENT="$(mktemp --suffix=.mjs)"
STATUS_TMP="$(mktemp)"
EVIDENCE="/var/cache/andrik-radio-r622/diagnostics/r819-before-${STAMP}.log"
...
cat <<'HDR'
================================================================
 R819 • R784 VIEWER-PROVEN GEOMETRY + RAWVIDEO QUEUE24
 R818/R816 ONE PERSISTENT X264 PRESERVED
 EXACT R784/R814 FIT+PAD 1920x1080 + RAWVIDEO QUEUE24 + MASTER NO GEOMETRY FILTER
...
echo
echo "=== DOWNLOAD R819 CANONICAL FILES ==="
...
required=[
 'R819-R784-VIEWER-PROVEN-GEOMETRY-RAWVIDEO-QUEUE24',
 'R819-R784-GEOMETRY-RAWVIDEO-QUEUE24-R818-R816-R814-PRESERVED',
...
if missing:
 print('ERROR: missing R819 markers:')
...
for token in ["'-f','rawvideo'","'-s:v','1920x1080'","'-pix_fmt','yuv420p'","'-r',String(VIDEO_FPS)",'...h264EncoderArgsR721()']:
 if token not in pub: raise SystemExit(f'ERROR: publisher missing {token}')
if "'-c:v','copy'" in pub or "'-f','h264','-i','pipe:4'" in pub or "'-vf','setsar=1,setdar=16/9'" in pub or "'-aspect','16:9'" in pub:
 raise SystemExit('ERROR: R819 master must have no geometry rewrite')
...
print('R819 STATIC ARCHITECTURE ✅')
print('  geometry: EXACT R784/R814 viewer-proven FIT+PAD 1920x1080, SAR1')
print('  master  : exact R784 rawvideo demux semantics; NO master geometry filter')
print('  queue   : 24 rawvideo frames (~0.96s @25fps)')
print('  feeder : raw YUV420P full frames')
print('  master : ONE persistent libx264')
PY

grep -q "streamProfileR819" "$TMP_AGENT" || { echo "ERROR: R819 web-agent profile alias missing" >&2; exit 32; }
...
echo
echo "=== WAIT FOR VALID R819 API (UP TO 120s) ==="
API_READY=0
for N in $(seq 1 120); do
...
assert 'R819-R784-VIEWER-PROVEN-GEOMETRY-RAWVIDEO-QUEUE24' in str(s.get('version') or '')
assert bool(s.get('publisherRunning'))
p=s.get('streamProfileR819') or s.get('streamProfileR818') or {}
g=p.get('geometry') or {}
assert g.get('raster')=='1920x1080'
assert g.get('sampleAspectRatio')=='1:1'
assert g.get('displayAspectRatio')=='16:9'
assert int(s.get('videoInputQueuePackets') or 0)==24
assert 'R784-VIEWER-PROVEN-FIT-PAD' in str(s.get('liveGeometryModeR819') or '')
PY
    then
      API_READY=1
      echo "R819 API READY ${N}/120 ✅"
      break
    fi
  fi
  sleep 1
...
if [ "$API_READY" != "1" ]; then
  echo "ERROR: valid R819 API did not recover within 120 seconds" >&2
...
echo
echo "=== INSTALL / RESTART R803 WEB AGENT WITH R819 PROFILE ==="
if [ -f "$AGENT" ]; then
  cat "$TMP_AGENT" > "$AGENT"
...
if ! node --check "$AGENT"; then
  echo "WARNING: R819 web agent syntax failed; restoring ONLY agent" >&2
  restore_agent
else
  systemctl restart "$AGENT_UNIT" || true
  sleep 4
  if ! systemctl is-active --quiet "$AGENT_UNIT"; then
    echo "WARNING: R819 web agent did not stay active; restoring ONLY agent" >&2
    restore_agent
  else
...
echo
echo "=== FINAL R819 STATUS ==="
if curl -fsS --max-time 4 "$BASE/status" -o "$STATUS_TMP" 2>/dev/null; then
python3 - "$STATUS_TMP" <<'PY'
import json,sys
s=json.load(open(sys.argv[1],encoding='utf-8'))
p=s.get('streamProfileR819') or s.get('streamProfileR818') or {}; v=p.get('video') or {}; a=p.get('audio') or {}; g=p.get('geometry') or {}
...
echo "VIEWER-PROVEN GEOMETRY RESTORED:"
echo "  live feeder    : R784/R814 scale=decrease + pad=1920x1080 + setsar=1"
echo "  rawvideo demux : pixel_format=yuv420p video_size=1920x1080 framerate=25"
echo "  master geometry: NONE (exact R784 rawvideo demux/encode semantics)"
echo "  output raster  : 1920x1080 from full raw frames"
echo "BACKPRESSURE GUARD:"
echo "  rawvideo input queue: 24 frames (~0.96s @25fps), was 8 (~0.32s)"
