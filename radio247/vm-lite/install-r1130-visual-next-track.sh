...
printf '%s\n' '=========================================================='
printf '%s\n' ' ANDRIK R1130 · MASTER VIDEO ON NEXT TRACK'
printf '%s\n' ' R1129 CPU HEADROOM PRESERVED'
printf '%s\n' ' ONE RADIO RESTART'
printf '%s\n' '=========================================================='
...
curl -fsSL --retry 6 --retry-delay 2 "${SITE_BASE%/}/radio247/server.mjs?v=R1130-${STAMP}" -o "$TMP_SERVER"
curl -fsSL --retry 6 --retry-delay 2 "${SITE_BASE%/}/radio247/vm-lite/andrik-radio-web-agent-r803.mjs?v=R1130-${STAMP}" -o "$TMP_AGENT"
...
CANON=''
if systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Fxq andrik-radio-web.service; then
  CANON='andrik-radio-web.service'
elif systemctl list-unit-files --type=service --no-legend 2>/dev/null | awk '{print $1}' | grep -Fxq andrik-radio-web-agent.service; then
  CANON='andrik-radio-web-agent.service'
fi
[ -n "$CANON" ] || { echo '❌ web-agent service not found'; exit 40; }
...
printf '\n==========================================================\n'
echo '✅ R1130 INSTALLED'
echo '✅ R1129 CPU HEADROOM PRESERVED'
