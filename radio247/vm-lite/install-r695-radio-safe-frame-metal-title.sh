...
TMP="$(mktemp /tmp/andrik-radio-r695.XXXXXX.mjs)"
BACKUP="$SERVER.bak-r695-$(date +%Y%m%d-%H%M%S)"
trap 'rm -f "$TMP"' EXIT
...
echo '[1/5] Загружаю R695…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r695-$(date +%s)" -o "$TMP"
node --check "$TMP" >/dev/null
grep -q 'R695-R2-RADIO-CLIPS-SAFE-FRAME-METAL-TITLES' "$TMP" || { echo 'СТОП: в источнике ещё не R695.'; exit 3; }
grep -q 'detectInsetBlackFrameCrop' "$TMP" || { echo 'СТОП: R695 safe-frame detector отсутствует.'; exit 3; }
grep -q 'cropdetect=limit=4:round=2:reset=0' "$TMP" || { echo 'СТОП: R695 black-inset detector отсутствует.'; exit 3; }
...
rollback(){
  echo 'R695 не подтвердился — возвращаю предыдущий server.mjs.'
  cp -f "$BACKUP" "$SERVER" || true
  systemctl restart "$SERVICE" || true
}
if ! systemctl restart "$SERVICE"; then rollback; exit 4; fi

echo '[4/5] Жду статус R695…'
STATUS=''
for i in $(seq 1 15); do
  sleep 3
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R695-R2-RADIO-CLIPS'; then break; fi
done
if ! printf '%s' "$STATUS" | grep -q 'R695-R2-RADIO-CLIPS'; then
  echo 'R695 не подтвердился за 45 секунд.'
