...
TMP_DIR="$(mktemp -d /tmp/andrik-r716.XXXXXX)"
DROPIN_DIR=/etc/systemd/system/andrik-radio.service.d
ENGINE_DROPIN="$DROPIN_DIR/r716-engine.conf"
...
ffmpeg -hide_banner -filters 2>/dev/null > "$TMP_DIR/ffmpeg-filters.txt"
for f in showfreqs drawgrid colorkey colorchannelmixer; do grep -q "$f" "$TMP_DIR/ffmpeg-filters.txt" || { echo "СТОП: FFmpeg без $f"; exit 2; }; done

echo '[1/10] Загружаю R716: R715 stable transport + ВИДИМЫЕ 4 audio EQ + AUTO LIVE…'
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/server.mjs?v=55.00-r716-$(date +%s)" -o "$TMP_DIR/server.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-radio-web-agent-r715.mjs?v=55.00-r716-$(date +%s)" -o "$TMP_DIR/agent.mjs"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/andrik-visual-auto-r703.sh?v=55.00-r716-$(date +%s)" -o "$TMP_DIR/auto.sh"
curl -fsSL --retry 5 --retry-all-errors --connect-timeout 15 --max-time 120 \
  "$SITE_BASE/radio247/vm-lite/force-full-fit-r712.sh?v=55.00-r716-$(date +%s)" -o "$TMP_DIR/fullfit.sh"
...
echo '[2/10] Проверяю DIRECT transport + видимый EQ без extra video input…'
grep -q 'R716-R715-STABLE-DIRECT-VISIBLE-4-AUDIO-EQ-AUTO-LIVE-R2' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R716'; exit 3; }
...
if grep -q "'-c:v','mjpeg'\|'pipe:4'\|normalVideoProducerArgs\|startNormalVisualProducer\|equalizer-.*r714.mov\|'-i',eq.path" "$TMP_DIR/server.mjs"; then
  echo 'СТОП: найден MJPEG/extra-video EQ — R716 должен иметь только прямой video clock.'; exit 3
fi
...
grep -q 'R716-VISIBLE-AUDIO-SHOWFREQS-4-SLOT-NO-EXTRA-VIDEO-INPUT' "$TMP_DIR/server.mjs" || { echo 'СТОП: R716 visible 4 audio EQ отсутствуют'; exit 3; }
grep -q 'showfreqs=s=\${eq.width}x\${eq.height}' "$TMP_DIR/server.mjs" || { echo 'СТОП: dynamic showfreqs EQ отсутствует'; exit 3; }
    grep -q 'drawgrid=w=\${eq.grid}:h=999:t=\${eq.gap}' "$TMP_DIR/server.mjs" || { echo 'СТОП: классические раздельные EQ bars отсутствуют'; exit 3; }
    grep -q 'overlay=x=(W-w)/2:y=H-h-192' "$TMP_DIR/server.mjs" || { echo 'СТОП: EQ не закреплён над названием'; exit 3; }
...
cp -a "$SERVER" "$SERVER.bak-r716-$TS"
[ -s "$AGENT_TARGET" ] && cp -a "$AGENT_TARGET" "$AGENT_TARGET.bak-r716-$TS" || true
[ -s "$AUTO_TARGET" ] && cp -a "$AUTO_TARGET" "$AUTO_TARGET.bak-r716-$TS" || true

echo '[4/10] Ставлю R716 server; Agent R715 AUTO LIVE сохраняю…'
...
rm -f "$DROPIN_DIR"/r70{1,2,3,4,5,6,7,8,9}-engine.conf "$DROPIN_DIR"/r71{0,1,2,3,4,5,6}-engine.conf 2>/dev/null || true
...
[Unit]
Description=ANDRIK four visual cycles scheduler R703/R716
...
echo '[6/10] Перезапускаю сохранённый Agent R715…'
...
echo '[9/10] Проверяю R716 DIRECT + visible EQ status…'
STATUS=''; OK=0
for i in $(seq 1 30); do
  sleep 2
  STATUS="$(curl -fsS --max-time 3 http://127.0.0.1:8080/status 2>/dev/null || true)"
  if printf '%s' "$STATUS" | grep -q 'R716-R715-STABLE-DIRECT-VISIBLE-4-AUDIO-EQ-AUTO-LIVE-R2'; then
    if printf '%s' "$STATUS" | python3 -c 'import sys,json; d=json.load(sys.stdin); raise SystemExit(0 if d.get("publisherRunning") and d.get("videoBitrate")=="4500k" and d.get("outputTimeshiftSeconds")==6 and d.get("engine")=="R678-R695-DIRECT-FFMPEG" and d.get("equalizerEngine")=="R716-VISIBLE-AUDIO-SHOWFREQS-4-SLOT-NO-EXTRA-VIDEO-INPUT" else 1)' 2>/dev/null; then OK=1; break; fi
  fi
done
[ "$OK" = 1 ] || {
  echo 'R716 radio не поднялся как DIRECT + visible EQ engine.'
...
echo
echo '======================================================'
echo '✅ R716 ГОТОВ · R715 STABLE DIRECT TRANSPORT СОХРАНЁН'
echo '✅ 1080p25 · 4500k CBR · 9000k VBV · GOP 50'
echo '✅ FIFO 8192 · timeshift 6 s · НИКАКОГО extra video input для EQ'
echo '✅ 4 ВИДИМЫХ EQ над названием: УТРО gold 48 · ДЕНЬ steel 64 · ВЕЧЕР amber 44 · НОЧЬ blue 36'
echo '✅ Красная линия и полупрозрачная подложка за названием удалены'
echo '✅ MP3 hotfix 234 + R2 клипы/delete + FULL FIT сохранены'
echo '✅ Agent R715 AUTO LIVE сохранён и продолжает восстанавливать реальный YouTube LIVE после обрыва'
echo '======================================================'
