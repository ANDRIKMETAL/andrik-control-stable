...
ENGINE_DROPIN="$DROPIN_DIR/r704-engine.conf"
...
echo '[1/6] Загружаю R704 equalizer engine…'
...
grep -q 'R704-FOUR-LIVE-EQUALIZERS-R703-PRESERVED' "$TMP_DIR/server.mjs" || { echo 'СТОП: server не R704'; exit 3; }
grep -q 'liveEqualizerFiltersR704' "$TMP_DIR/server.mjs" || { echo 'СТОП: нет живого equalizer engine'; exit 3; }
grep -q "morning-soft-gold" "$TMP_DIR/server.mjs" || { echo 'СТОП: нет MORNING equalizer'; exit 3; }
grep -q "day-steel" "$TMP_DIR/server.mjs" || { echo 'СТОП: нет DAY equalizer'; exit 3; }
grep -q "evening-amber" "$TMP_DIR/server.mjs" || { echo 'СТОП: нет EVENING equalizer'; exit 3; }
grep -q "night-blue" "$TMP_DIR/server.mjs" || { echo 'СТОП: нет NIGHT equalizer'; exit 3; }
grep -q "baseOffset=84" "$TMP_DIR/server.mjs" || { echo 'СТОП: equalizer position guard missing'; exit 3; }
...
mkdir -p "$DROPIN_DIR"
rm -f "$DROPIN_DIR/r702-engine.conf" "$DROPIN_DIR/r703-engine.conf"
...
for i in $(seq 1 30); do
  ...
  if printf '%s' "$STATUS" | grep -q 'R704-FOUR-LIVE-EQUALIZERS-R703-PRESERVED'; then
...
echo '✅ Линия расположена между названием песни и бегущей строкой.'
echo '✅ Движение плавное и постоянное, не зависит от MP3 и не вмешивается в аудио.'
