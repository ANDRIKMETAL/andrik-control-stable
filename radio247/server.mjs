...
function overlayKey(previous,item,next){
  const raw=[
...
function pruneOverlayCache(keepPath=''){
  try{
    const files=readdirSync(CACHE_DIR)
      .filter(name=>/^overlay-[a-f0-9]+\.mp4$/i.test(name))
      .map(name=>{
    ...
    for(const entry of files.slice(OVERLAY_CACHE_LIMIT)){
      if(entry.path===keepPath)continue;
      try{unlinkSync(entry.path);}catch(_){ }
    }
  }catch(_){ }
}
...
async function prepareOverlayVisual(previous,item,next){
  ...
  const promise=(async()=>{
    state.overlayPreparing=true;
    const cleanVisual=await ensureCleanVisual();
    const font=chooseFont();
    const previousFile=`${CACHE_DIR}/prev-${key}.txt`;
    const currentFile=`${CACHE_DIR}/current-${key}.txt`;
    const nextFile=`${CACHE_DIR}/next-${key}.txt`;
    const tickerFile=`${CACHE_DIR}/ticker-${key}.txt`;
    ...
    const curPath=ffFilterPath(currentFile);
    ...
    try{
      ...
      for(const path of [previousFile,currentFile,nextFile,tickerFile]){
        try{unlinkSync(path);}catch(_){ }
      }
      state.overlayPreparing=false;
    }
  })().catch(error=>{
    state.overlayPreparing=false;
    state.lastError=`Overlay fallback: ${String(error?.message||error).slice(-700)}`;
    console.error('[overlay]',error);
    return ensureCleanVisual().catch(()=>AUDIO_VISUAL);
  }).finally(()=>overlayPromises.delete(key));
...
function publicStatus(){
  ...
  return {
    ...
    overlayPreparing:state.overlayPreparing,
