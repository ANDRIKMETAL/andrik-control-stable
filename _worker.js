const ANDRIK_CONTROL_RELEASE = Object.freeze({ short:'R496', number:496, version:'55.00', full:'55.00 LIVE WEB AI FINAL R496', siteUpdater:'55.00-r356' });
...
async function handleYoutubeOAuthCallback(request, env, ctx) {
  ...
  return Response.redirect('https://control.andrikmetal.com/social-center-admin.html?youtube=connected&v=55.00-r496', 302);
}
...
async function handleControlSocialOverviewR487(request, env) {
  ...
  if (refresh || !gaRow || latestSnapshotAgeMinutesR487(gaRow) > 30 || !Array.isArray(google?.trend) || !google.trend.length) {
    try {
      ...
      if (live?.configured) {
        ...
        await savePlatformSnapshot(db, 'google-analytics', metrics, refresh ? 'manual-social-center-r496' : 'social-center-r496');
  ...
  if (shouldRefreshYoutube) {
    try {
      ...
      if (studio?.connected) {
        ...
        await savePlatformSnapshot(db, 'youtube', metrics, refresh ? 'manual-social-center-r496' : 'social-center-r496');
  ...
  if (igConfig.configured && (refresh || !igRow || latestSnapshotAgeMinutesR487(igRow) > 30 || !instagram?.connected || !Array.isArray(instagram?.trend))) {
    try {
      ...
      if (live?.configured) {
        await savePlatformSnapshot(db, 'instagram', live, refresh ? 'manual-social-center-r496' : 'social-center-r496');
