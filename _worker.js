...
function detectDeviceCategoryR536(request) {
  const mobileHint=cleanPlainText(request?.headers?.get?.('sec-ch-ua-mobile')||'',8).trim();
  const ua=cleanPlainText(request?.headers?.get?.('user-agent')||'',500).toLowerCase();
  ...
  const [trendResult,countryResult,pageResult,deviceResult,monthRow,weekRow] = await Promise.all([
    ...
    db.prepare(`
      SELECT device_category AS deviceCategory,
             COUNT(DISTINCT visitor_hash) AS activeUsers,
             COUNT(*) AS screenPageViews
      FROM site_visit_events
      WHERE event_type='visit' AND local_date>=?1 AND local_date<=?2
        AND device_category<>''
        AND path NOT LIKE '/cache-reset%' AND path NOT LIKE '/site-cache-reset%'
        AND path NOT LIKE '%-admin.html' AND path NOT LIKE '/admin/%'
        AND path NOT IN ('/offline.html','/open-youtube.html','/comment-collection.html')
      GROUP BY device_category ORDER BY activeUsers DESC, screenPageViews DESC
    `).bind(startDate,endDate).all().catch(()=>({results:[]})),
    db.prepare(`
...
function mergeGoogleWithFirstPartyR530(google={}, firstParty={}) {
  ...
  const devices=[...deviceMap.values()].filter(row=>row.activeUsers>0||row.sessions>0||row.screenPageViews>0).sort((a,b)=>Number(b.activeUsers||0)-Number(a.activeUsers||0)||Number(b.sessions||0)-Number(a.sessions||0));
  return {
    ...(google||{}),
...
async function handleControlEcosystemMap(request, env) {
  ...
  // R452 — preserve the R451 server-side all-time country memory for the interactive ALL map.
  // R450 kept all-time first-party site/push geography; R451 added
  ...
  const historyCountryMapR452 = new Map();
  for (const row of [...normalizeCountries(historySiteCountriesRaw), ...normalizeCountries(historyPushCountriesRaw)]) {
    if (!row.country) continue;
    historyCountryMapR452.set(row.country,{country:row.country,value:1,source:'first-party-history'});
  }
  const youtubeKnownStateR452 = await getPushState(db, 'youtube-audience-known-countries-v54-24').catch(() => null);
  let youtubeKnownCountriesR452 = [];
  try {
    const parsed = JSON.parse(youtubeKnownStateR452?.value || '[]');
    youtubeKnownCountriesR452 = Array.isArray(parsed) ? parsed : [];
  } catch (_) { youtubeKnownCountriesR452 = []; }
  for (const raw of youtubeKnownCountriesR452) {
    const country = cleanPlainText(raw || '', 120).trim();
    if (!country || country === '(не задано)') continue;
    const key = country.toLocaleUpperCase('ru');
    if (!historyCountryMapR452.has(key)) historyCountryMapR452.set(key,{country,value:1,source:'youtube-known-history'});
  }
...
function trafficSourceLabelR438(source = '', medium = '', channel = '') {
  ...
  if (/amazon/.test(hay)) return { key:'amazon', label:'Amazon', icon:'📦' };
  if (/apple/.test(hay)) return { key:'apple', label:'Apple Music', icon:'' };
  if (/google/.test(hay)) return { key:'google', label:rawMedium === 'organic' ? 'Google · поиск' : 'Google', icon:'🔎' };
  if (/bing/.test(hay)) return { key:'bing', label:rawMedium === 'organic' ? 'Bing · поиск' : 'Bing', icon:'🔎' };
  if (/facebook|instagram|threads|tiktok|x\.com|twitter/.test(hay)) return { key:'social', label:'Соцсети', icon:'💬' };
  if (/organic search/i.test(rawChannel)) return { key:'search', label:'Поиск', icon:'🔎' };
  if (/referral/i.test(rawMedium) || /referral/i.test(rawChannel)) return { key:'referral', label:rawSource || 'Переход по ссылке', icon:'🔗' };
...
async function firstPartyCitySourcesR438(db, { country='', city='', region='', range } = {}) {
  ...
  const result = await db.prepare(`
    SELECT
      ...
      CASE WHEN acquisition_medium<>'' THEN acquisition_medium ELSE '' END AS medium,
      MAX(referrer_host) AS referrerHost,
      COUNT(*) AS events,
    ...
    LIMIT 16
  `).bind(code, range.startDate, range.endDate, placeValue).all();
...
async function ga4CitySourcesR438(env, { country='', city='', region='', range } = {}) {
  ...
  const accessToken = await getGoogleAnalyticsAccessToken(env);
  const property = await resolveGoogleAnalyticsProperty(env, accessToken);
  const placeField = city ? 'city' : 'region';
  const placeValue = cleanPlainText(city || region || '', 120);
  const report = await googleAnalyticsPost(accessToken, property.id, 'runReport', {
    ...
    dimensionFilter:{
      andGroup:{ expressions:[
        { filter:{ fieldName:'countryId', stringFilter:{ matchType:'EXACT', value:country, caseSensitive:false } } },
        { filter:{ fieldName:placeField, stringFilter:{ matchType:'EXACT', value:placeValue, caseSensitive:false } } }
      ] }
    },
    orderBys:[{ metric:{ metricName:'sessions' }, desc:true }],
    keepEmptyRows:false,
  ...
  return {
    ...
    propertyName:property.name,
    rows,
    totalSessions:rows.reduce((sum,row)=>sum+Math.max(0,Number(row.sessions||0)),0),
    totalUsers:rows.reduce((sum,row)=>sum+Math.max(0,Number(row.activeUsers||0)),0)
  };
}
...
// R619 — one-time top-level OAuth bridge. The Control/PWA never opens the
// accounts.google.com URL directly; Android can hand this same-origin URL to Chrome,
// then the Worker performs a normal 302 to Google. Ticket expires after 15 minutes.
async function makeYoutubeOAuthBridgeTicketR619(env) {
  const payload = base64UrlEncodeText(JSON.stringify({ iat:Date.now(), nonce:crypto.randomUUID(), purpose:'youtube-oauth-bridge-r619' }));
  const key = await youtubeOAuthHmacKey(env);
  const signature = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(payload));
  return `${payload}.${base64UrlEncodeBytes(signature)}`;
}

async function verifyYoutubeOAuthBridgeTicketR619(env, ticket) {
  ...
  try {
    const data = JSON.parse(new TextDecoder().decode(base64UrlDecodeBytes(payload)));
    return data?.purpose === 'youtube-oauth-bridge-r619' && Number(data.iat || 0) > Date.now() - 15 * 60 * 1000;
  } catch (_) { return false; }
}

function youtubeOAuthAuthorizationUrlR619(env, state) {
  ...
  // Full fresh consent replaces the old read-only refresh token.
...
async function handleYoutubeOAuthBridgeR619(request, env) {
  const url = new URL(request.url);
  const ticket = url.searchParams.get('ticket') || '';
  const config = youtubeOAuthClient(env, request);
  if (!config.configured) return new Response('YouTube OAuth client is not configured', {status:409,headers:{'content-type':'text/plain; charset=utf-8','cache-control':'no-store'}});
  if (!await verifyYoutubeOAuthBridgeTicketR619(env, ticket)) return new Response('OAuth bridge link expired. Open Control and press reconnect again.', {status:403,headers:{'content-type':'text/plain; charset=utf-8','cache-control':'no-store'}});
  const state = await makeYoutubeOAuthState(env);
  return Response.redirect(youtubeOAuthAuthorizationUrlR619(env, state).toString(), 302);
}
...
async function handleYoutubeOAuthStart(request, env) {
  ...
  const ticket = await makeYoutubeOAuthBridgeTicketR619(env);
  const bridgeUrl = `https://andrikmetal.com/oauth/youtube/connect?ticket=${encodeURIComponent(ticket)}`;
  return json({ ok:true, url:bridgeUrl, bridgeUrl, redirectUri:config.redirectUri, mode:'chrome-top-level-r619' });
}
...
async function tiktokOAuthStatusR503(env, mode = 'sandbox') {
  ...
  return {
    ...
    scope:cleanPlainText(row?.scope || '',1200),
...
async function getTikTokAccessSessionR503(env, mode = 'sandbox') {
  ...
  const response = await fetchWithAbortTimeoutR409('https://open.tiktokapis.com/v2/oauth/token/', {
    method:'POST', headers:{'content-type':'application/x-www-form-urlencoded','cache-control':'no-cache'}, body
...
async function fetchTikTokAnalyticsR503(env, mode = 'sandbox') {
  ...
  if (granted.has('user.info.stats')) userFields.push('follower_count','following_count','likes_count','video_count');
  const userUrl = new URL('https://open.tiktokapis.com/v2/user/info/');
  userUrl.searchParams.set('fields', userFields.join(','));
  let user = {};
  let userError = '';
  try {
  ...
  if (granted.has('video.list')) {
    try {
      ...
      for (let page = 0; page < 5; page++) {
        ...
        if (nextCursor === undefined || nextCursor === null || nextCursor === cursor) break;
        cursor = nextCursor;
      }
    } catch (error) { videoError = cleanPlainText(error?.message || error,300); }
  }
  if (userError && videoError) throw new Error(`TikTok API: ${userError} · ${videoError}`);
  const sum = field => videos.reduce((total,row)=>total+Math.max(0,Number(row?.[field]||0)),0);
  const summary = {
    followerCount:user?.follower_count === undefined ? null : Math.max(0,Number(user.follower_count||0)),
    followingCount:user?.following_count === undefined ? null : Math.max(0,Number(user.following_count||0)),
    profileLikes:user?.likes_count === undefined ? null : Math.max(0,Number(user.likes_count||0)),
    videoCount:user?.video_count === undefined ? null : Math.max(0,Number(user.video_count||0)),
    fetchedVideos:videos.length,
    allVideosFetched:!hasMore,
    totalVideoViews:sum('view_count'),
    totalVideoLikes:sum('like_count'),
  ...
  return {
    ...
    handle:'@andrikmetal',
    profileUrl:'https://www.tiktok.com/@andrikmetal',
    avatarUrl:cleanPlainText(user?.avatar_url || '',900),
...
async function handleTikTokOAuthCallbackR503(request, env) {
  ...
  const response = await fetchWithAbortTimeoutR409('https://open.tiktokapis.com/v2/oauth/token/', {
    method:'POST', headers:{'content-type':'application/x-www-form-urlencoded','cache-control':'no-cache'}, body
  }, 10000, 'tiktok-token-exchange-timeout');
  const data = await response.json().catch(()=>({}));
...
async function handleCheckYoutubeEvents(request, env) {
  ...
  try {
    ...
    if (settled[2].status === 'fulfilled') {
      ...
      // R468: keep a durable 06:05 baseline for BOTH views and aggregate likes.
      // This makes future active-day counters exact even if older snapshots are pruned.
      await captureYoutubeLiveRolloverR468(db, {
        views:Number(identity.views || 0),
        likesTotal:aggregateLikes
...
async function buildPlatformControlData(env, google = {}, youtube = {}, searchConsole = {}, instagram = {}) {
  const db = requireDb(env);
  ...
  return {
    ...
    platforms: [
      ...
      { id:'instagram', name:'Instagram', icon:'📸', status:instagram?.connected?'live':(instagramConfigR487(env).configured?'configured':'foundation'), label:instagram?.connected?'Insights · прямой Instagram API':'Добавьте INSTAGRAM_ACCESS_TOKEN в Worker Secret', metric:instagram?.connected?Number(instagram?.summary?.views||0):null, metricLabel:instagram?.connected?'просмотров · 28 дней':'views · reach · interactions', url:'https://www.instagram.com/andrikmetal/' },
...
// R487 — direct Instagram professional-account analytics via Instagram Login.
// The access token stays server-side in the Cloudflare Worker secret INSTAGRAM_ACCESS_TOKEN.
function instagramConfigR487(env) {
  const token = String(env.INSTAGRAM_ACCESS_TOKEN || env.IG_ACCESS_TOKEN || '').trim();
  const rawVersion = cleanPlainText(env.INSTAGRAM_API_VERSION || 'v26.0', 20);
  const version = /^v\d+\.\d+$/.test(rawVersion) ? rawVersion : 'v26.0';
  return { token, version, configured:Boolean(token) };
}

async function instagramApiGetR487(env, path, params = {}, timeoutMs = 8000) {
  const config = instagramConfigR487(env);
  if (!config.configured) throw new Error('instagram-access-token-not-configured');
  const safePath = String(path || '').replace(/^\/+/, '');
  const url = new URL(`https://graph.instagram.com/${config.version}/${safePath}`);
  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== '') url.searchParams.set(key, String(value));
  });
  const response = await fetchWithAbortTimeoutR409(url.toString(), {
    headers:{ authorization:`Bearer ${config.token}`, accept:'application/json' }
  }, timeoutMs, 'instagram-api-timeout');
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data?.error?.message || data?.error?.error_user_msg || `instagram-api-${response.status}`);
  return data;
}

function instagramInsightScalarR487(payload) {
  const item = payload?.data?.[0] || {};
  const direct = item?.total_value?.value;
...
function instagramInsightTrendR487(payload, metricName = '') {
  const item = (payload?.data || []).find(row => String(row?.name || '') === metricName) || payload?.data?.[0] || {};
...
function latestSnapshotAgeMinutesR487(row) {
...
async function refreshControlSnapshots(env, { force = false } = {}) {
  ...
  const [last, latestYoutubeSnapshot, latestInstagramSnapshot] = await Promise.all([
    ...
    db.prepare(`SELECT metrics_json FROM platform_snapshots WHERE platform='instagram' ORDER BY created_at DESC LIMIT 1`).first()
  ]);
  ...
  if (results[2].status === 'fulfilled' && results[2].value?.configured) {
    const ig = results[2].value;
    await savePlatformSnapshot(db, 'instagram', ig, 'Instagram Insights API · Instagram Login');
    stored.push('instagram');
  } else if (results[2].status === 'rejected') {
    errors.push(`Instagram: ${cleanPlainText(results[2].reason?.message || results[2].reason, 260)}`);
  }

  if (results[3].status === 'fulfilled') {
    const sc = results[3].value || {};
    ...
    errors.push(`Search Console: ${cleanPlainText(results[3].reason?.message || results[3].reason, 260)}`);
  }
  ...
  return {
    ...
    searchConsole:results[3].status === 'fulfilled' ? results[3].value : null
  };
}
...
// R468: live "today" cards are NOT push-event counters. They are cumulative
// YouTube channel totals minus the rollover snapshot closest to 06:05 Bratislava.
// This prevents old push high-water (for example 69 likes) from polluting the live
// card and avoids the R410 current-fallback baseline that could freeze views at 0.
// Completed/archive windows still use the exact YouTube Analytics day row from R467.
function parseYoutubeLiveBaselineR468(row, windowKey='') {
  if (!row?.value || !windowKey) return null;
  try {
    const parsed=JSON.parse(row.value || '{}') || {};
    if (cleanPlainText(parsed.windowKey || '',20)!==windowKey) return null;
    const views=Number(parsed.views), likesTotal=Number(parsed.likesTotal);
    if (!Number.isFinite(views) || views<0 || !Number.isFinite(likesTotal) || likesTotal<0) return null;
    return {
      views, likesTotal,
      capturedAt:cleanPlainText(parsed.capturedAt || row.updatedAt || '',80),
      source:cleanPlainText(parsed.source || 'youtube-live-baseline-r468',80)
    };
  } catch (_) { return null; }
}

async function persistYoutubeLiveBaselineR468(db, window, metrics={}, capturedAt='', source='rollover-r468', {replace=false}={}) {
  if (!window?.key) return null;
  const views=Number(metrics?.views), likesTotal=Number(metrics?.likesTotal);
  if (!Number.isFinite(views) || views<0 || !Number.isFinite(likesTotal) || likesTotal<0) return null;
  const key=`youtube-live-baseline-r468:${window.key}`;
  const current=parseYoutubeLiveBaselineR468(await getPushState(db,key).catch(()=>null),window.key);
  if (current && !replace) return current;
  const payload={
    windowKey:window.key,
    views:Math.max(0,views),
    likesTotal:Math.max(0,likesTotal),
    capturedAt:cleanPlainText(capturedAt || new Date().toISOString(),80),
    source:cleanPlainText(source || 'r468',80),
    savedAt:new Date().toISOString()
  };
  await setPushState(db,key,JSON.stringify(payload)).catch(()=>{});
  await db.prepare(`DELETE FROM push_state WHERE key LIKE 'youtube-live-baseline-r468:%' AND key != ? AND updated_at < datetime('now','-4 days')`).bind(key).run().catch(()=>{});
  return payload;
}

async function captureYoutubeLiveRolloverR468(db, metrics={}, capturedAt='') {
  ...
  if (!Number.isFinite(startMs) || ageMs<0 || ageMs>20*60*1000) return null;
  return persistYoutubeLiveBaselineR468(db,window,metrics,at,'live-rollover-r468');
}

async function youtubeLiveWindowMetricsR468(db, window, currentMetrics = {}, baselineBeforeRow = null, baselineAfterRow = null) {
  if (!window?.key || window?.completed) return null;
  const startMs = Date.parse(window.startAt || '');
  if (!Number.isFinite(startMs)) return null;

  let baseline=parseYoutubeLiveBaselineR468(
    await getPushState(db,`youtube-live-baseline-r468:${window.key}`).catch(()=>null),
    window.key
  );
  ...
  if (!baseline) {
    const candidates = [baselineBeforeRow, baselineAfterRow].map(row => {
      ...
      const likesTotal = Number(metrics?.likesTotal);
      return {
        at, atMs, views, likesTotal,
        distance: Number.isFinite(atMs) ? Math.abs(atMs - startMs) : Number.POSITIVE_INFINITY
      };
    }).filter(item =>
      Number.isFinite(item.atMs) &&
      Number.isFinite(item.views) && item.views >= 0 &&
      Number.isFinite(item.likesTotal) && item.likesTotal >= 0
    ).sort((a,b) => a.distance - b.distance);

    const nearest = candidates[0] || null;
    // Normal snapshot cadence should put a point close to 06:05. Refuse a many-hours
    // old point: a wrong baseline is worse than temporarily showing the safe fallback.
    if (!nearest || nearest.distance > 90 * 60 * 1000) return null;
    baseline=await persistYoutubeLiveBaselineR468(
      db,window,
      {views:nearest.views,likesTotal:nearest.likesTotal},
      nearest.at,
      'nearest-0605-snapshot-r468',
      {replace:true}
    );
  }
  ...
  const currentLikes = Number(currentMetrics?.likesTotal);
  if (!Number.isFinite(currentViews) || currentViews < baseline.views) return null;
  if (!Number.isFinite(currentLikes) || currentLikes < baseline.likesTotal) return null;

  return {
    day:window.key,
    likes:Math.max(0, currentLikes - baseline.likesTotal),
    views:Math.max(0, currentViews - baseline.views),
    baselineAt:baseline.capturedAt,
    source:'youtube-cumulative-0605-r468'
  };
}

function applyYoutubeAuthoritativeWindowR468(summary = {}, metrics = {}, windowKey = '') {
  const result = {...(summary || {})};
  const safeKey = cleanPlainText(windowKey || '', 20);
  if (metrics?.youtubeDailyExact === true && cleanPlainText(metrics?.youtubeDailyDate || '',20) === safeKey) {
    result.youtubeLikes = Math.max(0, Number(metrics?.youtubeLikes || 0));
    result.youtubeViews = Math.max(0, Number(metrics?.youtubeViewDelta ?? metrics?.youtubeViews ?? 0));
    result.youtubeViewDelta = result.youtubeViews;
    return result;
  }
  if (metrics?.youtubeLiveWindowR468 === true && cleanPlainText(metrics?.youtubeLiveWindowDate || '',20) === safeKey) {
    result.youtubeLikes = Math.max(0, Number(metrics?.youtubeLikes || 0));
    result.youtubeViews = Math.max(0, Number(metrics?.youtubeViewDelta ?? metrics?.youtubeViews ?? 0));
    result.youtubeViewDelta = result.youtubeViews;
  }
...
async function collectDailyOwnerSummary(env, { liveExternal = true, windowOverride = null } = {}) {
  ...
  const youtubeViewSnapshotDeltaR468 = ytBaseline && Number.isFinite(Number(ytNow.views)) && Number.isFinite(Number(ytStart.views))
    ? Math.max(0, Number(ytNow.views || 0) - Number(ytStart.views || 0)) : youtubeViewDeltaR410;
...
async function collectDailyOwnerSummaryFallback(env, window, reason = '') {
  ...
  return {
    ...
    // R468: push notifications are events, not an authoritative daily-like counter.
    youtubeLikes:0,
...
async function handleControlSummaryAutoRefreshR403(request, env) {
  ...
  try {
    ...
    let summary=mergeControlHomeSummaryR213(storedSummary,highWater.summary,resultSummary);
    summary=applyYoutubeAuthoritativeWindowR468(summary,result?.metrics || {},window.key);
...
async function handleExternalCronGatewayR334(request, env, ctx) {
  ...
  try{
    if(clock.due2){
      ...
      if(claimed){
        const local15=getBratislavaClock();
        // R430 redundant delivery path: the dedicated */15 invocation can rescue
        // a missed 05:xx/17:xx summary without combining it with the normal
        // checkpoint. D1 send claims prevent a duplicate if */5 already succeeded.
        if(local15.hour===5 || local15.hour===17){
          task='daily-summary-fallback';
          try{ value=await maybeSendDailyOwnerSummary(env); }
          catch(error){ value={ok:false,error:cleanPlainText(error?.message||error,500)}; }
        }else{
          ...
          await Promise.all([
            ...
            setPushState(db,'automation-last-check-summary',JSON.stringify({mode:'cpu-safe-r430',task:'summary-checkpoint',at,value}).slice(0,12000)).catch(()=>{})
          ]);
        }
      }else value={ok:true,skipped:true,reason:'slot-already-claimed'};
    }
...
function parseYoutubeCountsCheckpointR440(row) {
  ...
  try {
    ...
    const likesTotal=Number(parsed.likesTotal);
    const commentsTotal=Number(parsed.commentsTotal);
    ...
    return {
      ...
      likesTotal:Number.isFinite(likesTotal) && likesTotal >= 0 ? likesTotal : null,
      commentsTotal:Number.isFinite(commentsTotal) && commentsTotal >= 0 ? commentsTotal : null,
      updatedAt,
...
async function mergeYoutubeCurrentCountsR440(db, snapshotMetrics={}, snapshotAt='', window=null) {
  ...
  // R468: the 5-minute YouTube event cycle already persists aggregate channel
  // likes/comments here. Merge them too so the live daily cards do not wait for
  // the slower platform snapshot refresh.
  if (checkpoint.likesTotal !== null) base.likesTotal=checkpoint.likesTotal;
  if (checkpoint.commentsTotal !== null) base.commentsTotal=checkpoint.commentsTotal;
  base.__currentCountsSource='youtube-cron-r468';
...
async function persistControlHomeHighWaterFromMetricsR260(db, windowKey, metrics = {}) {
  ...
  let merged = mergeControlHomeSummaryR213(previous, incoming);
  // R468: YouTube daily/live values are authoritative and are allowed to DECREASE
  // a polluted historical high-water value.
  merged = applyYoutubeAuthoritativeWindowR468(merged, metrics, windowKey);
  await setPushState(db, key, JSON.stringify({
    windowKey,
    summary:merged,
    updatedAt:new Date().toISOString(),
    source:'daily-accumulator-r468',
    youtubeAuthoritativeR468:Boolean(
      (metrics?.youtubeDailyExact === true && cleanPlainText(metrics?.youtubeDailyDate || '',20) === windowKey) ||
      (metrics?.youtubeLiveWindowR468 === true && cleanPlainText(metrics?.youtubeLiveWindowDate || '',20) === windowKey)
    )
  })).catch(() => {});
}
...
async function handleControlHome(request, env) {
  ...
  if (!forceRefresh) {
    ...
    let mergedFastSummaryR395 = mergeControlHomeSummaryR213(pushSummaryR395, highWaterR395.summary);
    ...
    // R468: once the current-window accumulator is newer than the last push snapshot,
    // never let an old push summary resurrect inflated YouTube likes/views.
    if (accumulatorIsCurrentR395) {
      mergedFastSummaryR395.youtubeLikes = Math.max(0, Number(highWaterR395.summary?.youtubeLikes || 0));
      mergedFastSummaryR395.youtubeViews = Math.max(0, Number(highWaterR395.summary?.youtubeViews || 0));
      mergedFastSummaryR395.youtubeViewDelta = Math.max(0, Number(highWaterR395.summary?.youtubeViewDelta || highWaterR395.summary?.youtubeViews || 0));
    }
  ...
  const youtubeViewSnapshotDeltaR468 = ytBaseline && Number.isFinite(Number(ytNow.views)) && Number.isFinite(Number(ytStart.views))
    ? Math.max(0, Number(ytNow.views || 0) - Number(ytStart.views || 0)) : youtubeViewDelta;
  const youtubeSubscriberDelta = ytBaseline ? Math.max(0, Number(ytNow.subscribers || 0) - Number(ytStart.subscribers || 0)) : 0;
  const youtubeLikeSnapshotDelta = ytBaseline && Number.isFinite(Number(ytNow.likesTotal)) && Number.isFinite(Number(ytStart.likesTotal))
    ? Math.max(0, Number(ytNow.likesTotal || 0) - Number(ytStart.likesTotal || 0)) : 0;
  const youtubeCommentSnapshotDelta = ytBaseline && Number.isFinite(Number(ytNow.commentsTotal)) && Number.isFinite(Number(ytStart.commentsTotal))
  ...
  // R468: both exact archive values and the active 06:05 cumulative delta are
...
function deviceQualityR536(devices=[]) {
...
async function recoverRecentDeviceRowsR536(db, current={}) {
  const currentQ=deviceQualityR536(current?.devices||[]);
  const monthUsers=Math.max(0,Number(current?.month?.activeUsers||0));
  const suspicious=(monthUsers>=5 && currentQ.users<Math.max(2,Math.floor(monthUsers*0.15))) || (monthUsers>=8 && currentQ.categories<2);
  if(!suspicious)return current;
  const rows=await db.prepare(`
    SELECT metrics_json, created_at
    FROM platform_snapshots
    WHERE platform='google-analytics' AND datetime(created_at)>=datetime('now','-14 days')
    ORDER BY datetime(created_at) DESC LIMIT 24
  `).all().catch(()=>({results:[]}));
  const propertyId=cleanPlainText(current?.propertyId||'',80);
  let best=null,bestQ=currentQ,bestAt='';
  for(const row of rows?.results||[]){
    const snap=parseSnapshotMetrics(row);
    if(propertyId&&snap?.propertyId&&cleanPlainText(snap.propertyId,80)!==propertyId)continue;
    const q=deviceQualityR536(snap?.devices||[]);
    if(q.categories<2 && q.users<=bestQ.users)continue;
    const score=q.categories*100000+q.users*100+q.views;
    const bestScore=bestQ.categories*100000+bestQ.users*100+bestQ.views;
    if(score>bestScore){best=snap;bestQ=q;bestAt=cleanPlainText(row?.created_at||'',80);}
  }
  if(!best?.devices?.length)return current;
  return {...current,devices:best.devices,deviceRecoveryAt:bestAt,deviceRecoverySource:'recent-ga4-snapshot-r536'};
}
...
async function handleControlGoogleAnalytics(request, env) {
  ...
  google=await recoverRecentDeviceRowsR536(db,google);
...
// R552 — focused YouTube top content.
 // One official Top videos report returns video + creatorContentType + the counters we need.
 // The main YouTube dashboard stays independent: this endpoint is used only by the two top pages.
function normalizeYoutubeTopContentR552(row = {}, type = '') {
  const videoId=cleanPlainText(row?.videoId||row?.video||'',80);
  return {
    videoId,
    creatorContentType:cleanPlainText(row?.creatorContentType||type||'',40).toUpperCase(),
    views:Math.max(0,Number(row?.views||0)),
...
async function refreshYoutubeTopContentR552(env) {
  ...
  const addKnownMeta=item=>{
    const id=cleanPlainText(item?.videoId||item?.id||'',80);
    if(!id) return;
...
async function handleControlYoutubeTopContentR552(request, env) {
  ...
  const url=new URL(request.url);
  const wanted=url.searchParams.get('type')==='videos'?'videos':'shorts';
  ...
  const latestRow=await db.prepare(`
    ...
    LIMIT 1
  `).first().catch(()=>null);
  const snap=parseSnapshotMetrics(latestRow);
  const studio=snap?.studio||{};

  const cachedShorts=(Array.isArray(studio.topShorts28)?studio.topShorts28:[])
  ...
  try{
    const fresh=await Promise.race([
      refreshYoutubeTopContentR552(env),
      new Promise((_,reject)=>setTimeout(()=>reject(new Error('youtube-top-r552-timeout')),18000))
    ]);
    return json({
      ok:true,
      available:Boolean((wanted==='videos'?fresh.videos:fresh.shorts).length),
      source:'youtube-top-live-r552',
      ...fresh
    });
  }catch(error){
    if(wantedCached.length){
      return json({
        ok:true,
        available:true,
...
async function handleControlAudience(request, env) {
  ...
  const instagram = igRow ? {
    ...instagramSnapshot,
    configured:Boolean(instagramSnapshot.configured),
    connected:Boolean(instagramSnapshot.connected),
    updatedAt:igRow.created_at || instagramSnapshot.updatedAt || ''
  } : { configured:instagramConfigR487(env).configured, connected:false, summary:{}, trend:[] };

  let control = { today:{}, platforms:[], searchEngines:[], albums:[], youtubeMonitor:{}, recentEvents:[] };
  try { control = await buildPlatformControlData(env, google, youtube, searchConsole, instagram); }
...
// R616 — canonical public single title. Users often paste a filename into the
// title field (for example "Жизнь идёт сама.mp3"). Keep storage forgiving,
// but never expose the extension as part of the song title.
function musicSingleTitleR616(value){
  return cleanPlainText(String(value||'').replace(/(?:\.(?:mp3|wav))+$/ig,'').trim(),220);
}
...
async function handleMusicMp3PutR314(request, env) {
  ...
  const metadata={
    source:'ANDRIK Control R616',
    title:musicSingleTitleR616(musicHeaderR317(request,'x-andrik-track-title')),
...
const MUSIC_ALBUMS_R446 = Object.freeze({
  ...
  'illusion-of-life': Object.freeze({
    ...
    zipName:'ANDRIK-Illusion-of-Life-MP3.zip'
  }),
  'ocean': Object.freeze({
...
function musicAlbumDuplicateScoreR446(object){
  ...
  if(title)score+=20;
  if(!/track[_ -]?\d+/i.test(title))score+=5;
...
async function musicAlbumStatusOneR446(bucket,def,includeTracks=true){
  ...
  return {
    slug:def.slug,label:def.label,prefix:def.prefix,
    sourceTrackCount:source.length,trackCount:tracks.length,totalBytes,
...
async function publishSingleReleaseR616(env,{key,title,url='',publishedAt=''}={}){
  const safeKey=musicObjectKeyR317(key);
  if(!safeKey||!/^singles\//i.test(safeKey))return {ok:false,error:'invalid-single-key'};
  ...
  const onceKey=`push-once:single-release:${safeKey}`;
  let claimed=false;
  if(db){
    try{
      claimed=await claimPushOnce(db,onceKey,safeTitle);
      if(!claimed)return {ok:true,alreadyPublished:true,key:safeKey,title:safeTitle,url:siteUrl};
    }catch(_){claimed=false}
  }
  const result=await sendOneSignalPush(env,{
    title:'🎵 Новый сингл ANDRIK',
    message:`«${safeTitle}» уже на сайте — слушать и скачать MP3 бесплатно.`,
    url:siteUrl,
    audience:'all',
    name:`single-release-${safeKey}`,
    history:{type:'single-release',source:safeKey,details:{key:safeKey,publishedAt:cleanPlainText(publishedAt,80),audioUrl:cleanPlainText(url,700)}}
  });
  if(!result.ok&&db&&claimed)await releasePushOnceClaim(db,onceKey).catch(()=>{});
  if(result.ok&&db)await setPushState(db,'latest-single-release',JSON.stringify({key:safeKey,title:safeTitle,publishedAt:cleanPlainText(publishedAt,80),oneSignalId:result.oneSignalId||'',sentAt:new Date().toISOString()})).catch(()=>{});
  return {...result,key:safeKey,title:safeTitle,url:siteUrl};
}
...
async function handleMusicLibraryR317(request, env){
  ...
  const tracks=(listed.objects||[]).filter(o=>musicObjectKeyR317(o.key)).map(o=>{
    ...
    const trikaNo=trikaTrackNumberR517(o),trikaTitle=trikaTrackTitleR517(o);
    return {key:o.key,name:base,title:/^singles\//i.test(o.key)?musicSingleTitleR616(m.title||legacyTitles[o.key]||base):(trikaTitle||m.title||legacyTitles[o.key]||base),artist:m.artist||'',album:m.album||'',track:trikaNo?String(trikaNo):(m.track||''),year:m.year||'',genre:m.genre||'',size:o.size||0,uploaded:o.uploaded||null,url:'https://music.andrikmetal.com/'+o.key};
...
function radioVisualSlotR620(request){
  const slot=String(new URL(request.url).searchParams.get('slot')||'').trim().toLowerCase();
  return Object.prototype.hasOwnProperty.call(RADIO_VISUAL_KEYS_R620,slot)?slot:'';
}
async function handleRadioVisualPutR620(request,env){
  ...
  await bucket.put(key,request.body,{
    ...
    customMetadata:{source:'ANDRIK R620 original master',slot,quality:'1080p25',uploadedBy:'radio-visuals-admin-r620'}
  });
...
async function handleProsnisVideoMpuStartR481(request,env){
  if(!adminAuthorized(request,env))return json({ok:false,error:'unauthorized'},401);
  const bucket=getMusicBucketR314(env);if(!bucket)return json({ok:false,error:'music-bucket-not-configured'},503);
  try{const upload=await bucket.createMultipartUpload(PROSNIS_VIDEO_KEY_R481,{httpMetadata:{contentType:'video/mp4',cacheControl:'public, max-age=86400'},customMetadata:{source:'ANDRIK Control R481 browser multipart',title:'ANDRIK — ПРОСНИСЬ · Visual Fragment',release:'2026'}});return json({ok:true,uploadId:upload.uploadId,key:upload.key,partSize:8*1024*1024});}
  catch(error){return json({ok:false,error:'multipart-start-failed',message:cleanPlainText(error?.message||error,420)},502);}
}
async function handleProsnisVideoMpuPartR481(request,env){
  if(!adminAuthorized(request,env))return json({ok:false,error:'unauthorized'},401);const bucket=getMusicBucketR314(env);if(!bucket)return json({ok:false,error:'music-bucket-not-configured'},503);
  const url=new URL(request.url),uploadId=mediaUploadIdR478(request),partNumber=parseInt(url.searchParams.get('partNumber')||'',10);if(!uploadId||!Number.isFinite(partNumber)||partNumber<1||partNumber>10000)return json({ok:false,error:'invalid-multipart-request'},400);if(!request.body)return json({ok:false,error:'missing-part-body'},400);
  try{const upload=bucket.resumeMultipartUpload(PROSNIS_VIDEO_KEY_R481,uploadId);const part=await upload.uploadPart(partNumber,request.body);return json({ok:true,partNumber:part.partNumber,etag:part.etag});}catch(error){return json({ok:false,error:'multipart-part-failed',message:cleanPlainText(error?.message||error,420)},502);}
}
async function handleProsnisVideoMpuCompleteR481(request,env){
  if(!adminAuthorized(request,env))return json({ok:false,error:'unauthorized'},401);const bucket=getMusicBucketR314(env);if(!bucket)return json({ok:false,error:'music-bucket-not-configured'},503);const uploadId=mediaUploadIdR478(request);if(!uploadId)return json({ok:false,error:'invalid-multipart-request'},400);
  const body=await request.json().catch(()=>null),parts=Array.isArray(body?.parts)?body.parts:[];const normalized=parts.map(p=>({partNumber:parseInt(p?.partNumber,10),etag:String(p?.etag||'')})).filter(p=>Number.isFinite(p.partNumber)&&p.partNumber>0&&p.etag).sort((a,b)=>a.partNumber-b.partNumber);if(!normalized.length||normalized.length!==parts.length)return json({ok:false,error:'invalid-multipart-parts'},400);
  try{const upload=bucket.resumeMultipartUpload(PROSNIS_VIDEO_KEY_R481,uploadId);const object=await upload.complete(normalized);const head=await bucket.head(PROSNIS_VIDEO_KEY_R481);return json({ok:true,key:PROSNIS_VIDEO_KEY_R481,size:Number(head?.size||0),etag:object?.httpEtag||'',url:'/api/media/video/prosnis-fragment.mp4'});}catch(error){return json({ok:false,error:'multipart-complete-failed',message:cleanPlainText(error?.message||error,420)},502);}
}
async function handleProsnisVideoMpuAbortR481(request,env){
  if(!adminAuthorized(request,env))return json({ok:false,error:'unauthorized'},401);const bucket=getMusicBucketR314(env);if(!bucket)return json({ok:false,error:'music-bucket-not-configured'},503);const uploadId=mediaUploadIdR478(request);if(!uploadId)return json({ok:false,error:'invalid-multipart-request'},400);
  try{await bucket.resumeMultipartUpload(PROSNIS_VIDEO_KEY_R481,uploadId).abort();return json({ok:true});}catch(error){return json({ok:false,error:'multipart-abort-failed',message:cleanPlainText(error?.message||error,300)},400);}
}
async function handleProsnisVideoStatusR481(request,env){
...
async function routeApi(request, env, ctx) {
  ...
  try {
    ...
    if (path === '/api/control/media/prosnis-r481/mpu/abort' && request.method === 'DELETE') return await handleProsnisVideoMpuAbortR481(request, env);
    if (path === '/api/control/media/prosnis-r481/status' && request.method === 'GET') return await handleProsnisVideoStatusR481(request, env);
    if (path === '/api/media/video/prosnis-fragment.mp4' && (request.method === 'GET' || request.method === 'HEAD')) return await handleProsnisVideoPublicR481(request, env);
    if (path === '/download/ANDRIK-Prosnis-Fragment-2026.mp4' && (request.method === 'GET' || request.method === 'HEAD')) return await handleProsnisVideoPublicR481(request, env, true);
...
let youtubePublicLiveFallbackCacheR619 = { expiresAt:0, data:null };

async function youtubePublicLiveByApiKeyR619(env, {fresh=false} = {}) {
  const now=Date.now();
  if(!fresh && youtubePublicLiveFallbackCacheR619.data && youtubePublicLiveFallbackCacheR619.expiresAt > now){
    return youtubePublicLiveFallbackCacheR619.data;
  }
  const apiKey=String(env.YOUTUBE_API_KEY||'').trim();
  ...
  if(!channelId){
    ...
    channelUrl.searchParams.set('forHandle',String(env.YOUTUBE_CHANNEL_HANDLE||'@andrikmetal').replace(/^@/,''));
    channelUrl.searchParams.set('maxResults','1');
    channelUrl.searchParams.set('key',apiKey);
    const response=await fetchWithAbortTimeoutR409(channelUrl.toString(),{headers:{accept:'application/json'}},7000,'youtube-public-live-channel-r619');
    const data=await response.json().catch(()=>({}));
    if(response.ok)channelId=cleanPlainText(data?.items?.[0]?.id||'',120);
  }
  ...
  const response=await fetchWithAbortTimeoutR409(searchUrl.toString(),{headers:{accept:'application/json'}},7000,'youtube-public-live-search-r619');
  const data=await response.json().catch(()=>({}));
  if(!response.ok)return null;
  const item=Array.isArray(data?.items)?data.items.find(x=>x?.id?.videoId)||null:null;
  const videoId=cleanPlainText(item?.id?.videoId||'',80);
...
async function handlePublicYoutubeLiveTargetR610(request, env) {
  ...
  const fresh=new URL(request.url).searchParams.get('fresh')==='1';
  // First choice: private Live API (cheap, exact and already available with the old read-only token).
  try {
    const accessToken=await getYoutubeOAuthAccessToken(env);
    const {broadcast,streamStatus}=await youtubeCurrentBroadcastR609(env,accessToken);
    const videoId=cleanPlainText(broadcast?.id||'',80);
    const lifeCycleStatus=youtubeLifeKeyR609(broadcast?.status?.lifeCycleStatus);
    if(videoId && lifeCycleStatus==='live'){
      return json({ok:true,videoId,watchUrl:`https://www.youtube.com/watch?v=${encodeURIComponent(videoId)}`,lifeCycleStatus,streamStatus:cleanPlainText(streamStatus||'',80),active:true,source:'oauth-live-r619'},200,JSON_HEADERS);
    }
  } catch (_) {}

  // R619 fallback does NOT need Studio OAuth. It asks the public YouTube Data API
  // only for an actually LIVE video, never an old completed/created broadcast.
  try {
    const found=await youtubePublicLiveByApiKeyR619(env,{fresh});
    if(found)return json({ok:true,...found,lifeCycleStatus:'live',streamStatus:'active'},200,JSON_HEADERS);
  } catch (_) {}
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (isControlHost && (normalizedPath === '/cache-reset.html' || normalizedPath.startsWith('/cache-reset-v54-') || normalizedPath.startsWith('/cache-reset-v55-'))) {
      ...
      if (request.method === 'GET' || request.method === 'HEAD') {
        ...
        if (isControlHost && (path === '/analytics-admin.html' || path === '/analytics-admin')) {
          ...
          if (!allowedSide && !allowedMap) {
            const adminUrl = new URL('/control-home.html?page=menu&source=launch-guard-r457&v=55.00-r457', url);
